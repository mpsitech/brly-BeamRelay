...
Brlyopd::Brlyopd(
	...
	xchg->shrdatOpprc.init(xchg, &dbsbrly);

	BrlyMapgen::shrdat.init(xchg, &dbsbrly);

	// 5. start op processor threads
	pthread_attr_init(&attr);
	pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);

	cout << "\tstarting " << xchg->stgbrlyopd.opprcn << " operation processor threads ... {" << flush;

	for (unsigned int i = 0; i < xchg->stgbrlyopd.opprcn; i++) {
		if (i != 0) cout << ", " << flush;

		xchg->cStable.lockMutex("Brlyopd", "Brlyopd");

		opprcs.push_back(0);
		pthread_create(&(opprcs[i]), &attr, &BrlyopdOpprc::run, (void*) xchg);

		xchg->cStable.wait();
		xchg->cStable.unlockMutex("Brlyopd", "Brlyopd");
	};
	...
	// 7. cURL to BrlydOpengsrv with DpchBrlyopdReg to retrieve scrNref
	...
	if (CurlPostDpch("http://" + xchg->stgbrlyopd.engip + ":" + to_string(xchg->stgbrlyopd.engport) + "/dpch", xbuf, &doc, &docctx, rplydpch)) {
		// evaluate reply
...
Brlyopd::~Brlyopd() {
	...
	// 1. cURL to BrlydOpengsrv with DpchBrlyOpengUnreg ; note that this will fail in case the termination was initiated by the engine
...
void Brlyopd::loadPref() {
	...
	parseFile(xchg->exedir + "/PrefBrlyopd.xml", &doc, &docctx);

	if (checkUclcXPaths(docctx, basexpath, "/", "PrefBrlyopd")) {
		...
		xchg->stgbrlyopd.readXML(docctx, basexpath, true);
...
void Brlyopd::storePref() {
	...
	startwriteFile(xchg->exedir + "/PrefBrlyopd.xml", &wr);
...
ShrdatBrlyMapgen BrlyMapgen::shrdat;
