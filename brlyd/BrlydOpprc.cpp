/**
	* \file BrlydOpprc.cpp
	* remote operation processor for Brly daemon (implementation)
...
void* BrlydOpprc::run(
	...
	BrlydOpprc_arg* arg = (BrlydOpprc_arg*) _arg;
	...
	BrlydOpprc_cuarg* cuarg = new BrlydOpprc_cuarg();
	cuarg->xchg = xchg;
	cuarg->req = &req;
	cuarg->oref = &oref;
	cuarg->jref = &jref;
	...
	while (true) {
		xchg->cOpprcs.lockMutex("BrlydOpprc", "run");
		inv = xchg->pullFirstInv(node);

		while (!inv) {
			xchg->cOpprcs.wait("BrlydOpprc", "run");
			inv = xchg->pullFirstInv(node);
		};

		xchg->cOpprcs.unlockMutex("BrlydOpprc", "run");
		...
		if ((curlres == CURLE_OK) && req->reply) {
			...
			while ((req->dpchret->ixBrlyVDpch == VecBrlyVDpch::DPCHRETBRLY) && (req->dpchret->ixOpVOpres == VecOpVOpres::PROGRESS)) {
				...
				if ((curlres == CURLE_OK) && req->reply) {
					...
					req->dpchret = new DpchRetBrly(VecBrlyVDpch::DPCHRETBRLY, "", "", VecOpVOpres::COMMERR, 0);
	...
	xchg->setNodeState(node, NodeBrly::VecVState::ERROR);
...
void BrlydOpprc::cleanup(
			void* _arg
		) {
	BrlydOpprc_cuarg* arg = (BrlydOpprc_cuarg*) _arg;
	XchgBrlyd* xchg = arg->xchg;
	ReqBrly* req = *(arg->req);
	...
	xchg->cOpprcs.unlockMutex("BrlydOpprc", "cleanup");
};

uint BrlydOpprc::CurlPostrecv(
			void* data
			, uint size
			, uint blocksize
			, void* _req
