...
DpchBrlyopdReg::DpchBrlyopdReg() :
			DpchBrly(VecBrlyVDpch::DPCHBRLYOPDREG)
		{
};
...
DpchBrlyopdUnreg::DpchBrlyopdUnreg() :
			DpchBrly(VecBrlyVDpch::DPCHBRLYOPDUNREG)
		{
};
...
/******************************************************************************
 class XchgBrlyd::ShrdatJobprc
 ******************************************************************************/

XchgBrlyd::ShrdatJobprc::ShrdatJobprc() :
			ShrdatBrly("XchgBrly", "ShrdatJobprc")
		{
};

void XchgBrlyd::ShrdatJobprc::init(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
		) {
...
void XchgBrlyd::ShrdatJobprc::term(
			XchgBrly* xchg
		) {
...
XchgBrlyd::XchgBrlyd() :
			cStable("cStable", "XchgBrlyd", "XchgBrlyd")
			, mLogfile("mLogfile", "XchgBrlyd", "XchgBrlyd")
			, cOpengcli("cOpengcli", "XchgBrlyd", "XchgBrlyd")
			, cJobprcs("cJobprcs", "XchgBrlyd", "XchgBrlyd")
			, cOpprcs("cOpprcs", "XchgBrlyd", "XchgBrlyd")
			, mReqs("mReqs", "XchgBrlyd", "XchgBrlyd")
			, orefseq("orefseq")
			, mInvs("mInvs", "XchgBrlyd", "XchgBrlyd")
			, rwmPresets("rwmPresets", "XchgBrlyd", "XchgBrlyd")
			, rwmStmgrs("rwmStmgrs", "XchgBrlyd", "XchgBrlyd")
			, rwmClstns("rwmClstns", "XchgBrlyd", "XchgBrlyd")
			, rwmDcols("rwmDcols", "XchgBrlyd", "XchgBrlyd")
			, jrefseq("jrefseq")
			, rwmJobs("rwmJobs", "XchgBrlyd", "XchgBrlyd")
			, rwmCsjobinfos("rwmCsjobinfos", "XchgBrlyd", "XchgBrlyd")
			, wrefseq("wrefseq")
			, nrefseq("nrefseq")
			, rwmNodes("rwmNodes", "XchgBrlyd", "XchgBrlyd")
		{
	// root job
	jrefRoot = 0;
...
void XchgBrlyd::startMon() {
	...
	rwmJobs.runlock("XchgBrlyd", "startMon");

	rwmClstns.rlock("XchgBrlyd", "startMon");
	...
	rwmClstns.runlock("XchgBrlyd", "startMon");

	rwmPresets.rlock("XchgBrlyd", "startMon");
	for (auto it = presets.begin(); it != presets.end(); it++) {
		preset = it->second;
		mon.insertPreset(preset->pref.jref, VecBrlyVPreset::getSref(preset->pref.ixVPreset), preset->arg);
	};
	rwmPresets.runlock("XchgBrlyd", "startMon");

	rwmNodes.rlock("XchgBrlyd", "startMon");
	for (auto it = nodes.begin(); it != nodes.end(); it++) {
		node = it->second;
		mon.insertNode(node->nref, node->ip, node->port, node->opprcn);
	};
	rwmNodes.runlock("XchgBrlyd", "startMon");
...
void XchgBrlyd::appendToLogfile(
	...
	mLogfile.lock("XchgBrlyd", "appendToLogfile");
	...
	mLogfile.unlock("XchgBrlyd", "appendToLogfile");
};
...
void XchgBrlyd::addReq(
	...
	mReqs.lock("XchgBrlyd", "addReq", "jref=" + to_string(req->jref));

	req->ixVState = ReqBrly::VecVState::WAITPRC;
	reqs.push_back(req);

	mReqs.unlock("XchgBrlyd", "addReq", "jref=" + to_string(req->jref));

	cJobprcs.signal("XchgBrlyd", "addReq", "jref=" + to_string(req->jref));
};
...
ReqBrly* XchgBrlyd::pullFirstReq() {
	...
	mReqs.lock("XchgBrlyd", "pullFirstReq");
	...
	mReqs.unlock("XchgBrlyd", "pullFirstReq");
...
void XchgBrlyd::addInvs(
	...
	mInvs.lock("XchgBrlyd", "addInvs", "jref=" + to_string(job->jref));
	job->mOps.lock("XchgBrlyd", "addInvs", "jref=" + to_string(job->jref));
	...
	job->mOps.unlock("XchgBrlyd", "addInvs", "jref=" + to_string(job->jref));
	mInvs.unlock("XchgBrlyd", "addInvs", "jref=" + to_string(job->jref));

	cOpprcs.broadcast("XchgBrlyd", "addInvs", "jref=" + to_string(job->jref));
};

DpchInvBrly* XchgBrlyd::pullFirstInv(
			NodeBrly* node
		) {
	bool match = false;
	...
	mInvs.lock("XchgBrlyd", "pullFirstInv");

	for (auto it = invs.begin(); it != invs.end(); it++) {
		inv = *it;

		if (node->icsBrlyVDpch.find(inv->ixBrlyVDpch) != node->icsBrlyVDpch.end()) {
			// match found
	...
	mInvs.unlock("XchgBrlyd", "pullFirstInv");
...
void XchgBrlyd::returnInv(
	...
	mInvs.lock("XchgBrlyd", "returnInv", "oref=" + to_string(dpchinv->oref));
	invs.push_back(dpchinv);
	mInvs.unlock("XchgBrlyd", "returnInv", "oref=" + to_string(dpchinv->oref));

	cOpprcs.broadcast("XchgBrlyd", "returnInv", "oref=" + to_string(dpchinv->oref));
};
...
Preset* XchgBrlyd::addPreset(
	...
	rwmPresets.wlock("XchgBrlyd", "addPreset", "jref=" + to_string(jref));
	...
	rwmPresets.wunlock("XchgBrlyd", "addPreset", "jref=" + to_string(jref));
...
Preset* XchgBrlyd::getPresetByPref(
	...
	rwmPresets.rlock("XchgBrlyd", "getPresetByPref");

	auto it = presets.find(pref);
	if (it != presets.end()) preset = it->second;

	rwmPresets.runlock("XchgBrlyd", "getPresetByPref");
...
Arg XchgBrlyd::getPreset(
	...
	if ( (ixBrlyVPreset == VecBrlyVPreset::PREBRLYSYSDATE) || (ixBrlyVPreset == VecBrlyVPreset::PREBRLYSYSTIME) || (ixBrlyVPreset == VecBrlyVPreset::PREBRLYSYSSTAMP) ) {
		...
		rwmPresets.rlock("XchgBrlyd", "getPreset", "jref=" + to_string(jref));
		...
		rwmPresets.runlock("XchgBrlyd", "getPreset", "jref=" + to_string(jref));
		rwmJobs.runlock("XchgBrlyd", "getPreset", "jref=" + to_string(jref));
	};
...
void XchgBrlyd::getPresetsByJref(
	...
	rwmPresets.rlock("XchgBrlyd", "getPresetsByJref", "jref=" + to_string(jref));
	...
	rwmPresets.runlock("XchgBrlyd", "getPresetsByJref", "jref=" + to_string(jref));
};
...
void XchgBrlyd::removePreset(
	...
	rwmPresets.wlock("XchgBrlyd", "removePreset", "jref=" + to_string(jref) + ",srefIxBrlyVPreset=" + VecBrlyVPreset::getSref(ixBrlyVPreset));
	...
	rwmPresets.wunlock("XchgBrlyd", "removePreset", "jref=" + to_string(jref) + ",srefIxBrlyVPreset=" + VecBrlyVPreset::getSref(ixBrlyVPreset));
};

void XchgBrlyd::removePresetsByJref(
			const ubigint jref
		) {
	rwmPresets.wlock("XchgBrlyd", "removePresetsByJref", "jref=" + to_string(jref));
	...
	rwmPresets.wunlock("XchgBrlyd", "removePresetsByJref", "jref=" + to_string(jref));
};
...
StmgrBrly* XchgBrlyd::addStmgr(
	...
	if (!stmgr) {
		rwmStmgrs.wlock("XchgBrlyd", "addStmgr", "jref=" + to_string(jref));

		stmgr = new StmgrBrly(this, jref, ixVNonetype);
		stmgrs[jref] = stmgr;

		rwmStmgrs.wunlock("XchgBrlyd", "addStmgr", "jref=" + to_string(jref));
...
StmgrBrly* XchgBrlyd::getStmgrByJref(
	...
	rwmStmgrs.rlock("XchgBrlyd", "getStmgrByJref", "jref=" + to_string(jref));
	...
	rwmStmgrs.runlock("XchgBrlyd", "getStmgrByJref", "jref=" + to_string(jref));
...
void XchgBrlyd::removeStmgrByJref(
	...
	rwmStmgrs.wlock("XchgBrlyd", "removeStmgrByJref", "jref=" + to_string(jref));
	...
	rwmStmgrs.wunlock("XchgBrlyd", "removeStmgrByJref", "jref=" + to_string(jref));
};
...
Clstn* XchgBrlyd::addClstn(
	...
	rwmClstns.wlock("XchgBrlyd", "addClstn", "jref=" + to_string(jref) + ",srefIxBrlyVCall=" + VecBrlyVCall::getSref(ixBrlyVCall));
	...
	rwmClstns.wunlock("XchgBrlyd", "addClstn", "jref=" + to_string(jref) + ",srefIxBrlyVCall=" + VecBrlyVCall::getSref(ixBrlyVCall));
...
Clstn* XchgBrlyd::addClstnStmgr(
	...
	if (!clstn) {
		rwmClstns.wlock("XchgBrlyd", "addClstnStmgr", "jref=" + to_string(jref) + ",srefIxBrlyVCall=" + VecBrlyVCall::getSref(ixBrlyVCall));

		clstn = new Clstn(cref, Clstn::VecVJactype::LOCK);
		clstns.insert(pair<clstnref_t,Clstn*>(cref, clstn));
		cref2sClstns.insert(pair<clstnref2_t,clstnref_t>(cref2, cref));

		rwmClstns.wunlock("XchgBrlyd", "addClstnStmgr", "jref=" + to_string(jref) + ",srefIxBrlyVCall=" + VecBrlyVCall::getSref(ixBrlyVCall));
...
Clstn* XchgBrlyd::getClstnByCref(
	...
	rwmClstns.rlock("XchgBrlyd", "getClstnByCref");

	auto it = clstns.find(cref);
	if (it != clstns.end()) clstn = it->second;

	rwmClstns.runlock("XchgBrlyd", "getClstnByCref");
...
void XchgBrlyd::getClstnsByJref(
	...
	rwmClstns.rlock("XchgBrlyd", "getClstnsByJref", "jref=" + to_string(jref));
	...
	rwmClstns.runlock("XchgBrlyd", "getClstnsByJref", "jref=" + to_string(jref));
};
...
void XchgBrlyd::removeClstns(
	...
	rwmClstns.wlock("XchgBrlyd", "removeClstns", "jref=" + to_string(jref));
	...
	rwmClstns.wunlock("XchgBrlyd", "removeClstns", "jref=" + to_string(jref));
};
...
void XchgBrlyd::removeClstnsByJref(
	...
	rwmClstns.wlock("XchgBrlyd", "removeClstnsByJref", "jref=" + to_string(jref));
	...
	rwmClstns.wunlock("XchgBrlyd", "removeClstnsByJref", "jref=" + to_string(jref));
};
...
void XchgBrlyd::findJrefsByCall(
	...
	rwmJobs.rlock("XchgBrlyd", "findJrefsByCall", "jref=" + to_string(call->jref));
	rwmClstns.rlock("XchgBrlyd", "findJrefsByCall", "jref=" + to_string(call->jref));
	...
	rwmClstns.runlock("XchgBrlyd", "findJrefsByCall", "jref=" + to_string(call->jref));
	rwmJobs.runlock("XchgBrlyd", "findJrefsByCall", "jref=" + to_string(call->jref));
};
			...
			if (job) {
				if (ixVJactype != Clstn::VecVJactype::WEAK) {
					if (ixVJactype == Clstn::VecVJactype::LOCK) {
						...
						if (!job->trylockAccess("XchgBrlyd", "triggerCall")) continue;
					};
				};
...
bool XchgBrlyd::triggerIxSrefToIxCall(
			DbsBrly* dbsbrly
			, const uint ixBrlyVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgBrlyd::triggerSrefCall(
			DbsBrly* dbsbrly
			, const uint ixBrlyVCall
			, const ubigint jref
			, const string& srefInv
...
DcolBrly* XchgBrlyd::addDcol(
	...
	rwmJobs.rlock("XchgBrlyd", "addDcol", "jref=" + to_string(jref));
	...
	if (job) {
		...
		if (!dcol) {
			rwmDcols.wlock("XchgBrlyd", "addDcol", "jref=" + to_string(jref));
			...
			rwmDcols.wunlock("XchgBrlyd", "addDcol", "jref=" + to_string(jref));

			mon.eventAddDcol(jref);
		};
	};

	rwmJobs.runlock("XchgBrlyd", "addDcol", "jref=" + to_string(jref));
...
DcolBrly* XchgBrlyd::getDcolByJref(
	...
	rwmJobs.rlock("XchgBrlyd", "getDcolByJref", "jref=" + to_string(jref));
	rwmDcols.rlock("XchgBrlyd", "getDcolByJref", "jref=" + to_string(jref));
	...
	rwmDcols.runlock("XchgBrlyd", "getDcolByJref", "jref=" + to_string(jref));
	rwmJobs.runlock("XchgBrlyd", "getDcolByJref", "jref=" + to_string(jref));
...
void XchgBrlyd::removeDcolByJref(
	...
	rwmDcols.wlock("XchgBrlyd", "removeDcolByJref", "jref=" + to_string(jref));
	...
	rwmDcols.wunlock("XchgBrlyd", "removeDcolByJref", "jref=" + to_string(jref));
};

void XchgBrlyd::removeDcolsByJref(
			const ubigint jref
		) {
	Jobinfo* jobinfo = NULL;
	...
	rwmJobs.rlock("XchgBrlyd", "removeDcolsByJref", "jref=" + to_string(jref));
	rwmDcols.wlock("XchgBrlyd", "removeDcolsByJref", "jref=" + to_string(jref));
	...
	rwmDcols.wunlock("XchgBrlyd", "removeDcolsByJref", "jref=" + to_string(jref));
	rwmJobs.runlock("XchgBrlyd", "removeDcolsByJref", "jref=" + to_string(jref));
};
...
void XchgBrlyd::submitDpch(
	...
	if (dcol) {
		...
		if (dcol->req != NULL) {
			...
			cReady_backup->signal("XchgBrlyd", "submitDpch", "jref=" + to_string(dcol->jref));
...
ubigint XchgBrlyd::addJob(
	...
	rwmJobs.wlock("XchgBrlyd", "addJob", "jrefSup=" + to_string(jrefSup));
	...
	if (csjobNotJob) {
		rwmCsjobinfos.wlock("XchgBrlyd", "addJob", "jref=" + to_string(job->jref));
		...
		rwmCsjobinfos.wunlock("XchgBrlyd", "addJob", "jref=" + to_string(job->jref));
	};

	rwmJobs.wunlock("XchgBrlyd", "addJob", "jref=" + to_string(job->jref));
...
JobBrly* XchgBrlyd::getJobByJref(
	...
	rwmJobs.rlock("XchgBrlyd", "getJobByJref", "jref=" + to_string(jref));

	auto it = jobs.find(jref);
	if (it != jobs.end()) job = it->second;

	rwmJobs.runlock("XchgBrlyd", "getJobByJref", "jref=" + to_string(jref));

	return job;
};

Jobinfo* XchgBrlyd::getJobinfoByJref(
			const ubigint jref
		) {
	Jobinfo* jobinfo = NULL;

	rwmJobs.rlock("XchgBrlyd", "getJobinfoByJref", "jref=" + to_string(jref));

	auto it = jobinfos.find(jref);
	if (it != jobinfos.end()) jobinfo = it->second;

	rwmJobs.runlock("XchgBrlyd", "getJobinfoByJref", "jref=" + to_string(jref));
...
void XchgBrlyd::removeJobByJref(
	...
	rwmJobs.rlock("XchgBrlyd", "removeJobByJref[1]", "jref=" + to_string(jref));
	...
	if (job) {
		...
		if (csjobNotJob) {
			rwmCsjobinfos.rlock("XchgBrlyd", "removeJobByJref[1]", "jref=" + to_string(jref));
			...
			rwmCsjobinfos.runlock("XchgBrlyd", "removeJobByJref[1]", "jref=" + to_string(jref));

			rwmCsjobinfos.wlock("XchgBrlyd", "removeJobByJref[2]", "jref=" + to_string(jref));
			...
			rwmCsjobinfos.wunlock("XchgBrlyd", "removeJobByJref[2]", "jref=" + to_string(jref));
		};

		jobinfo = jobinfos[jref];

		rwmJobs.runlock("XchgBrlyd", "removeJobByJref[1]", "jref=" + to_string(jref));

		rwmJobs.wlock("XchgBrlyd", "removeJobByJref[2]", "jref=" + to_string(jref));

		if (jobinfo->jrefSup != 0) jobinfos[jobinfo->jrefSup]->jrefsSub.erase(jref);

		rwmJobs.wunlock("XchgBrlyd", "removeJobByJref[2]", "jref=" + to_string(jref));

		while (jobinfo->jrefsSub.size() > 0) delete jobs[*(jobinfo->jrefsSub.begin())];

		rwmJobs.wlock("XchgBrlyd", "removeJobByJref[3]", "jref=" + to_string(jref));
		...
		rwmJobs.wunlock("XchgBrlyd", "removeJobByJref[3]", "jref=" + to_string(jref));

		mon.eventRemoveJob(jref);

	} else rwmJobs.wunlock("XchgBrlyd", "removeJobByJref[4]", "jref=" + to_string(jref));
};

bool XchgBrlyd::getJobSup(
			const ubigint jrefSup
			, ubigint jref
		) {
	bool retval = false;
...
void XchgBrlyd::setJobStage(
	...
	rwmJobs.rlock("XchgBrlyd", "setJobStage", "jref=" + to_string(job->jref));
	if (csjobNotJob) rwmCsjobinfos.rlock("XchgBrlyd", "setJobStage", "jref=" + to_string(job->jref));
	...
	if (csjobNotJob) rwmCsjobinfos.runlock("XchgBrlyd", "setJobStage", "jref=" + to_string(job->jref));
	rwmJobs.runlock("XchgBrlyd", "setJobStage", "jref=" + to_string(job->jref));

	triggerIxCall(dbsbrly, VecBrlyVCall::CALLBRLYSGECHG, job->jref, ixVSge);
};

void XchgBrlyd::addCsjobClaim(
			DbsBrly* dbsbrly
			, CsjobBrly* csjob
			, Claim* claim
		) {
	...
	if (!csjob->srvNotCli && csjob->srv) {
		rwmCsjobinfos.rlock("XchgBrlyd", "addCsjobClaim", "jref=" + to_string(csjob->jref));

		csjobinfo = csjobinfos[csjob->ixBrlyVJob];

		rwmCsjobinfos.runlock("XchgBrlyd", "addCsjobClaim", "jref=" + to_string(csjob->jref));

		csjob->srv->lockAccess("XchgBrlyd", "addCsjobClaim");

		csjobinfo->mClaims.wlock("XchgBrlyd", "addCsjobClaim", "jref=" + to_string(csjob->jref));
		...
		csjobinfo->mClaims.wunlock("XchgBrlyd", "addCsjobClaim", "jref=" + to_string(csjob->jref));

		csjob->srv->unlockAccess("XchgBrlyd", "addCsjobClaim");

		if (mod) triggerCall(dbsbrly, VecBrlyVCall::CALLBRLYCLAIMCHG, csjob->jref);
	};
};

bool XchgBrlyd::getCsjobClaim(
			CsjobBrly* csjob
	...
	if (!csjob->srvNotCli && csjob->srv) {
		rwmCsjobinfos.rlock("XchgBrlyd", "getCsjobClaim", "jref=" + to_string(csjob->jref));

		csjobinfo = csjobinfos[csjob->ixBrlyVJob];

		csjobinfo->mClaims.rlock("XchgBrlyd", "getCsjobClaim", "jref=" + to_string(csjob->jref));
		...
		csjobinfo->mClaims.runlock("XchgBrlyd", "getCsjobClaim", "jref=" + to_string(csjob->jref));

		rwmCsjobinfos.runlock("XchgBrlyd", "getCsjobClaim", "jref=" + to_string(csjob->jref));
	};
...
void XchgBrlyd::clearCsjobRun(
	...
	rwmCsjobinfos.rlock("XchgBrlyd", "clearCsjobRun", "srefIxBrlyVJob=" + VecBrlyVJob::getSref(ixBrlyVJob));

	auto it = csjobinfos.find(ixBrlyVJob);
	if (it != csjobinfos.end()) csjobinfo = it->second;

	rwmCsjobinfos.runlock("XchgBrlyd", "clearCsjobRun", "srefIxBrlyVJob=" + VecBrlyVJob::getSref(ixBrlyVJob));

	if (csjobinfo) {
		csjobinfo->mClaims.wlock("XchgBrlyd", "clearCsjobRun", "srefIxBrlyVJob=" + VecBrlyVJob::getSref(ixBrlyVJob));
		...
		csjobinfo->mClaims.wunlock("XchgBrlyd", "clearCsjobRun", "srefIxBrlyVJob=" + VecBrlyVJob::getSref(ixBrlyVJob));

		if (mod) triggerCall(dbsbrly, VecBrlyVCall::CALLBRLYCLAIMCHG, csjobinfo->jrefSrv);
	};
};

void XchgBrlyd::removeCsjobClaim(
			DbsBrly* dbsbrly
			, CsjobBrly* csjob
		) {
	Csjobinfo* csjobinfo = NULL;;
	...
	if (!csjob->srvNotCli && csjob->srv) {
		rwmCsjobinfos.rlock("XchgBrlyd", "removeCsjobClaim", "jref=" + to_string(csjob->jref));

		csjobinfo = csjobinfos[csjob->ixBrlyVJob];

		rwmCsjobinfos.runlock("XchgBrlyd", "removeCsjobClaim", "jref=" + to_string(csjob->jref));

		csjob->srv->lockAccess("XchgBrlyd", "removeCsjobClaim");

		csjobinfo->mClaims.wlock("XchgBrlyd", "removeCsjobClaim", "jref=" + to_string(csjob->jref));
		...
		csjobinfo->mClaims.wunlock("XchgBrlyd", "removeCsjobClaim", "jref=" + to_string(csjob->jref));

		csjob->srv->unlockAccess("XchgBrlyd", "removeCsjobClaim");

		if (mod) triggerCall(dbsbrly, VecBrlyVCall::CALLBRLYCLAIMCHG, csjobinfo->jrefSrv);
	};
};

bool XchgBrlyd::getCsjobNotJob(
			const uint ixBrlyVJob
		) {
	bool retval;

	rwmCsjobinfos.rlock("XchgBrlyd", "getCsjobNotJob", "srefIxBrlyVJob=" + VecBrlyVJob::getSref(ixBrlyVJob));

	retval = (csjobinfos.find(ixBrlyVJob) != csjobinfos.end());

	rwmCsjobinfos.runlock("XchgBrlyd", "getCsjobNotJob", "srefIxBrlyVJob=" + VecBrlyVJob::getSref(ixBrlyVJob));

	return retval;
};

set<ubigint> XchgBrlyd::getCsjobClisByJref(
			const ubigint jref
		) {
	set<ubigint> retval;

	rwmCsjobinfos.rlock("XchgBrlyd", "getCsjobClis", "jref=" + to_string(jref));

	auto it = csjobinfos.find(ixBrlyVJobs[jref]);
	
	if (it != csjobinfos.end()) retval = it->second->jrefsCli;

	rwmCsjobinfos.runlock("XchgBrlyd", "getCsjobClis", "jref=" + to_string(jref));
...
NodeBrly* XchgBrlyd::addNode(
	...
	rwmNodes.wlock("XchgBrlyd", "addNode", "ip:port=" + ip + ":" + to_string(port));

	node = new NodeBrly(nrefseq.getNewRef(), ip, port, opprcn, ixBrlyVOpengtype);
	nodes[node->nref] = node;

	rwmNodes.wunlock("XchgBrlyd", "addNode", "nref=" + to_string(node->nref));

	mon.eventAddNode(node->nref, ip, port, opprcn);

	setNodeState(node, NodeBrly::VecVState::IDLE);
...
NodeBrly* XchgBrlyd::getNodeByNref(
	...
	rwmNodes.rlock("XchgBrlyd", "getNodeByNref", "nref=" + to_string(nref));

	auto it = nodes.find(nref);
	if (it != nodes.end()) node = it->second;

	rwmNodes.runlock("XchgBrlyd", "getNodeByNref", "nref=" + to_string(nref));

	return(node);
};

void XchgBrlyd::terminateNodes() {
	rwmNodes.wlock("XchgBrlyd", "terminateNodes");

	for (auto it = nodes.begin(); it != nodes.end(); it++) terminateNodeByNref(it->first);

	rwmNodes.wunlock("XchgBrlyd", "terminateNodes");
};

void XchgBrlyd::terminateNodeByNref(
			const ubigint nref
		) {
	NodeBrly* node = NULL;

	rwmNodes.wlock("XchgBrlyd", "terminateNodeByNref", "nref=" + to_string(nref));

	node = getNodeByNref(nref);
	if (node) setNodeState(node, NodeBrly::VecVState::TERM);

	rwmNodes.wunlock("XchgBrlyd", "terminateNodeByNref", "nref=" + to_string(nref));
};

void XchgBrlyd::setNodeState(
			NodeBrly* node
			, const uint ixVState
		) {
	cOpengcli.lockMutex("XchgBrlyd", "setNodeState", "nref=" + to_string(node->nref));
	rwmNodes.wlock("XchgBrlyd", "setNodeState", "nref=" + to_string(node->nref));

	if ( ((ixVState == NodeBrly::VecVState::ERROR) && (node->ixVState == NodeBrly::VecVState::READY)) || (ixVState == NodeBrly::VecVState::TERM)) node->ixVState = ixVState;

	rwmNodes.wunlock("XchgBrlyd", "setNodeState", "nref=" + to_string(node->nref));
	cOpengcli.unlockMutex("XchgBrlyd", "setNodeState", "nref=" + to_string(node->nref));

	cOpengcli.signal("XchgBrlyd", "setNodeState", "nref=" + to_string(node->nref));
};
