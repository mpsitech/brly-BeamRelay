...
/**
	* DpchBrlydAck (written by dOpengsrv, identified by opd_exe)
	*/
...
public:
	...
	static const Sbecore::uint IXBRLYVOPENGTYPE = 2;
...
public:
	StgBrlyd(const Sbecore::usmallint jobprcn = 4, const Sbecore::usmallint opengsrvport = 13120, const bool appsrv = true);
...
public:
	bool readXML(xmlXPathContext* docctx, std::string basexpath = "", bool addbasetag = false);
	void writeXML(xmlTextWriter* wr, std::string difftag = "", bool shorttags = true);
	std::set<Sbecore::uint> comm(const StgBrlyd* comp);
	std::set<Sbecore::uint> diff(const StgBrlyd* comp);
};
...
public:
	ReqopengconBrly(const Sbecore::uint ixVState = VecVState::RECEIVE, const std::string& ip = "");
	~ReqopengconBrly();
...
public:
	NodeBrly(const Sbecore::ubigint nref, const std::string& ip, const Sbecore::uint port, const Sbecore::uint opprcn, const Sbecore::uint ixBrlyVOpengtype);
	~NodeBrly();
...
public:
	...
	std::set<Sbecore::uint> icsBrlyVDpch;
...
public:
	...
	// condition for operation processors (sub-threads of BrlydOpengcli)
	...
	std::map<Sbecore::ubigint,NodeBrly*> nodes;
...
public:
	...
	DpchInvBrly* pullFirstInv(NodeBrly* node);
	...
	// node methods
	NodeBrly* addNode(const std::string& ip, const Sbecore::uint port, const Sbecore::uint opprcn, const Sbecore::uint ixBrlyVOpengtype);
	NodeBrly* getNodeByNref(const Sbecore::ubigint nref);

	void terminateNodes();
	void terminateNodeByNref(const Sbecore::ubigint nref);

	void setNodeState(NodeBrly* node, const Sbecore::uint ixVState);
};
#endif
