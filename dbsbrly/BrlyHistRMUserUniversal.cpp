...
ubigint TblBrlyHistRMUserUniversal::insertRec(
			BrlyHistRMUserUniversal* rec
		) {
	return 0;
};
...
bool TblBrlyHistRMUserUniversal::loadRecByUsrMtbUnvCrd(
...
ubigint TblBrlyHistRMUserUniversal::loadRefsByUsrMtbCrd(
			ubigint refBrlyMUser
			, uint unvIxBrlyVMaintable
			, uint ixBrlyVCard
			, const bool append
...
bool MyTblBrlyHistRMUserUniversal::loadRecBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyHistRMUserUniversal::loadRecBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyHistRMUserUniversal::loadRecBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyHistRMUserUniversal::loadRstBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyHistRMUserUniversal::loadRstBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyHistRMUserUniversal::loadRstBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyHistRMUserUniversal::insertRec(
			BrlyHistRMUserUniversal* rec
		) {
	...
	if (mysql_stmt_bind_param(stmtInsertRec, bind)) {
		string dbms = "MyTblBrlyHistRMUserUniversal::insertRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtInsertRec)) {
		string dbms = "MyTblBrlyHistRMUserUniversal::insertRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void MyTblBrlyHistRMUserUniversal::updateRec(
	...
	if (mysql_stmt_bind_param(stmtUpdateRec, bind)) {
		string dbms = "MyTblBrlyHistRMUserUniversal::updateRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtUpdateRec)) {
		string dbms = "MyTblBrlyHistRMUserUniversal::updateRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void MyTblBrlyHistRMUserUniversal::removeRecByRef(
	...
	if (mysql_stmt_bind_param(stmtRemoveRecByRef, &bind)) {
		string dbms = "MyTblBrlyHistRMUserUniversal::removeRecByRef() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtRemoveRecByRef)) {
		string dbms = "MyTblBrlyHistRMUserUniversal::removeRecByRef() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
bool MyTblBrlyHistRMUserUniversal::loadRecByUsrMtbUnvCrd(
			ubigint refBrlyMUser
			, uint unvIxBrlyVMaintable
			, ubigint unvUref
			, uint ixBrlyVCard
	...
	return loadRecBySQL("SELECT ref, refBrlyMUser, unvIxBrlyVMaintable, unvUref, ixBrlyVCard, ixBrlyVPreset, preIxBrlyVMaintable, preUref, start FROM TblBrlyHistRMUserUniversal WHERE refBrlyMUser = " + to_string(refBrlyMUser) + " AND unvIxBrlyVMaintable = " + to_string(unvIxBrlyVMaintable) + " AND unvUref = " + to_string(unvUref) + " AND ixBrlyVCard = " + to_string(ixBrlyVCard) + "", rec);
};

ubigint MyTblBrlyHistRMUserUniversal::loadRefsByUsrMtbCrd(
			ubigint refBrlyMUser
			, uint unvIxBrlyVMaintable
			, uint ixBrlyVCard
			, const bool append
	...
	return loadRefsBySQL("SELECT ref FROM TblBrlyHistRMUserUniversal WHERE refBrlyMUser = " + to_string(refBrlyMUser) + " AND unvIxBrlyVMaintable = " + to_string(unvIxBrlyVMaintable) + " AND ixBrlyVCard = " + to_string(ixBrlyVCard) + " ORDER BY start DESC LIMIT " + to_string(offset) + "," + to_string(limit) + "", append, refs);
};
...
void PgTblBrlyHistRMUserUniversal::initStatements() {
	createStatement("TblBrlyHistRMUserUniversal_insertRec", "INSERT INTO TblBrlyHistRMUserUniversal (refBrlyMUser, unvIxBrlyVMaintable, unvUref, ixBrlyVCard, ixBrlyVPreset, preIxBrlyVMaintable, preUref, start) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING ref", 8);
	createStatement("TblBrlyHistRMUserUniversal_updateRec", "UPDATE TblBrlyHistRMUserUniversal SET refBrlyMUser = $1, unvIxBrlyVMaintable = $2, unvUref = $3, ixBrlyVCard = $4, ixBrlyVPreset = $5, preIxBrlyVMaintable = $6, preUref = $7, start = $8 WHERE ref = $9", 9);
	createStatement("TblBrlyHistRMUserUniversal_removeRecByRef", "DELETE FROM TblBrlyHistRMUserUniversal WHERE ref = $1", 1);

	createStatement("TblBrlyHistRMUserUniversal_loadRecByRef", "SELECT ref, refBrlyMUser, unvIxBrlyVMaintable, unvUref, ixBrlyVCard, ixBrlyVPreset, preIxBrlyVMaintable, preUref, start FROM TblBrlyHistRMUserUniversal WHERE ref = $1", 1);
	createStatement("TblBrlyHistRMUserUniversal_loadRecByUsrMtbUnvCrd", "SELECT ref, refBrlyMUser, unvIxBrlyVMaintable, unvUref, ixBrlyVCard, ixBrlyVPreset, preIxBrlyVMaintable, preUref, start FROM TblBrlyHistRMUserUniversal WHERE refBrlyMUser = $1 AND unvIxBrlyVMaintable = $2 AND unvUref = $3 AND ixBrlyVCard = $4", 4);
	createStatement("TblBrlyHistRMUserUniversal_loadRefsByUsrMtbCrd", "SELECT ref FROM TblBrlyHistRMUserUniversal WHERE refBrlyMUser = $1 AND unvIxBrlyVMaintable = $2 AND ixBrlyVCard = $3 ORDER BY start DESC OFFSET $4 LIMIT $5", 5);
	createStatement("TblBrlyHistRMUserUniversal_loadRstByUsrCrd", "SELECT ref, refBrlyMUser, unvIxBrlyVMaintable, unvUref, ixBrlyVCard, ixBrlyVPreset, preIxBrlyVMaintable, preUref, start FROM TblBrlyHistRMUserUniversal WHERE refBrlyMUser = $1 AND ixBrlyVCard = $2 ORDER BY start DESC", 2);
	createStatement("TblBrlyHistRMUserUniversal_loadUnuByRef", "SELECT unvUref FROM TblBrlyHistRMUserUniversal WHERE ref = $1", 1);
};
...
bool PgTblBrlyHistRMUserUniversal::loadRecByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyHistRMUserUniversal::loadRecByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
ubigint PgTblBrlyHistRMUserUniversal::loadRstByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyHistRMUserUniversal::loadRstByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
bool PgTblBrlyHistRMUserUniversal::loadRecBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyHistRMUserUniversal::loadRecBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyHistRMUserUniversal::loadRstBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyHistRMUserUniversal::loadRstBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyHistRMUserUniversal::insertRec(
			BrlyHistRMUserUniversal* rec
		) {
	PGresult* res;
	char* ptr;
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyHistRMUserUniversal::insertRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyHistRMUserUniversal::updateRec(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyHistRMUserUniversal::updateRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyHistRMUserUniversal::removeRecByRef(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyHistRMUserUniversal::removeRecByRef() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
bool PgTblBrlyHistRMUserUniversal::loadRecByUsrMtbUnvCrd(
			ubigint refBrlyMUser
			, uint unvIxBrlyVMaintable
			, ubigint unvUref
			, uint ixBrlyVCard
	...
	return loadRecByStmt("TblBrlyHistRMUserUniversal_loadRecByUsrMtbUnvCrd", 4, vals, l, f, rec);
};

ubigint PgTblBrlyHistRMUserUniversal::loadRefsByUsrMtbCrd(
			ubigint refBrlyMUser
			, uint unvIxBrlyVMaintable
			, uint ixBrlyVCard
			, const bool append
	...
	return loadRefsByStmt("TblBrlyHistRMUserUniversal_loadRefsByUsrMtbCrd", 3, vals, l, f, append, refs);
};
