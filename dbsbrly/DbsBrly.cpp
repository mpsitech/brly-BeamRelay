...
void DbsBrly::initMy() {
	...
	if (!mysql_real_connect(dbsMy, ip.c_str(), username.c_str(), password.c_str(), dbsname.c_str(), port, NULL, 0)) {
		string dbms = "DbsBrly::initMy() / " + string(mysql_error(dbsMy));
...
void DbsBrly::initPg() {
	...
	if (PQstatus(dbsPg) == CONNECTION_BAD) {
		string dbms = "DbsBrly::initPg() / " + string(PQerrorMessage(dbsPg));
		throw SbeException(SbeException::DBS_CONN, {{"dbms",dbms}});
	};
...
void DbsBrly::beginMy() {
	if (mysql_query(dbsMy, "BEGIN")) {
		string dbms = "DbsBrly::beginMy() / " + string(mysql_error(dbsMy));
...
void DbsBrly::rollbackMy() {
	if (mysql_query(dbsMy, "ROLLBACK")) {
		string dbms = "DbsBrly::rollbackMy() / " + string(mysql_error(dbsMy));
...
void DbsBrly::beginPg() {
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "DbsBrly::beginPg() / " + string(PQerrorMessage(dbsPg));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql","BEGIN"}});
	};
...
void DbsBrly::rollbackPg() {
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "DbsBrly::rollbackPg() / " + string(PQerrorMessage(dbsPg));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql","ROLLBACK"}});
	};
...
bool DbsBrly::loadUintBySQLMy(
	...
	if (mysql_real_query(dbsMy, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "DbsBrly::loadUintBySQLMy() / " + string(mysql_error(dbsMy));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbsMy);
	if (!dbresult) {
		string dbms = "DbsBrly::loadUintBySQLMy() / store result / " + string(mysql_error(dbsMy));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
bool DbsBrly::loadStringBySQLMy(
	...
	if (mysql_real_query(dbsMy, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "DbsBrly::loadStringBySQLMy() / " + string(mysql_error(dbsMy));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbsMy);
	if (!dbresult) {
		string dbms = "DbsBrly::loadStringBySQLMy() / store result / " + string(mysql_error(dbsMy));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
bool DbsBrly::loadRefBySQLMy(
	...
	if (mysql_real_query(dbsMy, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "DbsBrly::loadRefBySQLMy() / " + string(mysql_error(dbsMy));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbsMy);
	if (!dbresult) {
		string dbms = "DbsBrly::loadRefBySQLMy() / store result / " + string(mysql_error(dbsMy));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint DbsBrly::loadRefsBySQLMy(
	...
	if (mysql_real_query(dbsMy, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "DbsBrly::loadRefsBySQLMy() / " + string(mysql_error(dbsMy));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbsMy);
	if (!dbresult) {
		string dbms = "DbsBrly::loadRefsBySQLMy() / store result / " + string(mysql_error(dbsMy));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
bool DbsBrly::loadUintBySQLPg(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "DbsBrly::loadUintBySQLPg() / " + string(PQerrorMessage(dbsPg));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
bool DbsBrly::loadStringBySQLPg(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "DbsBrly::loadStringBySQLPg() / " + string(PQerrorMessage(dbsPg));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
bool DbsBrly::loadRefBySQLPg(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "DbsBrly::loadRefBySQLPg() / " + string(PQerrorMessage(dbsPg));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint DbsBrly::loadRefsBySQLPg(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "DbsBrly::loadRefsBySQLPg() / " + string(PQerrorMessage(dbsPg));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
void DbsBrly::executeQuery(
	...
	if (ixDbsVDbstype == VecDbsVDbstype::MY)
		if (mysql_real_query(dbsMy, sqlstr.c_str(), strlen(sqlstr.c_str()))) {
			string dbms = "DbsBrly::executeQuery() / MySQL / " + string(mysql_error(dbsMy));
			throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
		};
#endif
	...
	if (ixDbsVDbstype == VecDbsVDbstype::PG) {
		...
		res = PQexec(dbsPg, sqlstr.c_str());
		if (PQresultStatus(res) != PGRES_COMMAND_OK) {
			string dbms = "DbsBrly::executeQuery() / PgSQL / " + string(PQerrorMessage(dbsPg));
			throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
		};
	};
#endif
