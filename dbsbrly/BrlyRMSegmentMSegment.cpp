...
ubigint TblBrlyRMSegmentMSegment::insertRec(
			BrlyRMSegmentMSegment* rec
		) {
	return 0;
};
...
bool MyTblBrlyRMSegmentMSegment::loadRecBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyRMSegmentMSegment::loadRecBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyRMSegmentMSegment::loadRecBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyRMSegmentMSegment::loadRstBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyRMSegmentMSegment::loadRstBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyRMSegmentMSegment::loadRstBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyRMSegmentMSegment::insertRec(
			BrlyRMSegmentMSegment* rec
		) {
	...
	if (mysql_stmt_bind_param(stmtInsertRec, bind)) {
		string dbms = "MyTblBrlyRMSegmentMSegment::insertRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtInsertRec)) {
		string dbms = "MyTblBrlyRMSegmentMSegment::insertRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void MyTblBrlyRMSegmentMSegment::updateRec(
	...
	if (mysql_stmt_bind_param(stmtUpdateRec, bind)) {
		string dbms = "MyTblBrlyRMSegmentMSegment::updateRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtUpdateRec)) {
		string dbms = "MyTblBrlyRMSegmentMSegment::updateRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void MyTblBrlyRMSegmentMSegment::removeRecByRef(
	...
	if (mysql_stmt_bind_param(stmtRemoveRecByRef, &bind)) {
		string dbms = "MyTblBrlyRMSegmentMSegment::removeRecByRef() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtRemoveRecByRef)) {
		string dbms = "MyTblBrlyRMSegmentMSegment::removeRecByRef() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void PgTblBrlyRMSegmentMSegment::initStatements() {
	createStatement("TblBrlyRMSegmentMSegment_insertRec", "INSERT INTO TblBrlyRMSegmentMSegment (x1Start, x1Stop, x2VisDstSegphi0, x2VisDstSegphi1, dstRefBrlyMSegment, orgRefBrlyMSegment, VisOrgSegphi0, VisOrgSegphi1) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING ref", 8);
	createStatement("TblBrlyRMSegmentMSegment_updateRec", "UPDATE TblBrlyRMSegmentMSegment SET x1Start = $1, x1Stop = $2, x2VisDstSegphi0 = $3, x2VisDstSegphi1 = $4, dstRefBrlyMSegment = $5, orgRefBrlyMSegment = $6, VisOrgSegphi0 = $7, VisOrgSegphi1 = $8 WHERE ref = $9", 9);
	createStatement("TblBrlyRMSegmentMSegment_removeRecByRef", "DELETE FROM TblBrlyRMSegmentMSegment WHERE ref = $1", 1);

	createStatement("TblBrlyRMSegmentMSegment_loadRecByRef", "SELECT ref, x1Start, x1Stop, x2VisDstSegphi0, x2VisDstSegphi1, dstRefBrlyMSegment, orgRefBrlyMSegment, VisOrgSegphi0, VisOrgSegphi1 FROM TblBrlyRMSegmentMSegment WHERE ref = $1", 1);
	createStatement("TblBrlyRMSegmentMSegment_loadRstByDsg", "SELECT ref, x1Start, x1Stop, x2VisDstSegphi0, x2VisDstSegphi1, dstRefBrlyMSegment, orgRefBrlyMSegment, VisOrgSegphi0, VisOrgSegphi1 FROM TblBrlyRMSegmentMSegment WHERE dstRefBrlyMSegment = $1 ORDER BY x1VisDstSegphi0 ASC", 1);
	createStatement("TblBrlyRMSegmentMSegment_loadRstByOsg", "SELECT ref, x1Start, x1Stop, x2VisDstSegphi0, x2VisDstSegphi1, dstRefBrlyMSegment, orgRefBrlyMSegment, VisOrgSegphi0, VisOrgSegphi1 FROM TblBrlyRMSegmentMSegment WHERE orgRefBrlyMSegment = $1", 1);
};
...
bool PgTblBrlyRMSegmentMSegment::loadRecByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMSegmentMSegment::loadRecByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
ubigint PgTblBrlyRMSegmentMSegment::loadRstByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMSegmentMSegment::loadRstByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
bool PgTblBrlyRMSegmentMSegment::loadRecBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMSegmentMSegment::loadRecBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyRMSegmentMSegment::loadRstBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMSegmentMSegment::loadRstBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyRMSegmentMSegment::insertRec(
			BrlyRMSegmentMSegment* rec
		) {
	PGresult* res;
	char* ptr;
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMSegmentMSegment::insertRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyRMSegmentMSegment::updateRec(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyRMSegmentMSegment::updateRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyRMSegmentMSegment::removeRecByRef(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyRMSegmentMSegment::removeRecByRef() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
