...
ubigint TblBrlyRMFlightMFlight::insertRec(
			BrlyRMFlightMFlight* rec
		) {
	return 0;
};
...
bool MyTblBrlyRMFlightMFlight::loadRecBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyRMFlightMFlight::loadRecBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyRMFlightMFlight::loadRecBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyRMFlightMFlight::loadRstBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyRMFlightMFlight::loadRstBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyRMFlightMFlight::loadRstBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyRMFlightMFlight::insertRec(
			BrlyRMFlightMFlight* rec
		) {
	...
	if (mysql_stmt_bind_param(stmtInsertRec, bind)) {
		string dbms = "MyTblBrlyRMFlightMFlight::insertRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtInsertRec)) {
		string dbms = "MyTblBrlyRMFlightMFlight::insertRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void MyTblBrlyRMFlightMFlight::updateRec(
	...
	if (mysql_stmt_bind_param(stmtUpdateRec, bind)) {
		string dbms = "MyTblBrlyRMFlightMFlight::updateRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtUpdateRec)) {
		string dbms = "MyTblBrlyRMFlightMFlight::updateRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void MyTblBrlyRMFlightMFlight::removeRecByRef(
	...
	if (mysql_stmt_bind_param(stmtRemoveRecByRef, &bind)) {
		string dbms = "MyTblBrlyRMFlightMFlight::removeRecByRef() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtRemoveRecByRef)) {
		string dbms = "MyTblBrlyRMFlightMFlight::removeRecByRef() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void PgTblBrlyRMFlightMFlight::initStatements() {
	createStatement("TblBrlyRMFlightMFlight_insertRec", "INSERT INTO TblBrlyRMFlightMFlight (x1VisStart, x1VisStop, dstRefBrlyMFlight, orgRefBrlyMFlight, xVisStart, xVisStop) VALUES ($1,$2,$3,$4,$5,$6) RETURNING ref", 6);
	createStatement("TblBrlyRMFlightMFlight_updateRec", "UPDATE TblBrlyRMFlightMFlight SET x1VisStart = $1, x1VisStop = $2, dstRefBrlyMFlight = $3, orgRefBrlyMFlight = $4, xVisStart = $5, xVisStop = $6 WHERE ref = $7", 7);
	createStatement("TblBrlyRMFlightMFlight_removeRecByRef", "DELETE FROM TblBrlyRMFlightMFlight WHERE ref = $1", 1);

	createStatement("TblBrlyRMFlightMFlight_loadRecByRef", "SELECT ref, x1VisStart, x1VisStop, dstRefBrlyMFlight, orgRefBrlyMFlight, xVisStart, xVisStop FROM TblBrlyRMFlightMFlight WHERE ref = $1", 1);
	createStatement("TblBrlyRMFlightMFlight_loadRstByDfl", "SELECT ref, x1VisStart, x1VisStop, dstRefBrlyMFlight, orgRefBrlyMFlight, xVisStart, xVisStop FROM TblBrlyRMFlightMFlight WHERE dstRefBrlyMFlight = $1 ORDER BY x1VisStart ASC", 1);
	createStatement("TblBrlyRMFlightMFlight_loadRstByOfl", "SELECT ref, x1VisStart, x1VisStop, dstRefBrlyMFlight, orgRefBrlyMFlight, xVisStart, xVisStop FROM TblBrlyRMFlightMFlight WHERE orgRefBrlyMFlight = $1 ORDER BY x1VisStart ASC", 1);
};
...
bool PgTblBrlyRMFlightMFlight::loadRecByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMFlightMFlight::loadRecByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
ubigint PgTblBrlyRMFlightMFlight::loadRstByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMFlightMFlight::loadRstByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
bool PgTblBrlyRMFlightMFlight::loadRecBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMFlightMFlight::loadRecBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyRMFlightMFlight::loadRstBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMFlightMFlight::loadRstBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyRMFlightMFlight::insertRec(
			BrlyRMFlightMFlight* rec
		) {
	PGresult* res;
	char* ptr;
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMFlightMFlight::insertRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyRMFlightMFlight::updateRec(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyRMFlightMFlight::updateRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyRMFlightMFlight::removeRecByRef(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyRMFlightMFlight::removeRecByRef() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
