...
ubigint TblBrlyProxRMLocationMLocation::insertRec(
			BrlyProxRMLocationMLocation* rec
		) {
	return 0;
};
...
bool MyTblBrlyProxRMLocationMLocation::loadRecBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyProxRMLocationMLocation::loadRecBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyProxRMLocationMLocation::loadRecBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyProxRMLocationMLocation::loadRstBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyProxRMLocationMLocation::loadRstBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyProxRMLocationMLocation::loadRstBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyProxRMLocationMLocation::insertRec(
			BrlyProxRMLocationMLocation* rec
		) {
	...
	if (mysql_stmt_bind_param(stmtInsertRec, bind)) {
		string dbms = "MyTblBrlyProxRMLocationMLocation::insertRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtInsertRec)) {
		string dbms = "MyTblBrlyProxRMLocationMLocation::insertRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void MyTblBrlyProxRMLocationMLocation::updateRec(
	...
	if (mysql_stmt_bind_param(stmtUpdateRec, bind)) {
		string dbms = "MyTblBrlyProxRMLocationMLocation::updateRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtUpdateRec)) {
		string dbms = "MyTblBrlyProxRMLocationMLocation::updateRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void MyTblBrlyProxRMLocationMLocation::removeRecByRef(
	...
	if (mysql_stmt_bind_param(stmtRemoveRecByRef, &bind)) {
		string dbms = "MyTblBrlyProxRMLocationMLocation::removeRecByRef() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtRemoveRecByRef)) {
		string dbms = "MyTblBrlyProxRMLocationMLocation::removeRecByRef() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void PgTblBrlyProxRMLocationMLocation::initStatements() {
	createStatement("TblBrlyProxRMLocationMLocation_insertRec", "INSERT INTO TblBrlyProxRMLocationMLocation (dstRefBrlyMLocation, orgRefBrlyMLocation, deltaphi) VALUES ($1,$2,$3) RETURNING ref", 3);
	createStatement("TblBrlyProxRMLocationMLocation_updateRec", "UPDATE TblBrlyProxRMLocationMLocation SET dstRefBrlyMLocation = $1, orgRefBrlyMLocation = $2, deltaphi = $3 WHERE ref = $4", 4);
	createStatement("TblBrlyProxRMLocationMLocation_removeRecByRef", "DELETE FROM TblBrlyProxRMLocationMLocation WHERE ref = $1", 1);

	createStatement("TblBrlyProxRMLocationMLocation_loadRecByRef", "SELECT ref, dstRefBrlyMLocation, orgRefBrlyMLocation, deltaphi FROM TblBrlyProxRMLocationMLocation WHERE ref = $1", 1);
	createStatement("TblBrlyProxRMLocationMLocation_loadRstByDlo", "SELECT ref, dstRefBrlyMLocation, orgRefBrlyMLocation, deltaphi FROM TblBrlyProxRMLocationMLocation WHERE dstRefBrlyMLocation = $1", 1);
	createStatement("TblBrlyProxRMLocationMLocation_loadRstByOlo", "SELECT ref, dstRefBrlyMLocation, orgRefBrlyMLocation, deltaphi FROM TblBrlyProxRMLocationMLocation WHERE orgRefBrlyMLocation = $1", 1);
};
...
bool PgTblBrlyProxRMLocationMLocation::loadRecByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyProxRMLocationMLocation::loadRecByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
ubigint PgTblBrlyProxRMLocationMLocation::loadRstByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyProxRMLocationMLocation::loadRstByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
bool PgTblBrlyProxRMLocationMLocation::loadRecBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyProxRMLocationMLocation::loadRecBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyProxRMLocationMLocation::loadRstBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyProxRMLocationMLocation::loadRstBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyProxRMLocationMLocation::insertRec(
			BrlyProxRMLocationMLocation* rec
		) {
	PGresult* res;
	char* ptr;
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyProxRMLocationMLocation::insertRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyProxRMLocationMLocation::updateRec(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyProxRMLocationMLocation::updateRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyProxRMLocationMLocation::removeRecByRef(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyProxRMLocationMLocation::removeRecByRef() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
