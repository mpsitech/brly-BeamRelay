...
ubigint TblBrlyQConRef1NSegment::insertRec(
			BrlyQConRef1NSegment* rec
		) {
	return 0;
};
...
bool MyTblBrlyQConRef1NSegment::loadRecBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyQConRef1NSegment::loadRecBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyQConRef1NSegment::loadRecBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyQConRef1NSegment::loadRstBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyQConRef1NSegment::loadRstBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyQConRef1NSegment::loadRstBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyQConRef1NSegment::insertRec(
			BrlyQConRef1NSegment* rec
		) {
	...
	if (mysql_stmt_bind_param(stmtInsertRec, bind)) {
		string dbms = "MyTblBrlyQConRef1NSegment::insertRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtInsertRec)) {
		string dbms = "MyTblBrlyQConRef1NSegment::insertRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void MyTblBrlyQConRef1NSegment::updateRec(
	...
	if (mysql_stmt_bind_param(stmtUpdateRec, bind)) {
		string dbms = "MyTblBrlyQConRef1NSegment::updateRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtUpdateRec)) {
		string dbms = "MyTblBrlyQConRef1NSegment::updateRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void MyTblBrlyQConRef1NSegment::removeRecByQref(
	...
	if (mysql_stmt_bind_param(stmtRemoveRecByQref, &bind)) {
		string dbms = "MyTblBrlyQConRef1NSegment::removeRecByQref() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtRemoveRecByQref)) {
		string dbms = "MyTblBrlyQConRef1NSegment::removeRecByQref() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void MyTblBrlyQConRef1NSegment::removeRstByJref(
	...
	if (mysql_stmt_bind_param(stmtRemoveRstByJref, &bind)) {
		string dbms = "MyTblBrlyQConRef1NSegment::removeRstByJref() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtRemoveRstByJref)) {
		string dbms = "MyTblBrlyQConRef1NSegment::removeRstByJref() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void PgTblBrlyQConRef1NSegment::initStatements() {
	createStatement("TblBrlyQConRef1NSegment_insertRec", "INSERT INTO TblBrlyQConRef1NSegment (jref, jnum, ref, refNum) VALUES ($1,$2,$3,$4) RETURNING qref", 4);
	createStatement("TblBrlyQConRef1NSegment_updateRec", "UPDATE TblBrlyQConRef1NSegment SET jref = $1, jnum = $2, ref = $3, refNum = $4 WHERE qref = $5", 5);
	createStatement("TblBrlyQConRef1NSegment_removeRecByQref", "DELETE FROM TblBrlyQConRef1NSegment WHERE qref = $1", 1);
	createStatement("TblBrlyQConRef1NSegment_removeRstByJref", "DELETE FROM TblBrlyQConRef1NSegment WHERE jref = $1", 1);

	createStatement("TblBrlyQConRef1NSegment_loadRecByQref", "SELECT qref, jref, jnum, ref, refNum FROM TblBrlyQConRef1NSegment WHERE qref = $1", 1);
	createStatement("TblBrlyQConRef1NSegment_loadRstByJref", "SELECT qref, jref, jnum, ref, refNum FROM TblBrlyQConRef1NSegment WHERE jref = $1 ORDER BY jnum ASC", 1);
};
...
bool PgTblBrlyQConRef1NSegment::loadRecByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK)	{
		string dbms = "PgTblBrlyQConRef1NSegment::loadRecByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
ubigint PgTblBrlyQConRef1NSegment::loadRstByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK)	{
		string dbms = "PgTblBrlyQConRef1NSegment::loadRstByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
bool PgTblBrlyQConRef1NSegment::loadRecBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyQConRef1NSegment::loadRecBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyQConRef1NSegment::loadRstBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyQConRef1NSegment::loadRstBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyQConRef1NSegment::insertRec(
			BrlyQConRef1NSegment* rec
		) {
	PGresult* res;
	char* ptr;
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK)	{
		string dbms = "PgTblBrlyQConRef1NSegment::insertRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyQConRef1NSegment::updateRec(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyQConRef1NSegment::updateRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyQConRef1NSegment::removeRecByQref(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyQConRef1NSegment::removeRecByQref() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyQConRef1NSegment::removeRstByJref(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyQConRef1NSegment::removeRstByJref() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
