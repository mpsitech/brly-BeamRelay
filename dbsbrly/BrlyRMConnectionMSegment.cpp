...
ubigint TblBrlyRMConnectionMSegment::insertRec(
			BrlyRMConnectionMSegment* rec
		) {
	return 0;
};
...
bool MyTblBrlyRMConnectionMSegment::loadRecBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyRMConnectionMSegment::loadRecBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyRMConnectionMSegment::loadRecBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyRMConnectionMSegment::loadRstBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyRMConnectionMSegment::loadRstBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyRMConnectionMSegment::loadRstBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyRMConnectionMSegment::insertRec(
			BrlyRMConnectionMSegment* rec
		) {
	...
	if (mysql_stmt_bind_param(stmtInsertRec, bind)) {
		string dbms = "MyTblBrlyRMConnectionMSegment::insertRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtInsertRec)) {
		string dbms = "MyTblBrlyRMConnectionMSegment::insertRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void MyTblBrlyRMConnectionMSegment::updateRec(
	...
	if (mysql_stmt_bind_param(stmtUpdateRec, bind)) {
		string dbms = "MyTblBrlyRMConnectionMSegment::updateRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtUpdateRec)) {
		string dbms = "MyTblBrlyRMConnectionMSegment::updateRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void MyTblBrlyRMConnectionMSegment::removeRecByRef(
	...
	if (mysql_stmt_bind_param(stmtRemoveRecByRef, &bind)) {
		string dbms = "MyTblBrlyRMConnectionMSegment::removeRecByRef() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtRemoveRecByRef)) {
		string dbms = "MyTblBrlyRMConnectionMSegment::removeRecByRef() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void PgTblBrlyRMConnectionMSegment::initStatements() {
	createStatement("TblBrlyRMConnectionMSegment_insertRec", "INSERT INTO TblBrlyRMConnectionMSegment (x1Start, x1Stop, x2Legphi0, x2Legphi1, refBrlyMConnection, refBrlyMSegment) VALUES ($1,$2,$3,$4,$5,$6) RETURNING ref", 6);
	createStatement("TblBrlyRMConnectionMSegment_updateRec", "UPDATE TblBrlyRMConnectionMSegment SET x1Start = $1, x1Stop = $2, x2Legphi0 = $3, x2Legphi1 = $4, refBrlyMConnection = $5, refBrlyMSegment = $6 WHERE ref = $7", 7);
	createStatement("TblBrlyRMConnectionMSegment_removeRecByRef", "DELETE FROM TblBrlyRMConnectionMSegment WHERE ref = $1", 1);

	createStatement("TblBrlyRMConnectionMSegment_loadRecByRef", "SELECT ref, x1Start, x1Stop, x2Legphi0, x2Legphi1, refBrlyMConnection, refBrlyMSegment FROM TblBrlyRMConnectionMSegment WHERE ref = $1", 1);
	createStatement("TblBrlyRMConnectionMSegment_loadRstByCon", "SELECT ref, x1Start, x1Stop, x2Legphi0, x2Legphi1, refBrlyMConnection, refBrlyMSegment FROM TblBrlyRMConnectionMSegment WHERE refBrlyMConnection = $1 ORDER BY x1Start ASC", 1);
	createStatement("TblBrlyRMConnectionMSegment_loadRstBySeg", "SELECT ref, x1Start, x1Stop, x2Legphi0, x2Legphi1, refBrlyMConnection, refBrlyMSegment FROM TblBrlyRMConnectionMSegment WHERE refBrlyMSegment = $1 ORDER BY x1Start ASC", 1);
};
...
bool PgTblBrlyRMConnectionMSegment::loadRecByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMConnectionMSegment::loadRecByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
ubigint PgTblBrlyRMConnectionMSegment::loadRstByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMConnectionMSegment::loadRstByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
bool PgTblBrlyRMConnectionMSegment::loadRecBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMConnectionMSegment::loadRecBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyRMConnectionMSegment::loadRstBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMConnectionMSegment::loadRstBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyRMConnectionMSegment::insertRec(
			BrlyRMConnectionMSegment* rec
		) {
	PGresult* res;
	char* ptr;
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMConnectionMSegment::insertRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyRMConnectionMSegment::updateRec(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyRMConnectionMSegment::updateRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyRMConnectionMSegment::removeRecByRef(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyRMConnectionMSegment::removeRecByRef() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
