...
ubigint TblBrlyVisRMLocationMLocation::insertRec(
			BrlyVisRMLocationMLocation* rec
		) {
	return 0;
};
...
bool MyTblBrlyVisRMLocationMLocation::loadRecBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyVisRMLocationMLocation::loadRecBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyVisRMLocationMLocation::loadRecBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyVisRMLocationMLocation::loadRstBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyVisRMLocationMLocation::loadRstBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyVisRMLocationMLocation::loadRstBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyVisRMLocationMLocation::insertRec(
			BrlyVisRMLocationMLocation* rec
		) {
	...
	if (mysql_stmt_bind_param(stmtInsertRec, bind)) {
		string dbms = "MyTblBrlyVisRMLocationMLocation::insertRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtInsertRec)) {
		string dbms = "MyTblBrlyVisRMLocationMLocation::insertRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void MyTblBrlyVisRMLocationMLocation::updateRec(
	...
	if (mysql_stmt_bind_param(stmtUpdateRec, bind)) {
		string dbms = "MyTblBrlyVisRMLocationMLocation::updateRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtUpdateRec)) {
		string dbms = "MyTblBrlyVisRMLocationMLocation::updateRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void MyTblBrlyVisRMLocationMLocation::removeRecByRef(
	...
	if (mysql_stmt_bind_param(stmtRemoveRecByRef, &bind)) {
		string dbms = "MyTblBrlyVisRMLocationMLocation::removeRecByRef() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtRemoveRecByRef)) {
		string dbms = "MyTblBrlyVisRMLocationMLocation::removeRecByRef() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void PgTblBrlyVisRMLocationMLocation::initStatements() {
	createStatement("TblBrlyVisRMLocationMLocation_insertRec", "INSERT INTO TblBrlyVisRMLocationMLocation (dstRefBrlyMLocation, orgRefBrlyMLocation) VALUES ($1,$2) RETURNING ref", 2);
	createStatement("TblBrlyVisRMLocationMLocation_updateRec", "UPDATE TblBrlyVisRMLocationMLocation SET dstRefBrlyMLocation = $1, orgRefBrlyMLocation = $2 WHERE ref = $3", 3);
	createStatement("TblBrlyVisRMLocationMLocation_removeRecByRef", "DELETE FROM TblBrlyVisRMLocationMLocation WHERE ref = $1", 1);

	createStatement("TblBrlyVisRMLocationMLocation_loadRecByRef", "SELECT ref, dstRefBrlyMLocation, orgRefBrlyMLocation FROM TblBrlyVisRMLocationMLocation WHERE ref = $1", 1);
	createStatement("TblBrlyVisRMLocationMLocation_loadDlosByOlo", "SELECT dstRefBrlyMLocation FROM TblBrlyVisRMLocationMLocation WHERE orgRefBrlyMLocation = $1", 1);
	createStatement("TblBrlyVisRMLocationMLocation_loadOlosByDlo", "SELECT orgRefBrlyMLocation FROM TblBrlyVisRMLocationMLocation WHERE dstRefBrlyMLocation = $1", 1);
};
...
bool PgTblBrlyVisRMLocationMLocation::loadRecByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyVisRMLocationMLocation::loadRecByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
bool PgTblBrlyVisRMLocationMLocation::loadRecBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyVisRMLocationMLocation::loadRecBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyVisRMLocationMLocation::loadRstBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyVisRMLocationMLocation::loadRstBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyVisRMLocationMLocation::insertRec(
			BrlyVisRMLocationMLocation* rec
		) {
	PGresult* res;
	char* ptr;
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyVisRMLocationMLocation::insertRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyVisRMLocationMLocation::updateRec(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyVisRMLocationMLocation::updateRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyVisRMLocationMLocation::removeRecByRef(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyVisRMLocationMLocation::removeRecByRef() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
