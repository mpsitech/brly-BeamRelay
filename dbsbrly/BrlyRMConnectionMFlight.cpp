...
ubigint TblBrlyRMConnectionMFlight::insertRec(
			BrlyRMConnectionMFlight* rec
		) {
	return 0;
};
...
bool MyTblBrlyRMConnectionMFlight::loadRecBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyRMConnectionMFlight::loadRecBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyRMConnectionMFlight::loadRecBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyRMConnectionMFlight::loadRstBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyRMConnectionMFlight::loadRstBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyRMConnectionMFlight::loadRstBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyRMConnectionMFlight::insertRec(
			BrlyRMConnectionMFlight* rec
		) {
	...
	if (mysql_stmt_bind_param(stmtInsertRec, bind)) {
		string dbms = "MyTblBrlyRMConnectionMFlight::insertRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtInsertRec)) {
		string dbms = "MyTblBrlyRMConnectionMFlight::insertRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void MyTblBrlyRMConnectionMFlight::updateRec(
	...
	if (mysql_stmt_bind_param(stmtUpdateRec, bind)) {
		string dbms = "MyTblBrlyRMConnectionMFlight::updateRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtUpdateRec)) {
		string dbms = "MyTblBrlyRMConnectionMFlight::updateRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void MyTblBrlyRMConnectionMFlight::removeRecByRef(
	...
	if (mysql_stmt_bind_param(stmtRemoveRecByRef, &bind)) {
		string dbms = "MyTblBrlyRMConnectionMFlight::removeRecByRef() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtRemoveRecByRef)) {
		string dbms = "MyTblBrlyRMConnectionMFlight::removeRecByRef() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void PgTblBrlyRMConnectionMFlight::initStatements() {
	createStatement("TblBrlyRMConnectionMFlight_insertRec", "INSERT INTO TblBrlyRMConnectionMFlight (x1Start, x1Stop, x2Legphi0, x2Legphi1, refBrlyMConnection, refBrlyMFlight) VALUES ($1,$2,$3,$4,$5,$6) RETURNING ref", 6);
	createStatement("TblBrlyRMConnectionMFlight_updateRec", "UPDATE TblBrlyRMConnectionMFlight SET x1Start = $1, x1Stop = $2, x2Legphi0 = $3, x2Legphi1 = $4, refBrlyMConnection = $5, refBrlyMFlight = $6 WHERE ref = $7", 7);
	createStatement("TblBrlyRMConnectionMFlight_removeRecByRef", "DELETE FROM TblBrlyRMConnectionMFlight WHERE ref = $1", 1);

	createStatement("TblBrlyRMConnectionMFlight_loadRecByRef", "SELECT ref, x1Start, x1Stop, x2Legphi0, x2Legphi1, refBrlyMConnection, refBrlyMFlight FROM TblBrlyRMConnectionMFlight WHERE ref = $1", 1);
	createStatement("TblBrlyRMConnectionMFlight_loadRstByCon", "SELECT ref, x1Start, x1Stop, x2Legphi0, x2Legphi1, refBrlyMConnection, refBrlyMFlight FROM TblBrlyRMConnectionMFlight WHERE refBrlyMConnection = $1 ORDER BY x1Start ASC", 1);
	createStatement("TblBrlyRMConnectionMFlight_loadRstByFlt", "SELECT ref, x1Start, x1Stop, x2Legphi0, x2Legphi1, refBrlyMConnection, refBrlyMFlight FROM TblBrlyRMConnectionMFlight WHERE refBrlyMFlight = $1 ORDER BY x1Start ASC", 1);
};
...
bool PgTblBrlyRMConnectionMFlight::loadRecByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMConnectionMFlight::loadRecByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
ubigint PgTblBrlyRMConnectionMFlight::loadRstByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMConnectionMFlight::loadRstByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
bool PgTblBrlyRMConnectionMFlight::loadRecBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMConnectionMFlight::loadRecBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyRMConnectionMFlight::loadRstBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMConnectionMFlight::loadRstBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyRMConnectionMFlight::insertRec(
			BrlyRMConnectionMFlight* rec
		) {
	PGresult* res;
	char* ptr;
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyRMConnectionMFlight::insertRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyRMConnectionMFlight::updateRec(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyRMConnectionMFlight::updateRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyRMConnectionMFlight::removeRecByRef(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyRMConnectionMFlight::removeRecByRef() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
