...
ubigint TblBrlyLRMConnectionMEquipment::insertRec(
			BrlyLRMConnectionMEquipment* rec
		) {
	return 0;
};
...
bool MyTblBrlyLRMConnectionMEquipment::loadRecBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyLRMConnectionMEquipment::loadRecBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyLRMConnectionMEquipment::loadRecBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyLRMConnectionMEquipment::loadRstBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyLRMConnectionMEquipment::loadRstBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyLRMConnectionMEquipment::loadRstBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyLRMConnectionMEquipment::insertRec(
			BrlyLRMConnectionMEquipment* rec
		) {
	...
	if (mysql_stmt_bind_param(stmtInsertRec, bind)) {
		string dbms = "MyTblBrlyLRMConnectionMEquipment::insertRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtInsertRec)) {
		string dbms = "MyTblBrlyLRMConnectionMEquipment::insertRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void MyTblBrlyLRMConnectionMEquipment::updateRec(
	...
	if (mysql_stmt_bind_param(stmtUpdateRec, bind)) {
		string dbms = "MyTblBrlyLRMConnectionMEquipment::updateRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtUpdateRec)) {
		string dbms = "MyTblBrlyLRMConnectionMEquipment::updateRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void MyTblBrlyLRMConnectionMEquipment::removeRecByRef(
	...
	if (mysql_stmt_bind_param(stmtRemoveRecByRef, &bind)) {
		string dbms = "MyTblBrlyLRMConnectionMEquipment::removeRecByRef() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtRemoveRecByRef)) {
		string dbms = "MyTblBrlyLRMConnectionMEquipment::removeRecByRef() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void PgTblBrlyLRMConnectionMEquipment::initStatements() {
	createStatement("TblBrlyLRMConnectionMEquipment_insertRec", "INSERT INTO TblBrlyLRMConnectionMEquipment (conIxBrlyVLat, conRefBrlyMConnection, conNum, refBrlyMEquipment) VALUES ($1,$2,$3,$4) RETURNING ref", 4);
	createStatement("TblBrlyLRMConnectionMEquipment_updateRec", "UPDATE TblBrlyLRMConnectionMEquipment SET conIxBrlyVLat = $1, conRefBrlyMConnection = $2, conNum = $3, refBrlyMEquipment = $4 WHERE ref = $5", 5);
	createStatement("TblBrlyLRMConnectionMEquipment_removeRecByRef", "DELETE FROM TblBrlyLRMConnectionMEquipment WHERE ref = $1", 1);

	createStatement("TblBrlyLRMConnectionMEquipment_loadRecByRef", "SELECT ref, conIxBrlyVLat, conRefBrlyMConnection, conNum, refBrlyMEquipment FROM TblBrlyLRMConnectionMEquipment WHERE ref = $1", 1);
	createStatement("TblBrlyLRMConnectionMEquipment_loadConsByEqp", "SELECT conRefBrlyMConnection FROM TblBrlyLRMConnectionMEquipment WHERE refBrlyMEquipment = $1", 1);
	createStatement("TblBrlyLRMConnectionMEquipment_loadEqpsByCon", "SELECT refBrlyMEquipment FROM TblBrlyLRMConnectionMEquipment WHERE conRefBrlyMConnection = $1 ORDER BY conNum ASC", 1);
	createStatement("TblBrlyLRMConnectionMEquipment_loadRefsByConLat", "SELECT ref FROM TblBrlyLRMConnectionMEquipment WHERE conRefBrlyMConnection = $1 AND conIxBrlyVLat = $2 ORDER BY conNum ASC", 2);
	createStatement("TblBrlyLRMConnectionMEquipment_loadRstByCon", "SELECT ref, conIxBrlyVLat, conRefBrlyMConnection, conNum, refBrlyMEquipment FROM TblBrlyLRMConnectionMEquipment WHERE conRefBrlyMConnection = $1 ORDER BY conNum ASC", 1);
};
...
bool PgTblBrlyLRMConnectionMEquipment::loadRecByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyLRMConnectionMEquipment::loadRecByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
ubigint PgTblBrlyLRMConnectionMEquipment::loadRstByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyLRMConnectionMEquipment::loadRstByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
bool PgTblBrlyLRMConnectionMEquipment::loadRecBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyLRMConnectionMEquipment::loadRecBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyLRMConnectionMEquipment::loadRstBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyLRMConnectionMEquipment::loadRstBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyLRMConnectionMEquipment::insertRec(
			BrlyLRMConnectionMEquipment* rec
		) {
	PGresult* res;
	char* ptr;
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyLRMConnectionMEquipment::insertRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyLRMConnectionMEquipment::updateRec(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyLRMConnectionMEquipment::updateRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyLRMConnectionMEquipment::removeRecByRef(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyLRMConnectionMEquipment::removeRecByRef() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
