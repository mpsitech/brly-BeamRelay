...
ubigint TblBrlyJAVKeylistKey::insertRec(
			BrlyJAVKeylistKey* rec
		) {
	return 0;
};
...
bool MyTblBrlyJAVKeylistKey::loadRecBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyJAVKeylistKey::loadRecBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyJAVKeylistKey::loadRecBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyJAVKeylistKey::loadRstBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyJAVKeylistKey::loadRstBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyJAVKeylistKey::loadRstBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyJAVKeylistKey::insertRec(
			BrlyJAVKeylistKey* rec
		) {
	...
	if (mysql_stmt_bind_param(stmtInsertRec, bind)) {
		string dbms = "MyTblBrlyJAVKeylistKey::insertRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtInsertRec)) {
		string dbms = "MyTblBrlyJAVKeylistKey::insertRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void MyTblBrlyJAVKeylistKey::updateRec(
	...
	if (mysql_stmt_bind_param(stmtUpdateRec, bind)) {
		string dbms = "MyTblBrlyJAVKeylistKey::updateRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtUpdateRec)) {
		string dbms = "MyTblBrlyJAVKeylistKey::updateRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void MyTblBrlyJAVKeylistKey::removeRecByRef(
	...
	if (mysql_stmt_bind_param(stmtRemoveRecByRef, &bind)) {
		string dbms = "MyTblBrlyJAVKeylistKey::removeRecByRef() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtRemoveRecByRef)) {
		string dbms = "MyTblBrlyJAVKeylistKey::removeRecByRef() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void PgTblBrlyJAVKeylistKey::initStatements() {
	createStatement("TblBrlyJAVKeylistKey_insertRec", "INSERT INTO TblBrlyJAVKeylistKey (refBrlyAVKeylistKey, x1IxBrlyVLocale, Title, Comment) VALUES ($1,$2,$3,$4) RETURNING ref", 4);
	createStatement("TblBrlyJAVKeylistKey_updateRec", "UPDATE TblBrlyJAVKeylistKey SET refBrlyAVKeylistKey = $1, x1IxBrlyVLocale = $2, Title = $3, Comment = $4 WHERE ref = $5", 5);
	createStatement("TblBrlyJAVKeylistKey_removeRecByRef", "DELETE FROM TblBrlyJAVKeylistKey WHERE ref = $1", 1);

	createStatement("TblBrlyJAVKeylistKey_loadRecByRef", "SELECT ref, refBrlyAVKeylistKey, x1IxBrlyVLocale, Title, Comment FROM TblBrlyJAVKeylistKey WHERE ref = $1", 1);
	createStatement("TblBrlyJAVKeylistKey_loadRecByKlkLcl", "SELECT ref, refBrlyAVKeylistKey, x1IxBrlyVLocale, Title, Comment FROM TblBrlyJAVKeylistKey WHERE refBrlyAVKeylistKey = $1 AND x1IxBrlyVLocale = $2", 2);
	createStatement("TblBrlyJAVKeylistKey_loadRefsByKlk", "SELECT ref FROM TblBrlyJAVKeylistKey WHERE refBrlyAVKeylistKey = $1", 1);
	createStatement("TblBrlyJAVKeylistKey_loadRstByKlk", "SELECT ref, refBrlyAVKeylistKey, x1IxBrlyVLocale, Title, Comment FROM TblBrlyJAVKeylistKey WHERE refBrlyAVKeylistKey = $1 ORDER BY x1IxBrlyVLocale ASC", 1);
	createStatement("TblBrlyJAVKeylistKey_loadTitByKlkLcl", "SELECT Title FROM TblBrlyJAVKeylistKey WHERE refBrlyAVKeylistKey = $1 AND x1IxBrlyVLocale = $2", 2);
};
...
bool PgTblBrlyJAVKeylistKey::loadRecByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyJAVKeylistKey::loadRecByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
ubigint PgTblBrlyJAVKeylistKey::loadRstByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyJAVKeylistKey::loadRstByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
bool PgTblBrlyJAVKeylistKey::loadRecBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyJAVKeylistKey::loadRecBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyJAVKeylistKey::loadRstBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyJAVKeylistKey::loadRstBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyJAVKeylistKey::insertRec(
			BrlyJAVKeylistKey* rec
		) {
	PGresult* res;
	char* ptr;
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyJAVKeylistKey::insertRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyJAVKeylistKey::updateRec(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyJAVKeylistKey::updateRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyJAVKeylistKey::removeRecByRef(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyJAVKeylistKey::removeRecByRef() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
