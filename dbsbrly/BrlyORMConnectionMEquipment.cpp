...
ubigint TblBrlyORMConnectionMEquipment::insertRec(
			BrlyORMConnectionMEquipment* rec
		) {
	return 0;
};
...
bool MyTblBrlyORMConnectionMEquipment::loadRecBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyORMConnectionMEquipment::loadRecBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyORMConnectionMEquipment::loadRecBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyORMConnectionMEquipment::loadRstBySQL(
	...
	if (mysql_real_query(dbs, sqlstr.c_str(), sqlstr.length())) {
		string dbms = "MyTblBrlyORMConnectionMEquipment::loadRstBySQL() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};

	dbresult = mysql_store_result(dbs);
	if (!dbresult) {
		string dbms = "MyTblBrlyORMConnectionMEquipment::loadRstBySQL() / store result / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint MyTblBrlyORMConnectionMEquipment::insertRec(
			BrlyORMConnectionMEquipment* rec
		) {
	...
	if (mysql_stmt_bind_param(stmtInsertRec, bind)) {
		string dbms = "MyTblBrlyORMConnectionMEquipment::insertRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtInsertRec)) {
		string dbms = "MyTblBrlyORMConnectionMEquipment::insertRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void MyTblBrlyORMConnectionMEquipment::updateRec(
	...
	if (mysql_stmt_bind_param(stmtUpdateRec, bind)) {
		string dbms = "MyTblBrlyORMConnectionMEquipment::updateRec() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtUpdateRec)) {
		string dbms = "MyTblBrlyORMConnectionMEquipment::updateRec() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void MyTblBrlyORMConnectionMEquipment::removeRecByRef(
	...
	if (mysql_stmt_bind_param(stmtRemoveRecByRef, &bind)) {
		string dbms = "MyTblBrlyORMConnectionMEquipment::removeRecByRef() / bind / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
	if (mysql_stmt_execute(stmtRemoveRecByRef)) {
		string dbms = "MyTblBrlyORMConnectionMEquipment::removeRecByRef() / " + string(mysql_error(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
};
...
void PgTblBrlyORMConnectionMEquipment::initStatements() {
	createStatement("TblBrlyORMConnectionMEquipment_insertRec", "INSERT INTO TblBrlyORMConnectionMEquipment (refBrlyMConnection, x1Start, ixBrlyVOolop, pr1RefLR, pr2RefLR) VALUES ($1,$2,$3,$4,$5) RETURNING ref", 5);
	createStatement("TblBrlyORMConnectionMEquipment_updateRec", "UPDATE TblBrlyORMConnectionMEquipment SET refBrlyMConnection = $1, x1Start = $2, ixBrlyVOolop = $3, pr1RefLR = $4, pr2RefLR = $5 WHERE ref = $6", 6);
	createStatement("TblBrlyORMConnectionMEquipment_removeRecByRef", "DELETE FROM TblBrlyORMConnectionMEquipment WHERE ref = $1", 1);

	createStatement("TblBrlyORMConnectionMEquipment_loadRecByRef", "SELECT ref, refBrlyMConnection, x1Start, ixBrlyVOolop, pr1RefLR, pr2RefLR FROM TblBrlyORMConnectionMEquipment WHERE ref = $1", 1);
	createStatement("TblBrlyORMConnectionMEquipment_loadRecByP1lP2lLop", "SELECT ref, refBrlyMConnection, x1Start, ixBrlyVOolop, pr1RefLR, pr2RefLR FROM TblBrlyORMConnectionMEquipment WHERE pr1RefLR = $1 AND pr2RefLR = $2 AND ixBrlyVOolop = $3", 3);
	createStatement("TblBrlyORMConnectionMEquipment_loadRstByCon", "SELECT ref, refBrlyMConnection, x1Start, ixBrlyVOolop, pr1RefLR, pr2RefLR FROM TblBrlyORMConnectionMEquipment WHERE refBrlyMConnection = $1 ORDER BY x1Start ASC, ixBrlyVOolop ASC, ref ASC", 1);
};
...
bool PgTblBrlyORMConnectionMEquipment::loadRecByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyORMConnectionMEquipment::loadRecByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
ubigint PgTblBrlyORMConnectionMEquipment::loadRstByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyORMConnectionMEquipment::loadRstByStmt(" + srefStmt + ") / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
bool PgTblBrlyORMConnectionMEquipment::loadRecBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyORMConnectionMEquipment::loadRecBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyORMConnectionMEquipment::loadRstBySQL(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyORMConnectionMEquipment::loadRstBySQL() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_QUERY, {{"dbms",dbms}, {"sql",sqlstr}});
	};
...
ubigint PgTblBrlyORMConnectionMEquipment::insertRec(
			BrlyORMConnectionMEquipment* rec
		) {
	PGresult* res;
	char* ptr;
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) {
		string dbms = "PgTblBrlyORMConnectionMEquipment::insertRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyORMConnectionMEquipment::updateRec(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyORMConnectionMEquipment::updateRec() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
...
void PgTblBrlyORMConnectionMEquipment::removeRecByRef(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		string dbms = "PgTblBrlyORMConnectionMEquipment::removeRecByRef() / " + string(PQerrorMessage(dbs));
		throw SbeException(SbeException::DBS_STMTEXEC, {{"dbms",dbms}});
	};
