...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyRlyList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyRlyListData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyRlyList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyRlyList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyRlyListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyRlyListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacBrlyRlyList) this.contiac = dpcheng.ContIacBrlyRlyList;
				if (dpcheng.ContInfBrlyRlyList) this.continf = dpcheng.ContInfBrlyRlyList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrBrlyRlyList) this.statshr = dpcheng.StatShrBrlyRlyList;
				if (dpcheng.StgIacBrlyRlyList) this.stgiac = dpcheng.StgIacBrlyRlyList;
				if (dpcheng.TagBrlyRlyList) {
					Brly.unescapeBlock(dpcheng.TagBrlyRlyList);
					this.tag = dpcheng.TagBrlyRlyList;
				}
				if (dpcheng.ListBrlyQRlyList) this.rst = dpcheng.ListBrlyQRlyList;
				if (dpcheng.StatAppQryBrlyRlyList) this.statappqry = dpcheng.StatAppQryBrlyRlyList;
				if (dpcheng.StatShrQryBrlyRlyList) this.statshrqry = dpcheng.StatShrQryBrlyRlyList;
				if (dpcheng.StgIacQryBrlyRlyList) this.stgiacqry = dpcheng.StgIacQryBrlyRlyList;
				if (dpcheng.ContInfBrlyRlyList) {
				...
				*/
				...
				if (dpcheng.TagBrlyRlyList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSta", text: this.tag.TcoSta}, {value: "TcoSto", text: this.tag.TcoSto}, {value: "TcoCon", text: this.tag.TcoCon}, {value: "TcoDir", text: this.tag.TcoDir}, {value: "TcoCtd", text: this.tag.TcoCtd});
					/*
					*/
				...
				if (dpcheng.ListBrlyQRlyList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSta: rec["sta"], TcoSto: rec["sto"], TcoCon: rec["con"], TcoDir: rec["dir2"], TcoCtd: rec["ctd"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyRlyListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyRlyListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
