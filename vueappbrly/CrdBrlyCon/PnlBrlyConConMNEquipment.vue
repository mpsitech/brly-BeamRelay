...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyConConMNEquipment.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleButClick: function(ctlsref) {
				const dpchapp = {
					"DpchAppBrlyConConMNEquipmentDo": {
						"scrJref": this.scrJref,
						"srefIxVDo": ctlsref
					}
				};
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyConConMNEquipmentData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyConConMNEquipment": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyConConMNEquipment"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyConConMNEquipmentData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyConConMNEquipmentData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyConConMNEquipment) this.continf = dpcheng.ContInfBrlyConConMNEquipment;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyConConMNEquipment) this.statapp = dpcheng.StatAppBrlyConConMNEquipment;
				if (dpcheng.StatShrBrlyConConMNEquipment) this.statshr = dpcheng.StatShrBrlyConConMNEquipment;
				if (dpcheng.StgIacBrlyConConMNEquipment) this.stgiac = dpcheng.StgIacBrlyConConMNEquipment;
				if (dpcheng.TagBrlyConConMNEquipment) {
					Brly.unescapeBlock(dpcheng.TagBrlyConConMNEquipment);
					this.tag = dpcheng.TagBrlyConConMNEquipment;
				}
				if (dpcheng.ListBrlyQConConMNEquipment) this.rst = dpcheng.ListBrlyQConConMNEquipment;
				if (dpcheng.StatAppQryBrlyConConMNEquipment) this.statappqry = dpcheng.StatAppQryBrlyConConMNEquipment;
				if (dpcheng.StatShrQryBrlyConConMNEquipment) this.statshrqry = dpcheng.StatShrQryBrlyConConMNEquipment;
				if (dpcheng.StgIacQryBrlyConConMNEquipment) this.stgiacqry = dpcheng.StgIacQryBrlyConConMNEquipment;
				/*
				...
				*/
				...
				if (dpcheng.ListBrlyQConConMNEquipment) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyConConMNEquipmentData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyConConMNEquipmentData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
