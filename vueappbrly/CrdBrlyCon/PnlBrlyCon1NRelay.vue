...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyCon1NRelay.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyCon1NRelayData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyCon1NRelay": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyCon1NRelay"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyCon1NRelayData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyCon1NRelayData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyCon1NRelay) this.continf = dpcheng.ContInfBrlyCon1NRelay;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyCon1NRelay) this.statapp = dpcheng.StatAppBrlyCon1NRelay;
				if (dpcheng.StatShrBrlyCon1NRelay) this.statshr = dpcheng.StatShrBrlyCon1NRelay;
				if (dpcheng.StgIacBrlyCon1NRelay) this.stgiac = dpcheng.StgIacBrlyCon1NRelay;
				if (dpcheng.TagBrlyCon1NRelay) {
					Brly.unescapeBlock(dpcheng.TagBrlyCon1NRelay);
					this.tag = dpcheng.TagBrlyCon1NRelay;
				}
				if (dpcheng.ListBrlyQCon1NRelay) this.rst = dpcheng.ListBrlyQCon1NRelay;
				if (dpcheng.StatAppQryBrlyCon1NRelay) this.statappqry = dpcheng.StatAppQryBrlyCon1NRelay;
				if (dpcheng.StatShrQryBrlyCon1NRelay) this.statshrqry = dpcheng.StatShrQryBrlyCon1NRelay;
				if (dpcheng.StgIacQryBrlyCon1NRelay) this.stgiacqry = dpcheng.StgIacQryBrlyCon1NRelay;
				/*
				...
				*/
				...
				if (dpcheng.ListBrlyQCon1NRelay) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyCon1NRelayData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyCon1NRelayData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
