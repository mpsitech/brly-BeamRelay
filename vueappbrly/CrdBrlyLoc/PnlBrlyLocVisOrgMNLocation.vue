...
<script>
	...
	*/
	...
	export default {
		name: 'PnlBrlyLocVisOrgMNLocation',
		...
		mounted() {
			//console.log("PnlBrlyLocVisOrgMNLocation.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleButClick: function(ctlsref) {
				const dpchapp = {
					"DpchAppBrlyLocVisOrgMNLocationDo": {
						"scrJref": this.scrJref,
						"srefIxVDo": ctlsref
					}
				};
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyLocVisOrgMNLocationData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyLocVisOrgMNLocation": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyLocVisOrgMNLocation"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyLocVisOrgMNLocationData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyLocVisOrgMNLocationData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyLocVisOrgMNLocation) this.continf = dpcheng.ContInfBrlyLocVisOrgMNLocation;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyLocVisOrgMNLocation) this.statapp = dpcheng.StatAppBrlyLocVisOrgMNLocation;
				if (dpcheng.StatShrBrlyLocVisOrgMNLocation) this.statshr = dpcheng.StatShrBrlyLocVisOrgMNLocation;
				if (dpcheng.StgIacBrlyLocVisOrgMNLocation) this.stgiac = dpcheng.StgIacBrlyLocVisOrgMNLocation;
				if (dpcheng.TagBrlyLocVisOrgMNLocation) {
					Brly.unescapeBlock(dpcheng.TagBrlyLocVisOrgMNLocation);
					this.tag = dpcheng.TagBrlyLocVisOrgMNLocation;
				}
				if (dpcheng.ListBrlyQLocVisOrgMNLocation) this.rst = dpcheng.ListBrlyQLocVisOrgMNLocation;
				if (dpcheng.StatAppQryBrlyLocVisOrgMNLocation) this.statappqry = dpcheng.StatAppQryBrlyLocVisOrgMNLocation;
				if (dpcheng.StatShrQryBrlyLocVisOrgMNLocation) this.statshrqry = dpcheng.StatShrQryBrlyLocVisOrgMNLocation;
				if (dpcheng.StgIacQryBrlyLocVisOrgMNLocation) this.stgiacqry = dpcheng.StgIacQryBrlyLocVisOrgMNLocation;
				/*
				...
				*/
				...
				if (dpcheng.ListBrlyQLocVisOrgMNLocation) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyLocVisOrgMNLocationData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyLocVisOrgMNLocationData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
