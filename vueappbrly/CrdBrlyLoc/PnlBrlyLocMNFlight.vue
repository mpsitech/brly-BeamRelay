...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyLocMNFlight.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyLocMNFlightData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyLocMNFlight": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyLocMNFlight"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyLocMNFlightData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyLocMNFlightData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyLocMNFlight) this.continf = dpcheng.ContInfBrlyLocMNFlight;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyLocMNFlight) this.statapp = dpcheng.StatAppBrlyLocMNFlight;
				if (dpcheng.StatShrBrlyLocMNFlight) this.statshr = dpcheng.StatShrBrlyLocMNFlight;
				if (dpcheng.StgIacBrlyLocMNFlight) this.stgiac = dpcheng.StgIacBrlyLocMNFlight;
				if (dpcheng.TagBrlyLocMNFlight) {
					Brly.unescapeBlock(dpcheng.TagBrlyLocMNFlight);
					this.tag = dpcheng.TagBrlyLocMNFlight;
				}
				if (dpcheng.ListBrlyQLocMNFlight) this.rst = dpcheng.ListBrlyQLocMNFlight;
				if (dpcheng.StatAppQryBrlyLocMNFlight) this.statappqry = dpcheng.StatAppQryBrlyLocMNFlight;
				if (dpcheng.StatShrQryBrlyLocMNFlight) this.statshrqry = dpcheng.StatShrQryBrlyLocMNFlight;
				if (dpcheng.StgIacQryBrlyLocMNFlight) this.stgiacqry = dpcheng.StgIacQryBrlyLocMNFlight;
				/*
				...
				*/
				...
				if (dpcheng.ListBrlyQLocMNFlight) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyLocMNFlightData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyLocMNFlightData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
