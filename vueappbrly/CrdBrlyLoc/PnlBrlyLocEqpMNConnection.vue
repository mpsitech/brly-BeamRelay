...
<script>
	...
	*/
	...
	export default {
		name: 'PnlBrlyLocEqpMNConnection',
		...
		mounted() {
			//console.log("PnlBrlyLocEqpMNConnection.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleButClick: function(ctlsref) {
				const dpchapp = {
					"DpchAppBrlyLocEqpMNConnectionDo": {
						"scrJref": this.scrJref,
						"srefIxVDo": ctlsref
					}
				};
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyLocEqpMNConnectionData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyLocEqpMNConnection": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyLocEqpMNConnection"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyLocEqpMNConnectionData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyLocEqpMNConnectionData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyLocEqpMNConnection) this.continf = dpcheng.ContInfBrlyLocEqpMNConnection;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyLocEqpMNConnection) this.statapp = dpcheng.StatAppBrlyLocEqpMNConnection;
				if (dpcheng.StatShrBrlyLocEqpMNConnection) this.statshr = dpcheng.StatShrBrlyLocEqpMNConnection;
				if (dpcheng.StgIacBrlyLocEqpMNConnection) this.stgiac = dpcheng.StgIacBrlyLocEqpMNConnection;
				if (dpcheng.TagBrlyLocEqpMNConnection) {
					Brly.unescapeBlock(dpcheng.TagBrlyLocEqpMNConnection);
					this.tag = dpcheng.TagBrlyLocEqpMNConnection;
				}
				if (dpcheng.ListBrlyQLocEqpMNConnection) this.rst = dpcheng.ListBrlyQLocEqpMNConnection;
				if (dpcheng.StatAppQryBrlyLocEqpMNConnection) this.statappqry = dpcheng.StatAppQryBrlyLocEqpMNConnection;
				if (dpcheng.StatShrQryBrlyLocEqpMNConnection) this.statshrqry = dpcheng.StatShrQryBrlyLocEqpMNConnection;
				if (dpcheng.StgIacQryBrlyLocEqpMNConnection) this.stgiacqry = dpcheng.StgIacQryBrlyLocEqpMNConnection;
				/*
				...
				*/
				...
				if (dpcheng.ListBrlyQLocEqpMNConnection) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyLocEqpMNConnectionData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyLocEqpMNConnectionData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
