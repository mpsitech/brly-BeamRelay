...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyLocEqp1NNode.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyLocEqp1NNodeData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyLocEqp1NNode": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyLocEqp1NNode"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyLocEqp1NNodeData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyLocEqp1NNodeData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyLocEqp1NNode) this.continf = dpcheng.ContInfBrlyLocEqp1NNode;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyLocEqp1NNode) this.statapp = dpcheng.StatAppBrlyLocEqp1NNode;
				if (dpcheng.StgIacBrlyLocEqp1NNode) this.stgiac = dpcheng.StgIacBrlyLocEqp1NNode;
				if (dpcheng.TagBrlyLocEqp1NNode) {
					Brly.unescapeBlock(dpcheng.TagBrlyLocEqp1NNode);
					this.tag = dpcheng.TagBrlyLocEqp1NNode;
				}
				if (dpcheng.ListBrlyQLocEqp1NNode) this.rst = dpcheng.ListBrlyQLocEqp1NNode;
				if (dpcheng.StatAppQryBrlyLocEqp1NNode) this.statappqry = dpcheng.StatAppQryBrlyLocEqp1NNode;
				if (dpcheng.StatShrQryBrlyLocEqp1NNode) this.statshrqry = dpcheng.StatShrQryBrlyLocEqp1NNode;
				if (dpcheng.StgIacQryBrlyLocEqp1NNode) this.stgiacqry = dpcheng.StgIacQryBrlyLocEqp1NNode;
				/*
				...
				*/
				...
				if (dpcheng.ListBrlyQLocEqp1NNode) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyLocEqp1NNodeData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyLocEqp1NNodeData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
