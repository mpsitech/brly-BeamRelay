...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyLocMNLeg.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyLocMNLegData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyLocMNLeg": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyLocMNLeg"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyLocMNLegData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyLocMNLegData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyLocMNLeg) this.continf = dpcheng.ContInfBrlyLocMNLeg;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyLocMNLeg) this.statapp = dpcheng.StatAppBrlyLocMNLeg;
				if (dpcheng.StatShrBrlyLocMNLeg) this.statshr = dpcheng.StatShrBrlyLocMNLeg;
				if (dpcheng.StgIacBrlyLocMNLeg) this.stgiac = dpcheng.StgIacBrlyLocMNLeg;
				if (dpcheng.TagBrlyLocMNLeg) {
					Brly.unescapeBlock(dpcheng.TagBrlyLocMNLeg);
					this.tag = dpcheng.TagBrlyLocMNLeg;
				}
				if (dpcheng.ListBrlyQLocMNLeg) this.rst = dpcheng.ListBrlyQLocMNLeg;
				if (dpcheng.StatAppQryBrlyLocMNLeg) this.statappqry = dpcheng.StatAppQryBrlyLocMNLeg;
				if (dpcheng.StatShrQryBrlyLocMNLeg) this.statshrqry = dpcheng.StatShrQryBrlyLocMNLeg;
				if (dpcheng.StgIacQryBrlyLocMNLeg) this.stgiacqry = dpcheng.StgIacQryBrlyLocMNLeg;
				/*
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyLocMNLegData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyLocMNLegData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
