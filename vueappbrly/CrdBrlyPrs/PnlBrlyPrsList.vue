...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyPrsList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyPrsListData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyPrsList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyPrsList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyPrsListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyPrsListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacBrlyPrsList) this.contiac = dpcheng.ContIacBrlyPrsList;
				if (dpcheng.ContInfBrlyPrsList) this.continf = dpcheng.ContInfBrlyPrsList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrBrlyPrsList) this.statshr = dpcheng.StatShrBrlyPrsList;
				if (dpcheng.StgIacBrlyPrsList) this.stgiac = dpcheng.StgIacBrlyPrsList;
				if (dpcheng.TagBrlyPrsList) {
					Brly.unescapeBlock(dpcheng.TagBrlyPrsList);
					this.tag = dpcheng.TagBrlyPrsList;
				}
				if (dpcheng.ListBrlyQPrsList) this.rst = dpcheng.ListBrlyQPrsList;
				if (dpcheng.StatAppQryBrlyPrsList) this.statappqry = dpcheng.StatAppQryBrlyPrsList;
				if (dpcheng.StatShrQryBrlyPrsList) this.statshrqry = dpcheng.StatShrQryBrlyPrsList;
				if (dpcheng.StgIacQryBrlyPrsList) this.stgiacqry = dpcheng.StgIacQryBrlyPrsList;
				if (dpcheng.ContInfBrlyPrsList) {
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyPrsListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyPrsListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
