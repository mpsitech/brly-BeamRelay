...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyPrsADetail.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyPrsADetailData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyPrsADetail": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyPrsADetail"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyPrsADetailData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyPrsADetailData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyPrsADetail) this.continf = dpcheng.ContInfBrlyPrsADetail;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyPrsADetail) this.statapp = dpcheng.StatAppBrlyPrsADetail;
				if (dpcheng.StatShrBrlyPrsADetail) this.statshr = dpcheng.StatShrBrlyPrsADetail;
				if (dpcheng.StgIacBrlyPrsADetail) this.stgiac = dpcheng.StgIacBrlyPrsADetail;
				if (dpcheng.TagBrlyPrsADetail) {
					Brly.unescapeBlock(dpcheng.TagBrlyPrsADetail);
					this.tag = dpcheng.TagBrlyPrsADetail;
				}
				if (dpcheng.ListBrlyQPrsADetail) this.rst = dpcheng.ListBrlyQPrsADetail;
				if (dpcheng.StatAppQryBrlyPrsADetail) this.statappqry = dpcheng.StatAppQryBrlyPrsADetail;
				if (dpcheng.StatShrQryBrlyPrsADetail) this.statshrqry = dpcheng.StatShrQryBrlyPrsADetail;
				if (dpcheng.StgIacQryBrlyPrsADetail) this.stgiacqry = dpcheng.StgIacQryBrlyPrsADetail;
				/*
				...
				*/
				...
				if (dpcheng.ListBrlyQPrsADetail) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyPrsADetailData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyPrsADetailData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
