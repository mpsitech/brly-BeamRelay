...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlySegMNConnection.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlySegMNConnectionData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlySegMNConnection": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlySegMNConnection"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlySegMNConnectionData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlySegMNConnectionData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlySegMNConnection) this.continf = dpcheng.ContInfBrlySegMNConnection;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlySegMNConnection) this.statapp = dpcheng.StatAppBrlySegMNConnection;
				if (dpcheng.StatShrBrlySegMNConnection) this.statshr = dpcheng.StatShrBrlySegMNConnection;
				if (dpcheng.StgIacBrlySegMNConnection) this.stgiac = dpcheng.StgIacBrlySegMNConnection;
				if (dpcheng.TagBrlySegMNConnection) {
					Brly.unescapeBlock(dpcheng.TagBrlySegMNConnection);
					this.tag = dpcheng.TagBrlySegMNConnection;
				}
				if (dpcheng.ListBrlyQSegMNConnection) this.rst = dpcheng.ListBrlyQSegMNConnection;
				if (dpcheng.StatAppQryBrlySegMNConnection) this.statappqry = dpcheng.StatAppQryBrlySegMNConnection;
				if (dpcheng.StatShrQryBrlySegMNConnection) this.statshrqry = dpcheng.StatShrQryBrlySegMNConnection;
				if (dpcheng.StgIacQryBrlySegMNConnection) this.stgiacqry = dpcheng.StgIacQryBrlySegMNConnection;
				/*
				...
				*/
				...
				if (dpcheng.ListBrlyQSegMNConnection) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlySegMNConnectionData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlySegMNConnectionData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
