...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlySegMNLocation.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlySegMNLocationData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlySegMNLocation": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlySegMNLocation"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlySegMNLocationData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlySegMNLocationData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlySegMNLocation) this.continf = dpcheng.ContInfBrlySegMNLocation;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlySegMNLocation) this.statapp = dpcheng.StatAppBrlySegMNLocation;
				if (dpcheng.StatShrBrlySegMNLocation) this.statshr = dpcheng.StatShrBrlySegMNLocation;
				if (dpcheng.StgIacBrlySegMNLocation) this.stgiac = dpcheng.StgIacBrlySegMNLocation;
				if (dpcheng.TagBrlySegMNLocation) {
					Brly.unescapeBlock(dpcheng.TagBrlySegMNLocation);
					this.tag = dpcheng.TagBrlySegMNLocation;
				}
				if (dpcheng.ListBrlyQSegMNLocation) this.rst = dpcheng.ListBrlyQSegMNLocation;
				if (dpcheng.StatAppQryBrlySegMNLocation) this.statappqry = dpcheng.StatAppQryBrlySegMNLocation;
				if (dpcheng.StatShrQryBrlySegMNLocation) this.statshrqry = dpcheng.StatShrQryBrlySegMNLocation;
				if (dpcheng.StgIacQryBrlySegMNLocation) this.stgiacqry = dpcheng.StgIacQryBrlySegMNLocation;
				/*
				...
				*/
				...
				if (dpcheng.TagBrlySegMNLocation) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoMref", text: this.tag.TcoMref}, {value: "TcoSta", text: this.tag.TcoSta}, {value: "TcoSto", text: this.tag.TcoSto}, {value: "TcoSp0", text: this.tag.TcoSp0}, {value: "TcoSp1", text: this.tag.TcoSp1}, {value: "TcoSph", text: this.tag.TcoSph}, {value: "TcoSth", text: this.tag.TcoSth});
					/*
					*/
				...
				if (dpcheng.ListBrlyQSegMNLocation) {
					var rows = [];
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoMref: rec["mref"], TcoSta: rec["sta"], TcoSto: rec["sto"], TcoSp0: rec["sp0"], TcoSp1: rec["sp1"], TcoSph: rec["sph"], TcoSth: rec["sth"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlySegMNLocationData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlySegMNLocationData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
