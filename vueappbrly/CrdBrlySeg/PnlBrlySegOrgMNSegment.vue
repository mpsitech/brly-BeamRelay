...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlySegOrgMNSegment.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlySegOrgMNSegmentData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlySegOrgMNSegment": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlySegOrgMNSegment"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlySegOrgMNSegmentData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlySegOrgMNSegmentData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlySegOrgMNSegment) this.continf = dpcheng.ContInfBrlySegOrgMNSegment;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlySegOrgMNSegment) this.statapp = dpcheng.StatAppBrlySegOrgMNSegment;
				if (dpcheng.StatShrBrlySegOrgMNSegment) this.statshr = dpcheng.StatShrBrlySegOrgMNSegment;
				if (dpcheng.StgIacBrlySegOrgMNSegment) this.stgiac = dpcheng.StgIacBrlySegOrgMNSegment;
				if (dpcheng.TagBrlySegOrgMNSegment) {
					Brly.unescapeBlock(dpcheng.TagBrlySegOrgMNSegment);
					this.tag = dpcheng.TagBrlySegOrgMNSegment;
				}
				if (dpcheng.ListBrlyQSegOrgMNSegment) this.rst = dpcheng.ListBrlyQSegOrgMNSegment;
				if (dpcheng.StatAppQryBrlySegOrgMNSegment) this.statappqry = dpcheng.StatAppQryBrlySegOrgMNSegment;
				if (dpcheng.StatShrQryBrlySegOrgMNSegment) this.statshrqry = dpcheng.StatShrQryBrlySegOrgMNSegment;
				if (dpcheng.StgIacQryBrlySegOrgMNSegment) this.stgiacqry = dpcheng.StgIacQryBrlySegOrgMNSegment;
				/*
				...
				*/
				...
				if (dpcheng.TagBrlySegOrgMNSegment) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoMref", text: this.tag.TcoMref}, {value: "TcoSta", text: this.tag.TcoSta}, {value: "TcoSto", text: this.tag.TcoSto}, {value: "TcoDp0", text: this.tag.TcoDp0}, {value: "TcoDp1", text: this.tag.TcoDp1}, {value: "TcoOp0", text: this.tag.TcoOp0}, {value: "TcoOp1", text: this.tag.TcoOp1});
					/*
					*/
				...
				if (dpcheng.ListBrlyQSegOrgMNSegment) {
					var rows = [];
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoMref: rec["mref"], TcoSta: rec["sta"], TcoSto: rec["sto"], TcoDp0: rec["dp0"], TcoDp1: rec["dp1"], TcoOp0: rec["op0"], TcoOp1: rec["op1"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlySegOrgMNSegmentData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlySegOrgMNSegmentData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
