...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyUsrMNUsergroup.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyUsrMNUsergroupData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyUsrMNUsergroup": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyUsrMNUsergroup"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyUsrMNUsergroupData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyUsrMNUsergroupData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyUsrMNUsergroup) this.continf = dpcheng.ContInfBrlyUsrMNUsergroup;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyUsrMNUsergroup) this.statapp = dpcheng.StatAppBrlyUsrMNUsergroup;
				if (dpcheng.StatShrBrlyUsrMNUsergroup) this.statshr = dpcheng.StatShrBrlyUsrMNUsergroup;
				if (dpcheng.StgIacBrlyUsrMNUsergroup) this.stgiac = dpcheng.StgIacBrlyUsrMNUsergroup;
				if (dpcheng.TagBrlyUsrMNUsergroup) {
					Brly.unescapeBlock(dpcheng.TagBrlyUsrMNUsergroup);
					this.tag = dpcheng.TagBrlyUsrMNUsergroup;
				}
				if (dpcheng.ListBrlyQUsrMNUsergroup) this.rst = dpcheng.ListBrlyQUsrMNUsergroup;
				if (dpcheng.StatAppQryBrlyUsrMNUsergroup) this.statappqry = dpcheng.StatAppQryBrlyUsrMNUsergroup;
				if (dpcheng.StatShrQryBrlyUsrMNUsergroup) this.statshrqry = dpcheng.StatShrQryBrlyUsrMNUsergroup;
				if (dpcheng.StgIacQryBrlyUsrMNUsergroup) this.stgiacqry = dpcheng.StgIacQryBrlyUsrMNUsergroup;
				/*
				...
				*/
				...
				if (dpcheng.ListBrlyQUsrMNUsergroup) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyUsrMNUsergroupData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyUsrMNUsergroupData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
