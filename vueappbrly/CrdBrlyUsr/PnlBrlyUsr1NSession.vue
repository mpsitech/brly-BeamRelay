...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyUsr1NSession.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyUsr1NSessionData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyUsr1NSession": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyUsr1NSession"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyUsr1NSessionData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyUsr1NSessionData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyUsr1NSession) this.continf = dpcheng.ContInfBrlyUsr1NSession;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyUsr1NSession) this.statapp = dpcheng.StatAppBrlyUsr1NSession;
				if (dpcheng.StgIacBrlyUsr1NSession) this.stgiac = dpcheng.StgIacBrlyUsr1NSession;
				if (dpcheng.TagBrlyUsr1NSession) {
					Brly.unescapeBlock(dpcheng.TagBrlyUsr1NSession);
					this.tag = dpcheng.TagBrlyUsr1NSession;
				}
				if (dpcheng.ListBrlyQUsr1NSession) this.rst = dpcheng.ListBrlyQUsr1NSession;
				if (dpcheng.StatAppQryBrlyUsr1NSession) this.statappqry = dpcheng.StatAppQryBrlyUsr1NSession;
				if (dpcheng.StatShrQryBrlyUsr1NSession) this.statshrqry = dpcheng.StatShrQryBrlyUsr1NSession;
				if (dpcheng.StgIacQryBrlyUsr1NSession) this.stgiacqry = dpcheng.StgIacQryBrlyUsr1NSession;
				/*
				...
				*/
				...
				if (dpcheng.ListBrlyQUsr1NSession) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyUsr1NSessionData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyUsr1NSessionData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
