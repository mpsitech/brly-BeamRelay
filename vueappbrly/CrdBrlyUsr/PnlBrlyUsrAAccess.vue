...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyUsrAAccess.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyUsrAAccessData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyUsrAAccess": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyUsrAAccess"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyUsrAAccessData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyUsrAAccessData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyUsrAAccess) this.continf = dpcheng.ContInfBrlyUsrAAccess;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyUsrAAccess) this.statapp = dpcheng.StatAppBrlyUsrAAccess;
				if (dpcheng.StatShrBrlyUsrAAccess) this.statshr = dpcheng.StatShrBrlyUsrAAccess;
				if (dpcheng.StgIacBrlyUsrAAccess) this.stgiac = dpcheng.StgIacBrlyUsrAAccess;
				if (dpcheng.TagBrlyUsrAAccess) {
					Brly.unescapeBlock(dpcheng.TagBrlyUsrAAccess);
					this.tag = dpcheng.TagBrlyUsrAAccess;
				}
				if (dpcheng.ListBrlyQUsrAAccess) this.rst = dpcheng.ListBrlyQUsrAAccess;
				if (dpcheng.StatAppQryBrlyUsrAAccess) this.statappqry = dpcheng.StatAppQryBrlyUsrAAccess;
				if (dpcheng.StatShrQryBrlyUsrAAccess) this.statshrqry = dpcheng.StatShrQryBrlyUsrAAccess;
				if (dpcheng.StgIacQryBrlyUsrAAccess) this.stgiacqry = dpcheng.StgIacQryBrlyUsrAAccess;
				/*
				...
				*/
				...
				if (dpcheng.ListBrlyQUsrAAccess) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyUsrAAccessData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyUsrAAccessData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
