...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyUsrList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyUsrListData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyUsrList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyUsrList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyUsrListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyUsrListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacBrlyUsrList) this.contiac = dpcheng.ContIacBrlyUsrList;
				if (dpcheng.ContInfBrlyUsrList) this.continf = dpcheng.ContInfBrlyUsrList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrBrlyUsrList) this.statshr = dpcheng.StatShrBrlyUsrList;
				if (dpcheng.StgIacBrlyUsrList) this.stgiac = dpcheng.StgIacBrlyUsrList;
				if (dpcheng.TagBrlyUsrList) {
					Brly.unescapeBlock(dpcheng.TagBrlyUsrList);
					this.tag = dpcheng.TagBrlyUsrList;
				}
				if (dpcheng.ListBrlyQUsrList) this.rst = dpcheng.ListBrlyQUsrList;
				if (dpcheng.StatAppQryBrlyUsrList) this.statappqry = dpcheng.StatAppQryBrlyUsrList;
				if (dpcheng.StatShrQryBrlyUsrList) this.statshrqry = dpcheng.StatShrQryBrlyUsrList;
				if (dpcheng.StgIacQryBrlyUsrList) this.stgiacqry = dpcheng.StgIacQryBrlyUsrList;
				if (dpcheng.ContInfBrlyUsrList) {
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyUsrListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyUsrListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
