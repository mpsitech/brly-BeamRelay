...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyFilList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyFilListData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyFilList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyFilList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyFilListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyFilListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacBrlyFilList) this.contiac = dpcheng.ContIacBrlyFilList;
				if (dpcheng.ContInfBrlyFilList) this.continf = dpcheng.ContInfBrlyFilList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrBrlyFilList) this.statshr = dpcheng.StatShrBrlyFilList;
				if (dpcheng.StgIacBrlyFilList) this.stgiac = dpcheng.StgIacBrlyFilList;
				if (dpcheng.TagBrlyFilList) {
					Brly.unescapeBlock(dpcheng.TagBrlyFilList);
					this.tag = dpcheng.TagBrlyFilList;
				}
				if (dpcheng.ListBrlyQFilList) this.rst = dpcheng.ListBrlyQFilList;
				if (dpcheng.StatAppQryBrlyFilList) this.statappqry = dpcheng.StatAppQryBrlyFilList;
				if (dpcheng.StatShrQryBrlyFilList) this.statshrqry = dpcheng.StatShrQryBrlyFilList;
				if (dpcheng.StgIacQryBrlyFilList) this.stgiacqry = dpcheng.StgIacQryBrlyFilList;
				if (dpcheng.ContInfBrlyFilList) {
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyFilListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyFilListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
