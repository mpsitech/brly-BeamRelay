...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyFltList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyFltListData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyFltList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyFltList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyFltListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyFltListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacBrlyFltList) this.contiac = dpcheng.ContIacBrlyFltList;
				if (dpcheng.ContInfBrlyFltList) this.continf = dpcheng.ContInfBrlyFltList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrBrlyFltList) this.statshr = dpcheng.StatShrBrlyFltList;
				if (dpcheng.StgIacBrlyFltList) this.stgiac = dpcheng.StgIacBrlyFltList;
				if (dpcheng.TagBrlyFltList) {
					Brly.unescapeBlock(dpcheng.TagBrlyFltList);
					this.tag = dpcheng.TagBrlyFltList;
				}
				if (dpcheng.ListBrlyQFltList) this.rst = dpcheng.ListBrlyQFltList;
				if (dpcheng.StatAppQryBrlyFltList) this.statappqry = dpcheng.StatAppQryBrlyFltList;
				if (dpcheng.StatShrQryBrlyFltList) this.statshrqry = dpcheng.StatShrQryBrlyFltList;
				if (dpcheng.StgIacQryBrlyFltList) this.stgiacqry = dpcheng.StgIacQryBrlyFltList;
				if (dpcheng.ContInfBrlyFltList) {
				...
				*/
				...
				if (dpcheng.TagBrlyFltList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoEqp", text: this.tag.TcoEqp}, {value: "TcoLeg", text: this.tag.TcoLeg}, {value: "TcoSta", text: this.tag.TcoSta}, {value: "TcoSto", text: this.tag.TcoSto});
					/*
					*/
				...
				if (dpcheng.ListBrlyQFltList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoEqp: rec["eqp"], TcoLeg: rec["leg"], TcoSta: rec["sta"], TcoSto: rec["sto"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyFltListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyFltListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
