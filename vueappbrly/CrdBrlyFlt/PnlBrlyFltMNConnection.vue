...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyFltMNConnection.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyFltMNConnectionData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyFltMNConnection": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyFltMNConnection"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyFltMNConnectionData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyFltMNConnectionData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyFltMNConnection) this.continf = dpcheng.ContInfBrlyFltMNConnection;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyFltMNConnection) this.statapp = dpcheng.StatAppBrlyFltMNConnection;
				if (dpcheng.StatShrBrlyFltMNConnection) this.statshr = dpcheng.StatShrBrlyFltMNConnection;
				if (dpcheng.StgIacBrlyFltMNConnection) this.stgiac = dpcheng.StgIacBrlyFltMNConnection;
				if (dpcheng.TagBrlyFltMNConnection) {
					Brly.unescapeBlock(dpcheng.TagBrlyFltMNConnection);
					this.tag = dpcheng.TagBrlyFltMNConnection;
				}
				if (dpcheng.ListBrlyQFltMNConnection) this.rst = dpcheng.ListBrlyQFltMNConnection;
				if (dpcheng.StatAppQryBrlyFltMNConnection) this.statappqry = dpcheng.StatAppQryBrlyFltMNConnection;
				if (dpcheng.StatShrQryBrlyFltMNConnection) this.statshrqry = dpcheng.StatShrQryBrlyFltMNConnection;
				if (dpcheng.StgIacQryBrlyFltMNConnection) this.stgiacqry = dpcheng.StgIacQryBrlyFltMNConnection;
				/*
				...
				*/
				...
				if (dpcheng.TagBrlyFltMNConnection) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoMref", text: this.tag.TcoMref}, {value: "TcoSta", text: this.tag.TcoSta}, {value: "TcoSto", text: this.tag.TcoSto}, {value: "TcoLp0", text: this.tag.TcoLp0}, {value: "TcoLp1", text: this.tag.TcoLp1});
					/*
					*/
				...
				if (dpcheng.ListBrlyQFltMNConnection) {
					var rows = [];
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoMref: rec["mref"], TcoSta: rec["sta"], TcoSto: rec["sto"], TcoLp0: rec["lp0"], TcoLp1: rec["lp1"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyFltMNConnectionData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyFltMNConnectionData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
