...
<script>
	...
	*/
	...
	export default {
		name: 'PnlBrlyFltEqpMNConnection',
		...
		mounted() {
			//console.log("PnlBrlyFltEqpMNConnection.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleButClick: function(ctlsref) {
				const dpchapp = {
					"DpchAppBrlyFltEqpMNConnectionDo": {
						"scrJref": this.scrJref,
						"srefIxVDo": ctlsref
					}
				};
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyFltEqpMNConnectionData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyFltEqpMNConnection": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyFltEqpMNConnection"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyFltEqpMNConnectionData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyFltEqpMNConnectionData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyFltEqpMNConnection) this.continf = dpcheng.ContInfBrlyFltEqpMNConnection;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyFltEqpMNConnection) this.statapp = dpcheng.StatAppBrlyFltEqpMNConnection;
				if (dpcheng.StatShrBrlyFltEqpMNConnection) this.statshr = dpcheng.StatShrBrlyFltEqpMNConnection;
				if (dpcheng.StgIacBrlyFltEqpMNConnection) this.stgiac = dpcheng.StgIacBrlyFltEqpMNConnection;
				if (dpcheng.TagBrlyFltEqpMNConnection) {
					Brly.unescapeBlock(dpcheng.TagBrlyFltEqpMNConnection);
					this.tag = dpcheng.TagBrlyFltEqpMNConnection;
				}
				if (dpcheng.ListBrlyQFltEqpMNConnection) this.rst = dpcheng.ListBrlyQFltEqpMNConnection;
				if (dpcheng.StatAppQryBrlyFltEqpMNConnection) this.statappqry = dpcheng.StatAppQryBrlyFltEqpMNConnection;
				if (dpcheng.StatShrQryBrlyFltEqpMNConnection) this.statshrqry = dpcheng.StatShrQryBrlyFltEqpMNConnection;
				if (dpcheng.StgIacQryBrlyFltEqpMNConnection) this.stgiacqry = dpcheng.StgIacQryBrlyFltEqpMNConnection;
				/*
				...
				*/
				...
				if (dpcheng.ListBrlyQFltEqpMNConnection) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyFltEqpMNConnectionData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyFltEqpMNConnectionData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
