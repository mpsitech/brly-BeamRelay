...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyFltOrgMNFlight.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyFltOrgMNFlightData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyFltOrgMNFlight": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyFltOrgMNFlight"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyFltOrgMNFlightData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyFltOrgMNFlightData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyFltOrgMNFlight) this.continf = dpcheng.ContInfBrlyFltOrgMNFlight;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyFltOrgMNFlight) this.statapp = dpcheng.StatAppBrlyFltOrgMNFlight;
				if (dpcheng.StatShrBrlyFltOrgMNFlight) this.statshr = dpcheng.StatShrBrlyFltOrgMNFlight;
				if (dpcheng.StgIacBrlyFltOrgMNFlight) this.stgiac = dpcheng.StgIacBrlyFltOrgMNFlight;
				if (dpcheng.TagBrlyFltOrgMNFlight) {
					Brly.unescapeBlock(dpcheng.TagBrlyFltOrgMNFlight);
					this.tag = dpcheng.TagBrlyFltOrgMNFlight;
				}
				if (dpcheng.ListBrlyQFltOrgMNFlight) this.rst = dpcheng.ListBrlyQFltOrgMNFlight;
				if (dpcheng.StatAppQryBrlyFltOrgMNFlight) this.statappqry = dpcheng.StatAppQryBrlyFltOrgMNFlight;
				if (dpcheng.StatShrQryBrlyFltOrgMNFlight) this.statshrqry = dpcheng.StatShrQryBrlyFltOrgMNFlight;
				if (dpcheng.StgIacQryBrlyFltOrgMNFlight) this.stgiacqry = dpcheng.StgIacQryBrlyFltOrgMNFlight;
				/*
				...
				*/
				...
				if (dpcheng.TagBrlyFltOrgMNFlight) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoMref", text: this.tag.TcoMref}, {value: "TcoSta", text: this.tag.TcoSta}, {value: "TcoSto", text: this.tag.TcoSto}, {value: "TcoXsa", text: this.tag.TcoXsa}, {value: "TcoXso", text: this.tag.TcoXso});
					/*
					*/
				...
				if (dpcheng.ListBrlyQFltOrgMNFlight) {
					var rows = [];
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoMref: rec["mref"], TcoSta: rec["sta"], TcoSto: rec["sto"], TcoXsa: rec["xsa"], TcoXso: rec["xso"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyFltOrgMNFlightData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyFltOrgMNFlightData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
