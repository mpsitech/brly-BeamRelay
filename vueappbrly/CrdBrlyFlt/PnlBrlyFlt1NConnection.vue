...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyFlt1NConnection.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyFlt1NConnectionData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyFlt1NConnection": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyFlt1NConnection"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyFlt1NConnectionData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyFlt1NConnectionData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyFlt1NConnection) this.continf = dpcheng.ContInfBrlyFlt1NConnection;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyFlt1NConnection) this.statapp = dpcheng.StatAppBrlyFlt1NConnection;
				if (dpcheng.StatShrBrlyFlt1NConnection) this.statshr = dpcheng.StatShrBrlyFlt1NConnection;
				if (dpcheng.StgIacBrlyFlt1NConnection) this.stgiac = dpcheng.StgIacBrlyFlt1NConnection;
				if (dpcheng.TagBrlyFlt1NConnection) {
					Brly.unescapeBlock(dpcheng.TagBrlyFlt1NConnection);
					this.tag = dpcheng.TagBrlyFlt1NConnection;
				}
				if (dpcheng.ListBrlyQFlt1NConnection) this.rst = dpcheng.ListBrlyQFlt1NConnection;
				if (dpcheng.StatAppQryBrlyFlt1NConnection) this.statappqry = dpcheng.StatAppQryBrlyFlt1NConnection;
				if (dpcheng.StatShrQryBrlyFlt1NConnection) this.statshrqry = dpcheng.StatShrQryBrlyFlt1NConnection;
				if (dpcheng.StgIacQryBrlyFlt1NConnection) this.stgiacqry = dpcheng.StgIacQryBrlyFlt1NConnection;
				/*
				...
				*/
				...
				if (dpcheng.ListBrlyQFlt1NConnection) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyFlt1NConnectionData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyFlt1NConnectionData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
