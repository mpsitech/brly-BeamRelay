...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyUsgAAccess.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyUsgAAccessData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyUsgAAccess": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyUsgAAccess"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyUsgAAccessData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyUsgAAccessData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyUsgAAccess) this.continf = dpcheng.ContInfBrlyUsgAAccess;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyUsgAAccess) this.statapp = dpcheng.StatAppBrlyUsgAAccess;
				if (dpcheng.StatShrBrlyUsgAAccess) this.statshr = dpcheng.StatShrBrlyUsgAAccess;
				if (dpcheng.StgIacBrlyUsgAAccess) this.stgiac = dpcheng.StgIacBrlyUsgAAccess;
				if (dpcheng.TagBrlyUsgAAccess) {
					Brly.unescapeBlock(dpcheng.TagBrlyUsgAAccess);
					this.tag = dpcheng.TagBrlyUsgAAccess;
				}
				if (dpcheng.ListBrlyQUsgAAccess) this.rst = dpcheng.ListBrlyQUsgAAccess;
				if (dpcheng.StatAppQryBrlyUsgAAccess) this.statappqry = dpcheng.StatAppQryBrlyUsgAAccess;
				if (dpcheng.StatShrQryBrlyUsgAAccess) this.statshrqry = dpcheng.StatShrQryBrlyUsgAAccess;
				if (dpcheng.StgIacQryBrlyUsgAAccess) this.stgiacqry = dpcheng.StgIacQryBrlyUsgAAccess;
				/*
				...
				*/
				...
				if (dpcheng.ListBrlyQUsgAAccess) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyUsgAAccessData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyUsgAAccessData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
