...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyUsgList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyUsgListData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyUsgList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyUsgList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyUsgListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyUsgListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacBrlyUsgList) this.contiac = dpcheng.ContIacBrlyUsgList;
				if (dpcheng.ContInfBrlyUsgList) this.continf = dpcheng.ContInfBrlyUsgList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrBrlyUsgList) this.statshr = dpcheng.StatShrBrlyUsgList;
				if (dpcheng.StgIacBrlyUsgList) this.stgiac = dpcheng.StgIacBrlyUsgList;
				if (dpcheng.TagBrlyUsgList) {
					Brly.unescapeBlock(dpcheng.TagBrlyUsgList);
					this.tag = dpcheng.TagBrlyUsgList;
				}
				if (dpcheng.ListBrlyQUsgList) this.rst = dpcheng.ListBrlyQUsgList;
				if (dpcheng.StatAppQryBrlyUsgList) this.statappqry = dpcheng.StatAppQryBrlyUsgList;
				if (dpcheng.StatShrQryBrlyUsgList) this.statshrqry = dpcheng.StatShrQryBrlyUsgList;
				if (dpcheng.StgIacQryBrlyUsgList) this.stgiacqry = dpcheng.StgIacQryBrlyUsgList;
				if (dpcheng.ContInfBrlyUsgList) {
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyUsgListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyUsgListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
