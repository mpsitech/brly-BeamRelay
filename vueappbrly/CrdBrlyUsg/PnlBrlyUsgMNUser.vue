...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyUsgMNUser.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyUsgMNUserData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyUsgMNUser": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyUsgMNUser"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyUsgMNUserData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyUsgMNUserData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyUsgMNUser) this.continf = dpcheng.ContInfBrlyUsgMNUser;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyUsgMNUser) this.statapp = dpcheng.StatAppBrlyUsgMNUser;
				if (dpcheng.StatShrBrlyUsgMNUser) this.statshr = dpcheng.StatShrBrlyUsgMNUser;
				if (dpcheng.StgIacBrlyUsgMNUser) this.stgiac = dpcheng.StgIacBrlyUsgMNUser;
				if (dpcheng.TagBrlyUsgMNUser) {
					Brly.unescapeBlock(dpcheng.TagBrlyUsgMNUser);
					this.tag = dpcheng.TagBrlyUsgMNUser;
				}
				if (dpcheng.ListBrlyQUsgMNUser) this.rst = dpcheng.ListBrlyQUsgMNUser;
				if (dpcheng.StatAppQryBrlyUsgMNUser) this.statappqry = dpcheng.StatAppQryBrlyUsgMNUser;
				if (dpcheng.StatShrQryBrlyUsgMNUser) this.statshrqry = dpcheng.StatShrQryBrlyUsgMNUser;
				if (dpcheng.StgIacQryBrlyUsgMNUser) this.stgiacqry = dpcheng.StgIacQryBrlyUsgMNUser;
				/*
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyUsgMNUserData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyUsgMNUserData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
