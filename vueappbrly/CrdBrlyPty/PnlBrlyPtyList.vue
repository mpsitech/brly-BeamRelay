...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyPtyList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyPtyListData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyPtyList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyPtyList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyPtyListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyPtyListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacBrlyPtyList) this.contiac = dpcheng.ContIacBrlyPtyList;
				if (dpcheng.ContInfBrlyPtyList) this.continf = dpcheng.ContInfBrlyPtyList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrBrlyPtyList) this.statshr = dpcheng.StatShrBrlyPtyList;
				if (dpcheng.StgIacBrlyPtyList) this.stgiac = dpcheng.StgIacBrlyPtyList;
				if (dpcheng.TagBrlyPtyList) {
					Brly.unescapeBlock(dpcheng.TagBrlyPtyList);
					this.tag = dpcheng.TagBrlyPtyList;
				}
				if (dpcheng.ListBrlyQPtyList) this.rst = dpcheng.ListBrlyQPtyList;
				if (dpcheng.StatAppQryBrlyPtyList) this.statappqry = dpcheng.StatAppQryBrlyPtyList;
				if (dpcheng.StatShrQryBrlyPtyList) this.statshrqry = dpcheng.StatShrQryBrlyPtyList;
				if (dpcheng.StgIacQryBrlyPtyList) this.stgiacqry = dpcheng.StgIacQryBrlyPtyList;
				if (dpcheng.ContInfBrlyPtyList) {
				...
				*/
				...
				if (dpcheng.TagBrlyPtyList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoTit", text: this.tag.TcoTit}, {value: "TcoEty", text: this.tag.TcoEty}, {value: "TcoCap", text: this.tag.TcoCap});
					/*
					*/
				...
				if (dpcheng.ListBrlyQPtyList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoTit: rec["tit"], TcoEty: rec["ety2"], TcoCap: rec["cap"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyPtyListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyPtyListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
