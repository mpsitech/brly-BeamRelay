...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyReg1NLocation.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyReg1NLocationData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyReg1NLocation": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyReg1NLocation"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyReg1NLocationData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyReg1NLocationData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyReg1NLocation) this.continf = dpcheng.ContInfBrlyReg1NLocation;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyReg1NLocation) this.statapp = dpcheng.StatAppBrlyReg1NLocation;
				if (dpcheng.StatShrBrlyReg1NLocation) this.statshr = dpcheng.StatShrBrlyReg1NLocation;
				if (dpcheng.StgIacBrlyReg1NLocation) this.stgiac = dpcheng.StgIacBrlyReg1NLocation;
				if (dpcheng.TagBrlyReg1NLocation) {
					Brly.unescapeBlock(dpcheng.TagBrlyReg1NLocation);
					this.tag = dpcheng.TagBrlyReg1NLocation;
				}
				if (dpcheng.ListBrlyQReg1NLocation) this.rst = dpcheng.ListBrlyQReg1NLocation;
				if (dpcheng.StatAppQryBrlyReg1NLocation) this.statappqry = dpcheng.StatAppQryBrlyReg1NLocation;
				if (dpcheng.StatShrQryBrlyReg1NLocation) this.statshrqry = dpcheng.StatShrQryBrlyReg1NLocation;
				if (dpcheng.StgIacQryBrlyReg1NLocation) this.stgiacqry = dpcheng.StgIacQryBrlyReg1NLocation;
				/*
				...
				*/
				...
				if (dpcheng.ListBrlyQReg1NLocation) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyReg1NLocationData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyReg1NLocationData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
