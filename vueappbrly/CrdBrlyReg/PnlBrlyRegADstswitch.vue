...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyRegADstswitch.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyRegADstswitchData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyRegADstswitch": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyRegADstswitch"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyRegADstswitchData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyRegADstswitchData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyRegADstswitch) this.continf = dpcheng.ContInfBrlyRegADstswitch;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyRegADstswitch) this.statapp = dpcheng.StatAppBrlyRegADstswitch;
				if (dpcheng.StatShrBrlyRegADstswitch) this.statshr = dpcheng.StatShrBrlyRegADstswitch;
				if (dpcheng.StgIacBrlyRegADstswitch) this.stgiac = dpcheng.StgIacBrlyRegADstswitch;
				if (dpcheng.TagBrlyRegADstswitch) {
					Brly.unescapeBlock(dpcheng.TagBrlyRegADstswitch);
					this.tag = dpcheng.TagBrlyRegADstswitch;
				}
				if (dpcheng.ListBrlyQRegADstswitch) this.rst = dpcheng.ListBrlyQRegADstswitch;
				if (dpcheng.StatAppQryBrlyRegADstswitch) this.statappqry = dpcheng.StatAppQryBrlyRegADstswitch;
				if (dpcheng.StatShrQryBrlyRegADstswitch) this.statshrqry = dpcheng.StatShrQryBrlyRegADstswitch;
				if (dpcheng.StgIacQryBrlyRegADstswitch) this.stgiacqry = dpcheng.StgIacQryBrlyRegADstswitch;
				/*
				...
				*/
				...
				if (dpcheng.TagBrlyRegADstswitch) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoStd", text: this.tag.TcoStd}, {value: "TcoStt", text: this.tag.TcoStt});
					/*
					*/
				...
				if (dpcheng.ListBrlyQRegADstswitch) {
					var rows = [];
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoStd: rec["std"], TcoStt: rec["stt"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyRegADstswitchData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyRegADstswitchData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
