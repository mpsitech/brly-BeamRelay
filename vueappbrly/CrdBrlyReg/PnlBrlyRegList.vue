...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyRegList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyRegListData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyRegList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyRegList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyRegListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyRegListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacBrlyRegList) this.contiac = dpcheng.ContIacBrlyRegList;
				if (dpcheng.ContInfBrlyRegList) this.continf = dpcheng.ContInfBrlyRegList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrBrlyRegList) this.statshr = dpcheng.StatShrBrlyRegList;
				if (dpcheng.StgIacBrlyRegList) this.stgiac = dpcheng.StgIacBrlyRegList;
				if (dpcheng.TagBrlyRegList) {
					Brly.unescapeBlock(dpcheng.TagBrlyRegList);
					this.tag = dpcheng.TagBrlyRegList;
				}
				if (dpcheng.ListBrlyQRegList) this.rst = dpcheng.ListBrlyQRegList;
				if (dpcheng.StatAppQryBrlyRegList) this.statappqry = dpcheng.StatAppQryBrlyRegList;
				if (dpcheng.StatShrQryBrlyRegList) this.statshrqry = dpcheng.StatShrQryBrlyRegList;
				if (dpcheng.StgIacQryBrlyRegList) this.stgiacqry = dpcheng.StgIacQryBrlyRegList;
				if (dpcheng.ContInfBrlyRegList) {
				...
				*/
				...
				if (dpcheng.TagBrlyRegList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoTit", text: this.tag.TcoTit}, {value: "TcoSup", text: this.tag.TcoSup}, {value: "TcoDst", text: this.tag.TcoDst}, {value: "TcoTof", text: this.tag.TcoTof});
					/*
					*/
				...
				if (dpcheng.ListBrlyQRegList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoTit: rec["tit"], TcoSup: rec["sup"], TcoDst: rec["dst2"], TcoTof: rec["tof"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyRegListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyRegListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
