...
<script>
	...
	export default {
		...
		methods: {
			...
			handleLogin: function() {
				...
				dpch.post('', data).then(function (response) {
					const dpcheng = response.data.DpchEngBrlyConfirm;

					if (dpcheng.accepted) {
						//console.log("CrdBrlyStart.handleLogin() login accepted");
						...
						//console.log("CrdBrlyStart.handleLogin() login denied");

						vm.fiFSge = getFiBySref(vm.feedFSge, "dny");
					}

				}).catch(function (error) {
					console.log("CrdBrlyStart.handleLogin() error: " + error);
				});
			},
			...
			initSess: function(scrJrefSess) {
				...
				dpch.post('', data).then(function (response) {
					const dpcheng = response.data.DpchEngSessBrlyData;

					vm.$emit("login", {scrJrefSess: scrJrefSess, scrJrefCrdnav: dpcheng.StatShrSessBrly.scrJrefCrdnav});

				}).catch(function (error) {
					console.log("CrdBrlyStart.initSess() error: " + error);
				});
			},
			...
			handlePupLocaleChange: function() {
				...
				//console.log("CrdBrlyStart.handlePupLocaleChange() new locale is " + this.fiFPupLocale.sref);
			},
		...
		computed: {
			CptVersion() {
				return(Brly.title + " v" + Brly.version);
			},
