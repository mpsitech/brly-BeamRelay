...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyTtbList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyTtbListData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyTtbList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyTtbList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyTtbListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyTtbListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacBrlyTtbList) this.contiac = dpcheng.ContIacBrlyTtbList;
				if (dpcheng.ContInfBrlyTtbList) this.continf = dpcheng.ContInfBrlyTtbList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrBrlyTtbList) this.statshr = dpcheng.StatShrBrlyTtbList;
				if (dpcheng.StgIacBrlyTtbList) this.stgiac = dpcheng.StgIacBrlyTtbList;
				if (dpcheng.TagBrlyTtbList) {
					Brly.unescapeBlock(dpcheng.TagBrlyTtbList);
					this.tag = dpcheng.TagBrlyTtbList;
				}
				if (dpcheng.ListBrlyQTtbList) this.rst = dpcheng.ListBrlyQTtbList;
				if (dpcheng.StatAppQryBrlyTtbList) this.statappqry = dpcheng.StatAppQryBrlyTtbList;
				if (dpcheng.StatShrQryBrlyTtbList) this.statshrqry = dpcheng.StatShrQryBrlyTtbList;
				if (dpcheng.StgIacQryBrlyTtbList) this.stgiacqry = dpcheng.StgIacQryBrlyTtbList;
				if (dpcheng.ContInfBrlyTtbList) {
				...
				*/
				...
				if (dpcheng.TagBrlyTtbList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoTit", text: this.tag.TcoTit}, {value: "TcoFil", text: this.tag.TcoFil}, {value: "TcoAli", text: this.tag.TcoAli}, {value: "TcoSta", text: this.tag.TcoSta}, {value: "TcoSto", text: this.tag.TcoSto});
					/*
					*/
				...
				if (dpcheng.ListBrlyQTtbList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoTit: rec["tit"], TcoFil: rec["fil"], TcoAli: rec["ali2"], TcoSta: rec["sta"], TcoSto: rec["sto"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyTtbListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyTtbListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
