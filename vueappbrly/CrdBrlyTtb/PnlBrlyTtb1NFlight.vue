...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyTtb1NFlight.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyTtb1NFlightData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyTtb1NFlight": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyTtb1NFlight"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyTtb1NFlightData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyTtb1NFlightData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyTtb1NFlight) this.continf = dpcheng.ContInfBrlyTtb1NFlight;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyTtb1NFlight) this.statapp = dpcheng.StatAppBrlyTtb1NFlight;
				if (dpcheng.StatShrBrlyTtb1NFlight) this.statshr = dpcheng.StatShrBrlyTtb1NFlight;
				if (dpcheng.StgIacBrlyTtb1NFlight) this.stgiac = dpcheng.StgIacBrlyTtb1NFlight;
				if (dpcheng.TagBrlyTtb1NFlight) {
					Brly.unescapeBlock(dpcheng.TagBrlyTtb1NFlight);
					this.tag = dpcheng.TagBrlyTtb1NFlight;
				}
				if (dpcheng.ListBrlyQTtb1NFlight) this.rst = dpcheng.ListBrlyQTtb1NFlight;
				if (dpcheng.StatAppQryBrlyTtb1NFlight) this.statappqry = dpcheng.StatAppQryBrlyTtb1NFlight;
				if (dpcheng.StatShrQryBrlyTtb1NFlight) this.statshrqry = dpcheng.StatShrQryBrlyTtb1NFlight;
				if (dpcheng.StgIacQryBrlyTtb1NFlight) this.stgiacqry = dpcheng.StgIacQryBrlyTtb1NFlight;
				/*
				...
				*/
				...
				if (dpcheng.ListBrlyQTtb1NFlight) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyTtb1NFlightData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyTtb1NFlightData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
