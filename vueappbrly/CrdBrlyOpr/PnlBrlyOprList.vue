...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyOprList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyOprListData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyOprList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyOprList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyOprListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyOprListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacBrlyOprList) this.contiac = dpcheng.ContIacBrlyOprList;
				if (dpcheng.ContInfBrlyOprList) this.continf = dpcheng.ContInfBrlyOprList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrBrlyOprList) this.statshr = dpcheng.StatShrBrlyOprList;
				if (dpcheng.StgIacBrlyOprList) this.stgiac = dpcheng.StgIacBrlyOprList;
				if (dpcheng.TagBrlyOprList) {
					Brly.unescapeBlock(dpcheng.TagBrlyOprList);
					this.tag = dpcheng.TagBrlyOprList;
				}
				if (dpcheng.ListBrlyQOprList) this.rst = dpcheng.ListBrlyQOprList;
				if (dpcheng.StatAppQryBrlyOprList) this.statappqry = dpcheng.StatAppQryBrlyOprList;
				if (dpcheng.StatShrQryBrlyOprList) this.statshrqry = dpcheng.StatShrQryBrlyOprList;
				if (dpcheng.StgIacQryBrlyOprList) this.stgiacqry = dpcheng.StgIacQryBrlyOprList;
				if (dpcheng.ContInfBrlyOprList) {
				...
				*/
				...
				if (dpcheng.TagBrlyOprList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoIca", text: this.tag.TcoIca}, {value: "TcoTit", text: this.tag.TcoTit});
					/*
					*/
				...
				if (dpcheng.ListBrlyQOprList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoIca: rec["ica"], TcoTit: rec["tit"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyOprListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyOprListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
