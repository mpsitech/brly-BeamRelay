...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyLegOrgMNLeg.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleButClipboardClick: function() {
				...
				*/
				Brly.copyToClipboard(document, this.continf, this.rst, this.statshrqry, this.tag,
							["Mref","Dp0","Dp1","Op0","Op1","Xd0","Xd1","Xo0","Xo1"],
							["mref","dp0","dp1","op0","op1","xd0","xd1","xo0","xo1"]);
				/*
				*/
			},
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyLegOrgMNLegData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyLegOrgMNLeg": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyLegOrgMNLeg"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyLegOrgMNLegData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyLegOrgMNLegData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyLegOrgMNLeg) this.continf = dpcheng.ContInfBrlyLegOrgMNLeg;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyLegOrgMNLeg) this.statapp = dpcheng.StatAppBrlyLegOrgMNLeg;
				if (dpcheng.StatShrBrlyLegOrgMNLeg) this.statshr = dpcheng.StatShrBrlyLegOrgMNLeg;
				if (dpcheng.StgIacBrlyLegOrgMNLeg) this.stgiac = dpcheng.StgIacBrlyLegOrgMNLeg;
				if (dpcheng.TagBrlyLegOrgMNLeg) {
					Brly.unescapeBlock(dpcheng.TagBrlyLegOrgMNLeg);
					this.tag = dpcheng.TagBrlyLegOrgMNLeg;
				}
				if (dpcheng.ListBrlyQLegOrgMNLeg) this.rst = dpcheng.ListBrlyQLegOrgMNLeg;
				if (dpcheng.StatAppQryBrlyLegOrgMNLeg) this.statappqry = dpcheng.StatAppQryBrlyLegOrgMNLeg;
				if (dpcheng.StatShrQryBrlyLegOrgMNLeg) this.statshrqry = dpcheng.StatShrQryBrlyLegOrgMNLeg;
				if (dpcheng.StgIacQryBrlyLegOrgMNLeg) this.stgiacqry = dpcheng.StgIacQryBrlyLegOrgMNLeg;
				/*
				...
				*/
				...
				if (dpcheng.TagBrlyLegOrgMNLeg) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoMref", text: this.tag.TcoMref}, {value: "TcoDp0", text: this.tag.TcoDp0}, {value: "TcoDp1", text: this.tag.TcoDp1}, {value: "TcoOp0", text: this.tag.TcoOp0}, {value: "TcoOp1", text: this.tag.TcoOp1}, {value: "TcoXd0", text: this.tag.TcoXd0}, {value: "TcoXd1", text: this.tag.TcoXd1}, {value: "TcoXo0", text: this.tag.TcoXo0}, {value: "TcoXo1", text: this.tag.TcoXo1});
					/*
					*/
				...
				if (dpcheng.ListBrlyQLegOrgMNLeg) {
					var rows = [];
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoMref: rec["mref"], TcoDp0: rec["dp0"], TcoDp1: rec["dp1"], TcoOp0: rec["op0"], TcoOp1: rec["op1"], TcoXd0: rec["xd0"], TcoXd1: rec["xd1"], TcoXo0: rec["xo0"], TcoXo1: rec["xo1"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyLegOrgMNLegData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyLegOrgMNLegData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
