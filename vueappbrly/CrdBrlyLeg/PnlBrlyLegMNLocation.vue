...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyLegMNLocation.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyLegMNLocationData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyLegMNLocation": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyLegMNLocation"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyLegMNLocationData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyLegMNLocationData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyLegMNLocation) this.continf = dpcheng.ContInfBrlyLegMNLocation;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyLegMNLocation) this.statapp = dpcheng.StatAppBrlyLegMNLocation;
				if (dpcheng.StatShrBrlyLegMNLocation) this.statshr = dpcheng.StatShrBrlyLegMNLocation;
				if (dpcheng.StgIacBrlyLegMNLocation) this.stgiac = dpcheng.StgIacBrlyLegMNLocation;
				if (dpcheng.TagBrlyLegMNLocation) {
					Brly.unescapeBlock(dpcheng.TagBrlyLegMNLocation);
					this.tag = dpcheng.TagBrlyLegMNLocation;
				}
				if (dpcheng.ListBrlyQLegMNLocation) this.rst = dpcheng.ListBrlyQLegMNLocation;
				if (dpcheng.StatAppQryBrlyLegMNLocation) this.statappqry = dpcheng.StatAppQryBrlyLegMNLocation;
				if (dpcheng.StatShrQryBrlyLegMNLocation) this.statshrqry = dpcheng.StatShrQryBrlyLegMNLocation;
				if (dpcheng.StgIacQryBrlyLegMNLocation) this.stgiacqry = dpcheng.StgIacQryBrlyLegMNLocation;
				/*
				...
				*/
				...
				if (dpcheng.TagBrlyLegMNLocation) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoMref", text: this.tag.TcoMref}, {value: "TcoLp0", text: this.tag.TcoLp0}, {value: "TcoLp1", text: this.tag.TcoLp1}, {value: "TcoLph", text: this.tag.TcoLph}, {value: "TcoLth", text: this.tag.TcoLth}, {value: "TcoXp0", text: this.tag.TcoXp0}, {value: "TcoXp1", text: this.tag.TcoXp1});
					/*
					*/
				...
				if (dpcheng.ListBrlyQLegMNLocation) {
					var rows = [];
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoMref: rec["mref"], TcoLp0: rec["lp0"], TcoLp1: rec["lp1"], TcoLph: rec["lph"], TcoLth: rec["lth"], TcoXp0: rec["xp0"], TcoXp1: rec["xp1"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyLegMNLocationData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyLegMNLocationData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
