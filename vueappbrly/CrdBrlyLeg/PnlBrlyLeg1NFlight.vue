...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlBrlyLeg1NFlight.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppBrlyLeg1NFlightData": {
						"scrJref": this.scrJref,
						"StgIacQryBrlyLeg1NFlight": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacBrlyLeg1NFlight"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppBrlyLeg1NFlightData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppBrlyLeg1NFlightData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfBrlyLeg1NFlight) this.continf = dpcheng.ContInfBrlyLeg1NFlight;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppBrlyLeg1NFlight) this.statapp = dpcheng.StatAppBrlyLeg1NFlight;
				if (dpcheng.StatShrBrlyLeg1NFlight) this.statshr = dpcheng.StatShrBrlyLeg1NFlight;
				if (dpcheng.StgIacBrlyLeg1NFlight) this.stgiac = dpcheng.StgIacBrlyLeg1NFlight;
				if (dpcheng.TagBrlyLeg1NFlight) {
					Brly.unescapeBlock(dpcheng.TagBrlyLeg1NFlight);
					this.tag = dpcheng.TagBrlyLeg1NFlight;
				}
				if (dpcheng.ListBrlyQLeg1NFlight) this.rst = dpcheng.ListBrlyQLeg1NFlight;
				if (dpcheng.StatAppQryBrlyLeg1NFlight) this.statappqry = dpcheng.StatAppQryBrlyLeg1NFlight;
				if (dpcheng.StatShrQryBrlyLeg1NFlight) this.statshrqry = dpcheng.StatShrQryBrlyLeg1NFlight;
				if (dpcheng.StgIacQryBrlyLeg1NFlight) this.stgiacqry = dpcheng.StgIacQryBrlyLeg1NFlight;
				/*
				...
				*/
				...
				if (dpcheng.ListBrlyQLeg1NFlight) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxBrlyVDpch, dpcheng) {
				if (srefIxBrlyVDpch == "DpchEngBrlyLeg1NFlightData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxBrlyVDpch == "DpchEngBrlyLeg1NFlightData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
