...
function refreshBD(bNotD, chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyUsgMNUser", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyUsgMNUser", "ListBrlyQUsgMNUser", retrieveTi(srcdoc, "TagBrlyUsgMNUser", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyUsgMNUser", "TxtRecord2"),
				["Mref","Ulv",""],
				["mref","ulv","ulv2"]);
};
