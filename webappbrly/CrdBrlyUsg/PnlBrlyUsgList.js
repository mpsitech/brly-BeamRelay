...
function refreshB(chgcol) {
	...
	if (chgcol) {
		...
		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyUsgList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyUsgList", "TcoOwnWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyUsgList", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyUsgList", "ListBrlyQUsgList", retrieveTi(srcdoc, "TagBrlyUsgList", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyUsgList", "TxtRecord2"),
				["Grp","Own","Srf"],
				["grp","own","srf"]);
};
