	...
	if (chgcol) {
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoFil"); doc.widths.push(TcoFilWidth); doc.fs.push("fil"); doc.tos.push(1); wtot += TcoFilWidth;
			} else if (i == 3) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyTtbList", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyTtbList", "ListBrlyQTtbList", retrieveTi(srcdoc, "TagBrlyTtbList", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyTtbList", "TxtRecord2"),
				["Tit","Fil","","Ali","Sta","Sto"],
				["tit","fil","ali","ali2","sta","sto"]);
};
