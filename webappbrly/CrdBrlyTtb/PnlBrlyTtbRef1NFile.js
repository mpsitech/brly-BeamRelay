...
function refreshBD(bNotD, chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyTtbRef1NFile", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyTtbRef1NFile", "ListBrlyQTtbRef1NFile", retrieveTi(srcdoc, "TagBrlyTtbRef1NFile", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyTtbRef1NFile", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
