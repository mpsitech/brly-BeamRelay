...
function refreshBD(bNotD, chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlySegMNConnection", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlySegMNConnection", "ListBrlyQSegMNConnection", retrieveTi(srcdoc, "TagBrlySegMNConnection", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlySegMNConnection", "TxtRecord2"),
