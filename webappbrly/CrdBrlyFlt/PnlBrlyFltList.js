	...
	if (chgcol) {
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoEqp"); doc.widths.push(TcoEqpWidth); doc.fs.push("eqp"); doc.tos.push(5); wtot += TcoEqpWidth;
			} else if (i == 3) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyFltList", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyFltList", "ListBrlyQFltList", retrieveTi(srcdoc, "TagBrlyFltList", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyFltList", "TxtRecord2"),
				["Srf","Eqp","Leg","Sta","Sto"],
				["srf","eqp","leg","sta","sto"]);
};
