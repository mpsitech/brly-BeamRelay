...
function refreshBD(bNotD, chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyFlt1NConnection", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyFlt1NConnection", "ListBrlyQFlt1NConnection", retrieveTi(srcdoc, "TagBrlyFlt1NConnection", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyFlt1NConnection", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
