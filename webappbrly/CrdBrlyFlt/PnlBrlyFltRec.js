	...
	if (_scrJref == scrJref) {
		...
		if (srefIxBrlyVExpstate != "mind") {
			...
			if (_scrJref == scrJrefPre) {
				...
				if (getInitdone("FafAWaypoint")) lhsdoc.getElementById("FafAWaypoint").contentWindow.handleDpchEng(dom, dpch);
			} else if (_scrJref == scrJref1NConnection) {
				if (getInitdone("1NConnection")) rhsdoc.getElementById("1NConnection").contentWindow.handleDpchEng(dom, dpch);
