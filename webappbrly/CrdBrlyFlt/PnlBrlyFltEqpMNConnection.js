...
function refreshBD(bNotD, chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyFltEqpMNConnection", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyFltEqpMNConnection", "ListBrlyQFltEqpMNConnection", retrieveTi(srcdoc, "TagBrlyFltEqpMNConnection", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyFltEqpMNConnection", "TxtRecord2"),
				["Mref",""],
				["mref","lat"]);
};
