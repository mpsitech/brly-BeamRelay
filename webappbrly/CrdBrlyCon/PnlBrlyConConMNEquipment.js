...
function refreshBD(bNotD, chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyConConMNEquipment", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyConConMNEquipment", "ListBrlyQConConMNEquipment", retrieveTi(srcdoc, "TagBrlyConConMNEquipment", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyConConMNEquipment", "TxtRecord2"),
				["Mref",""],
				["mref","lat"]);
};
