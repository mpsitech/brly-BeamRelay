...
function refreshB(chgcol) {
	...
	if (chgcol) {
		...
		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyConList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyConList", "TcoOwnWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoMod"); doc.widths.push(TcoModWidth); doc.fs.push("mod2"); doc.tos.push(0); wtot += TcoModWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyConList", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyConList", "ListBrlyQConList", retrieveTi(srcdoc, "TagBrlyConList", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyConList", "TxtRecord2"),
				["Grp","Own","Flt","Cor","Sta","Sto","","Mod"],
				["grp","own","flt","cor","sta","sto","mod","mod2"]);
};
