...
function updateHeight() {
	...
	heightRhs = height1NRelay+13 + heightRef1NSegment+13 + heightConMNEquipment+13 + heightMNFlight+13 + heightMNSegment+13 + 5;
	...
	if (_scrJref == scrJref) {
		...
		if (srefIxBrlyVExpstate != "mind") {
			...
			if (_scrJref == scrJrefPre) {
				if (getInitdone("Pre")) lhsdoc.getElementById("Pre").contentWindow.handleDpchEng(dom, dpch);
				...
				if (getInitdone("1NRelay")) rhsdoc.getElementById("1NRelay").contentWindow.handleDpchEng(dom, dpch);
			} else if (_scrJref == scrJrefRef1NSegment) {
				if (getInitdone("Ref1NSegment")) rhsdoc.getElementById("Ref1NSegment").contentWindow.handleDpchEng(dom, dpch);
			} else if (_scrJref == scrJrefConMNEquipment) {
				if (getInitdone("ConMNEquipment")) rhsdoc.getElementById("ConMNEquipment").contentWindow.handleDpchEng(dom, dpch);
