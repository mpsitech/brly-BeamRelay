...
function refreshBD(bNotD, chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyCon1NRelay", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyCon1NRelay", "ListBrlyQCon1NRelay", retrieveTi(srcdoc, "TagBrlyCon1NRelay", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyCon1NRelay", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
