...
function refreshBD(bNotD) {
	...
	height -= setCtlAvail(contcontdoc, "Reg2", LstRegAvail && !LstRegAlt, (LstRegAvail) ? 71 : 0);
	if (LstRegAvail) {
	...
	height -= setCtlAvail(contcontdoc, "Leg2", LstLegAvail && !LstLegAlt, (LstLegAvail) ? 71 : 0);
	if (LstLegAvail) {
	...
	height -= setCtlAvail(contcontdoc, "Ttb2", LstTtbAvail && !LstTtbAlt, (LstTtbAvail) ? 71 : 0);
	if (LstTtbAvail) {
	...
	height -= setCtlAvail(contcontdoc, "Flt2", LstFltAvail && !LstFltAlt, (LstFltAvail) ? 71 : 0);
	if (LstFltAvail) {
...
function handleLstSelect(ctlsref, multsel, num) {
	...
	if (multsel) {
		...
		setCi(srcdoc, "ContIacBrlyNavBase", "numsF" + ctlsref, encodeUintvec(numsFLst));
