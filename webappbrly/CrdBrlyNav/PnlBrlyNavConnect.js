...
function refreshBD(bNotD) {
	...
	height -= setCtlAvail(contcontdoc, "Rly2", LstRlyAvail && !LstRlyAlt, (LstRlyAvail) ? 71 : 0);
	if (LstRlyAvail) {
		...
		if ((ButRlyViewAvail == !contcontdoc.getElementById("ButRlyView")) || !contcontdoc.getElementById("ButRlyNewcrd")) {
			...
			mytd.appendChild(makeImgBut(contcontdoc, "ButRlyNewcrd", "icon/newcrd"));
		};
...
function handleLstSelect(ctlsref, multsel, num) {
	...
	if (multsel) {
		...
		setCi(srcdoc, "ContIacBrlyNavConnect", "numsF" + ctlsref, encodeUintvec(numsFLst));
