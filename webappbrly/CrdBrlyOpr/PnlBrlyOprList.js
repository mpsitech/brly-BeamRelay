...
function refreshB(chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyOprList", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyOprList", "ListBrlyQOprList", retrieveTi(srcdoc, "TagBrlyOprList", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyOprList", "TxtRecord2"),
				["Srf","Ica","Tit"],
				["srf","ica","tit"]);
};
