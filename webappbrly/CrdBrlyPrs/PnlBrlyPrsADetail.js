...
function refreshBD(bNotD, chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyPrsADetail", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyPrsADetail", "ListBrlyQPrsADetail", retrieveTi(srcdoc, "TagBrlyPrsADetail", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyPrsADetail", "TxtRecord2"),
				["Typ","","Val"],
				["typ","typ2","val"]);
};
