...
function refreshB(chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyPrsList", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyPrsList", "ListBrlyQPrsList", retrieveTi(srcdoc, "TagBrlyPrsList", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyPrsList", "TxtRecord2"),
