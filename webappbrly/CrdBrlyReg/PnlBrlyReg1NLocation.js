...
function refreshBD(bNotD, chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyReg1NLocation", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyReg1NLocation", "ListBrlyQReg1NLocation", retrieveTi(srcdoc, "TagBrlyReg1NLocation", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyReg1NLocation", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
