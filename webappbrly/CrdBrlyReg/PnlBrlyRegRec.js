	...
	if (_scrJref == scrJref) {
		...
		if (srefIxBrlyVExpstate != "mind") {
			...
			if (_scrJref == scrJrefDetail) {
				...
				if (getInitdone("ADstswitch")) lhsdoc.getElementById("ADstswitch").contentWindow.handleDpchEng(dom, dpch);
			} else if (_scrJref == scrJref1NLocation) {
				if (getInitdone("1NLocation")) rhsdoc.getElementById("1NLocation").contentWindow.handleDpchEng(dom, dpch);
			} else if (_scrJref == scrJrefSup1NRegion) {
				if (getInitdone("Sup1NRegion")) rhsdoc.getElementById("Sup1NRegion").contentWindow.handleDpchEng(dom, dpch);
			} else {
