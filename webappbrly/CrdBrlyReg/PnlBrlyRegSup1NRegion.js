...
function refreshBD(bNotD, chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyRegSup1NRegion", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyRegSup1NRegion", "ListBrlyQRegSup1NRegion", retrieveTi(srcdoc, "TagBrlyRegSup1NRegion", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyRegSup1NRegion", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
