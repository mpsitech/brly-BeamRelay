	...
	if (chgcol) {
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoDst"); doc.widths.push(TcoDstWidth); doc.fs.push("dst2"); doc.tos.push(0); wtot += TcoDstWidth;
			} else if (i == 5) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyRegList", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyRegList", "ListBrlyQRegList", retrieveTi(srcdoc, "TagBrlyRegList", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyRegList", "TxtRecord2"),
				["Srf","Tit","Sup","","Dst","Tof"],
				["srf","tit","sup","dst","dst2","tof"]);
};
