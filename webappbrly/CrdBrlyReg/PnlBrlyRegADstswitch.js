...
function refreshBD(bNotD, chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyRegADstswitch", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyRegADstswitch", "ListBrlyQRegADstswitch", retrieveTi(srcdoc, "TagBrlyRegADstswitch", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyRegADstswitch", "TxtRecord2"),
				["Std","Stt"],
				["std","stt"]);
};
