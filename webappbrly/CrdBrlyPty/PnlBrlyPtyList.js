...
function refreshB(chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyPtyList", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyPtyList", "ListBrlyQPtyList", retrieveTi(srcdoc, "TagBrlyPtyList", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyPtyList", "TxtRecord2"),
				["Srf","Tit","","Ety","Cap"],
				["srf","tit","ety","ety2","cap"]);
};
