...
function refreshB(chgcol) {
	...
	if (chgcol) {
		...
		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegList", "TcoOwnWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoBlo"); doc.widths.push(TcoBloWidth); doc.fs.push("blo"); doc.tos.push(3); wtot += TcoBloWidth;
			} else if (i == 4) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyLegList", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyLegList", "ListBrlyQLegList", retrieveTi(srcdoc, "TagBrlyLegList", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyLegList", "TxtRecord2"),
