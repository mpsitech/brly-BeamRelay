...
function refreshBD(bNotD, chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyLegCor1NConnection", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyLegCor1NConnection", "ListBrlyQLegCor1NConnection", retrieveTi(srcdoc, "TagBrlyLegCor1NConnection", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyLegCor1NConnection", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
