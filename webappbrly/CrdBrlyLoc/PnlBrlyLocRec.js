...
function updateHeight() {
	...
	heightRhs = heightBgn1NLeg+13 + heightEnd1NLeg+13 + heightEqp1NNode+13 + heightMNSegment+13 + heightMNFlight+13 + heightMNLeg+13 + heightVisOrgMNLocation+13 + heightProxOrgMNLocation+13 + heightEqpMNConnection+13 + 5;
	...
	if (_scrJref == scrJref) {
		...
		if (srefIxBrlyVExpstate != "mind") {
			...
			if (_scrJref == scrJrefDetail) {
				...
				if (getInitdone("Map")) lhsdoc.getElementById("Map").contentWindow.handleDpchEng(dom, dpch);
			} else if (_scrJref == scrJrefBgn1NLeg) {
				if (getInitdone("Bgn1NLeg")) rhsdoc.getElementById("Bgn1NLeg").contentWindow.handleDpchEng(dom, dpch);
			} else if (_scrJref == scrJrefEnd1NLeg) {
				if (getInitdone("End1NLeg")) rhsdoc.getElementById("End1NLeg").contentWindow.handleDpchEng(dom, dpch);
				...
				if (getInitdone("MNSegment")) rhsdoc.getElementById("MNSegment").contentWindow.handleDpchEng(dom, dpch);
			} else if (_scrJref == scrJrefMNFlight) {
				if (getInitdone("MNFlight")) rhsdoc.getElementById("MNFlight").contentWindow.handleDpchEng(dom, dpch);
			} else if (_scrJref == scrJrefMNLeg) {
				if (getInitdone("MNLeg")) rhsdoc.getElementById("MNLeg").contentWindow.handleDpchEng(dom, dpch);
			} else if (_scrJref == scrJrefVisOrgMNLocation) {
				if (getInitdone("VisOrgMNLocation")) rhsdoc.getElementById("VisOrgMNLocation").contentWindow.handleDpchEng(dom, dpch);
			} else if (_scrJref == scrJrefProxOrgMNLocation) {
				if (getInitdone("ProxOrgMNLocation")) rhsdoc.getElementById("ProxOrgMNLocation").contentWindow.handleDpchEng(dom, dpch);
