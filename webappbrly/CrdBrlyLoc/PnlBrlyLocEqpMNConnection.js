...
function refreshBD(bNotD, chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyLocEqpMNConnection", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyLocEqpMNConnection", "ListBrlyQLocEqpMNConnection", retrieveTi(srcdoc, "TagBrlyLocEqpMNConnection", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyLocEqpMNConnection", "TxtRecord2"),
				["Mref",""],
				["mref","lat"]);
};
