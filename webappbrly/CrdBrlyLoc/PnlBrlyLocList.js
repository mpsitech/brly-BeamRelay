	...
	if (chgcol) {
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoIca"); doc.widths.push(TcoIcaWidth); doc.fs.push("ica"); doc.tos.push(5); wtot += TcoIcaWidth;
			} else if (i == 3) {
				...
				doc.cols.push("TcoReg"); doc.widths.push(TcoRegWidth); doc.fs.push("reg"); doc.tos.push(2); wtot += TcoRegWidth;
			} else if (i == 6) {
				doc.cols.push("TcoEqp"); doc.widths.push(TcoEqpWidth); doc.fs.push("eqp"); doc.tos.push(1); wtot += TcoEqpWidth;
			} else if (i == 7) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyLocList", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyLocList", "ListBrlyQLocList", retrieveTi(srcdoc, "TagBrlyLocList", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyLocList", "TxtRecord2"),
