...
function refreshBD(bNotD, chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyLocProxOrgMNLocation", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyLocProxOrgMNLocation", "ListBrlyQLocProxOrgMNLocation", retrieveTi(srcdoc, "TagBrlyLocProxOrgMNLocation", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyLocProxOrgMNLocation", "TxtRecord2"),
				["Mref","Dph"],
				["mref","dph"]);
};
