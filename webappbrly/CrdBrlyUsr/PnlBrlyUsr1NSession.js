...
function refreshBD(bNotD, chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyUsr1NSession", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyUsr1NSession", "ListBrlyQUsr1NSession", retrieveTi(srcdoc, "TagBrlyUsr1NSession", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyUsr1NSession", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
