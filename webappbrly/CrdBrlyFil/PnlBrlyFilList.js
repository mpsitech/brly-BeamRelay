...
function refreshB(chgcol) {
	...
	if (chgcol) {
		...
		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFilList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFilList", "TcoOwnWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyFilList", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyFilList", "ListBrlyQFilList", retrieveTi(srcdoc, "TagBrlyFilList", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyFilList", "TxtRecord2"),
