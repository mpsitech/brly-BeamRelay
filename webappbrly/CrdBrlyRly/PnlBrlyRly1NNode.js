...
function refreshBD(bNotD, chgcol) {
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyRly1NNode", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyRly1NNode", "ListBrlyQRly1NNode", retrieveTi(srcdoc, "TagBrlyRly1NNode", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyRly1NNode", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
