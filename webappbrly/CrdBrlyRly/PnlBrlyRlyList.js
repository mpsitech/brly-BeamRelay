	...
	if (chgcol) {
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoCon"); doc.widths.push(TcoConWidth); doc.fs.push("con"); doc.tos.push(2); wtot += TcoConWidth;
			} else if (i == 4) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyRlyList", doc.cols[j]));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "BrlyRlyList", "ListBrlyQRlyList", retrieveTi(srcdoc, "TagBrlyRlyList", "TxtRecord1"), retrieveTi(srcdoc, "TagBrlyRlyList", "TxtRecord2"),
				["Sta","Sto","Con","","Dir","Ctd"],
				["sta","sto","con","dir","dir2","ctd"]);
};
