...
ubigint Acv::addfile(
	...
	dbsbrly->tblbrlymfile->insertNewRec(&fil, grp, own, 0, refIxVTbl, refUref, osrefKContent, Archived, Filename, "", srefKMimetype, Size, Comment);
...
string StubBrly::getStub(
	...
	else if (ixBrlyVStub == VecBrlyVStub::STUBBRLYGROUP) return getStubGroup(dbsbrly, ref, ixBrlyVLocale, ixVNonetype, stcch, strefSub, refresh);
	...
	else if (ixBrlyVStub == VecBrlyVStub::STUBBRLYOWNER) return getStubOwner(dbsbrly, ref, ixBrlyVLocale, ixVNonetype, stcch, strefSub, refresh);
...
string StubBrly::getStubGroup(
	...
	stcchitemref_t stref(VecBrlyVStub::STUBBRLYGROUP, ref, ixBrlyVLocale);
	Stcchitem* stit = NULL;
...
string StubBrly::getStubLocStd(
	...
	if (ref != 0) {
		if (dbsbrly->tblbrlymlocation->loadRecByRef(ref, &rec)) {
			...
			stub = "(" + VecBrlyVMLocationBasetype::getTitle(rec->ixVBasetype, ixBrlyVLocale) + ") " + rec->Title;
...
string StubBrly::getStubOwner(
	...
	stcchitemref_t stref(VecBrlyVStub::STUBBRLYOWNER, ref, ixBrlyVLocale);
	Stcchitem* stit = NULL;
