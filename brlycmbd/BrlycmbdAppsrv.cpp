...
int BrlycmbdAppsrv::MhdCallback(
	...
	if (*con_cls == NULL) {
		...
		if (strcmp(method, "GET") == 0) {
			...
			if (s == "/") {
				ixVBasetype = ReqBrly::VecVBasetype::REDIRECT;
		...
		if (ixVBasetype == ReqBrly::VecVBasetype::NONE) {
			...
			MHD_add_response_header(response, "Location", "/web/CrdBrlyStart/CrdBrlyStart.html");
			...
			*con_cls = req;
			...
			if (req->ixVBasetype == ReqBrly::VecVBasetype::WEB) {
				...
				if (valid) {
					...
					if (dcol->dpchs.empty()) {
						if (dcol->hot) {
							...
							req->cReady.lockMutex("BrlycmbdAppsrv", "MhdCallback[1]");
							dcol->unlockAccess("BrlycmbdAppsrv", "MhdCallback[3]");
							req->cReady.timedwait(15000000, "BrlycmbdAppsrv", "MhdCallback");
							// cout << "got one after timedwait" << endl;
							req->cReady.unlockMutex("BrlycmbdAppsrv", "MhdCallback[1]");
				...
				if (valid) {
					...
					// wait for reply
					req->cReady.lockMutex("BrlycmbdAppsrv", "MhdCallback[2]");
					if (req->ixVState != ReqBrly::VecVState::REPLY) req->cReady.wait("BrlycmbdAppsrv", "MhdCallback[1]");
					req->cReady.unlockMutex("BrlycmbdAppsrv", "MhdCallback[2]");
				...
				*upload_data_size = 0;
				...
				if (req->ixVBasetype == ReqBrly::VecVBasetype::DPCHAPP) {
					...
					if (valid) {
						...
						// wait for reply
						req->cReady.lockMutex("BrlycmbdAppsrv", "MhdCallback[3]");
						if (req->ixVState != ReqBrly::VecVState::REPLY) req->cReady.wait("BrlycmbdAppsrv", "MhdCallback[2]");
						req->cReady.unlockMutex("BrlycmbdAppsrv", "MhdCallback[3]");
					...
					if (valid) {
						...
						req->cReady.lockMutex("BrlycmbdAppsrv", "MhdCallback[4]");
						if (req->ixVState != ReqBrly::VecVState::REPLY) req->cReady.wait("BrlycmbdAppsrv", "MhdCallback[3]");
						req->cReady.unlockMutex("BrlycmbdAppsrv", "MhdCallback[4]");
