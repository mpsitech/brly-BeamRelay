...
PnlBrlyNavConnect::PnlBrlyNavConnect(
	...
	xchg->addIxRefClstn(VecBrlyVCall::CALLBRLYHUSRRUNVMOD_CRDUSREQ, jref, Clstn::VecVJobmask::ALL, 0, false, VecBrlyVCard::CRDBRLYCON, xchg->getRefPreset(VecBrlyVPreset::PREBRLYOWNER, jref));
	xchg->addIxRefClstn(VecBrlyVCall::CALLBRLYHUSRRUNVMOD_CRDUSREQ, jref, Clstn::VecVJobmask::ALL, 0, false, VecBrlyVCard::CRDBRLYRLY, xchg->getRefPreset(VecBrlyVPreset::PREBRLYOWNER, jref));
};
...
void PnlBrlyNavConnect::refreshCon(
	...
	dbsbrly->tblbrlyhistrmuseruniversal->loadRstByUsrCrd(xchg->getRefPreset(VecBrlyVPreset::PREBRLYOWNER, jref), VecBrlyVCard::CRDBRLYCON, false, rst);
...
void PnlBrlyNavConnect::refreshRly(
	...
	dbsbrly->tblbrlyhistrmuseruniversal->loadRstByUsrCrd(xchg->getRefPreset(VecBrlyVPreset::PREBRLYOWNER, jref), VecBrlyVCard::CRDBRLYRLY, false, rst);
	...
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYINIT) {
			...
			if (dpchappdo->ixVDo != 0) {
				...
				} else if (dpchappdo->ixVDo == VecVDo::BUTRLYNEWCRDCLICK) {
					handleDpchAppDoButRlyNewcrdClick(dbsbrly, &(req->dpcheng));
				};
...
void PnlBrlyNavConnect::handleDpchAppDoButRlyNewcrdClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	ubigint jrefNew = 0;
