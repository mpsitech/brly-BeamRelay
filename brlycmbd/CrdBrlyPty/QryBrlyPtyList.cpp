...
QryBrlyPtyList::QryBrlyPtyList(
	...
	xchg->addClstn(VecBrlyVCall::CALLBRLYPTYMOD, jref, Clstn::VecVJobmask::ALL, 0, false, Arg(), 0, Clstn::VecVJactype::LOCK);
...
void QryBrlyPtyList::refreshJnum() {
	...
	if (preRefSel == 0) xchg->removeClstns(VecBrlyVCall::CALLBRLYPTYUPD_REFEQ, jref);
	else xchg->addRefClstn(VecBrlyVCall::CALLBRLYPTYUPD_REFEQ, jref, Clstn::VecVJobmask::ALL, 0, true, preRefSel);
};
...
void QryBrlyPtyList::rerun_orderSQL(
	...
	if (preIxOrd == VecVOrd::TIT) sqlstr += " ORDER BY TblBrlyMPlntype.Title ASC";
	else if (preIxOrd == VecVOrd::SRF) sqlstr += " ORDER BY TblBrlyMPlntype.sref ASC";
};
...
bool QryBrlyPtyList::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
