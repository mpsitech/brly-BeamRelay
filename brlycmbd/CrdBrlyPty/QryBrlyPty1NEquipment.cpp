...
void QryBrlyPty1NEquipment::rerun(
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYEQPMOD_PTYEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefPty);
};
...
void QryBrlyPty1NEquipment::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYEQPMOD_PTYEQ) {
		call->abort = handleCallBrlyEqpMod_ptyEq(dbsbrly, call->jref);
...
bool QryBrlyPty1NEquipment::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
