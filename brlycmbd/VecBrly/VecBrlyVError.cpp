/**
	* \file VecBrlyVError.cpp
	* vector VecBrlyVError (implementation)
...
/******************************************************************************
 namespace VecBrlyVError
 ******************************************************************************/

uint VecBrlyVError::getIx(
			const string& sref
		) {
...
void VecBrlyVError::fillFeed(
			const uint ixBrlyVLocale
			, Feed& feed
		) {
	feed.clear();

	for (unsigned int i = 1; i <= 24; i++) feed.appendIxSrefTitles(i, getSref(i), getTitle(i, ixBrlyVLocale));
};
