...
void QryBrlySegOrgMNSegment::rerun(
	...
	sqlstr += " FROM TblBrlyMSegment, TblBrlyRMSegmentMSegment";
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYSEGRSEGMOD_OSGEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefSeg);
};
...
void QryBrlySegOrgMNSegment::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYSEGRSEGMOD_OSGEQ) {
		call->abort = handleCallBrlySegRsegMod_osgEq(dbsbrly, call->jref);
...
bool QryBrlySegOrgMNSegment::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
