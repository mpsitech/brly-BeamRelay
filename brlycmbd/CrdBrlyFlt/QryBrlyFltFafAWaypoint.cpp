...
void QryBrlyFltFafAWaypoint::rerun(
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYFAFAWPTMOD_FAFEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefFaf);
};
...
void QryBrlyFltFafAWaypoint::fetch(
	...
	if (stmgr) {
		...
		for (unsigned int i = 0; i < rst.nodes.size(); i++) {
			...
			rec->ftmTimestamp = Ftm::stamp(rec->timestamp);
		};
...
void QryBrlyFltFafAWaypoint::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYFAFAWPTMOD_FAFEQ) {
		call->abort = handleCallBrlyFafAwptMod_fafEq(dbsbrly, call->jref);
...
bool QryBrlyFltFafAWaypoint::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
