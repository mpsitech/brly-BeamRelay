...
void QryBrlyFltMNConnection::rerun(
	...
	sqlstr += " FROM TblBrlyMConnection, TblBrlyRMConnectionMFlight";
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYCONRFLTMOD_FLTEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefFlt);
};
...
void QryBrlyFltMNConnection::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYCONRFLTMOD_FLTEQ) {
		call->abort = handleCallBrlyConRfltMod_fltEq(dbsbrly, call->jref);
...
bool QryBrlyFltMNConnection::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
