...
uint CrdBrlyFlt::VecVSge::getIx(
	...
	if (s == "coldone") return COLDONE;
	if (s == "plot") return PLOT;
	if (s == "pltdone") return PLTDONE;
...
string CrdBrlyFlt::VecVSge::getSref(
	...
	if (ix == COLDONE) return("coldone");
	if (ix == PLOT) return("plot");
	if (ix == PLTDONE) return("pltdone");
...
CrdBrlyFlt::DpchAppDo::DpchAppDo() :
			DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYFLTDO)
		{
	ixVDo = 0;
};
