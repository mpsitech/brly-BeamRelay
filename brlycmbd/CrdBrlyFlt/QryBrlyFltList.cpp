...
QryBrlyFltList::QryBrlyFltList(
	...
	xchg->addClstn(VecBrlyVCall::CALLBRLYFLTMOD, jref, Clstn::VecVJobmask::ALL, 0, false, Arg(), 0, Clstn::VecVJactype::LOCK);
...
void QryBrlyFltList::refreshJnum() {
	...
	if (preRefSel == 0) xchg->removeClstns(VecBrlyVCall::CALLBRLYFLTUPD_REFEQ, jref);
	else xchg->addRefClstn(VecBrlyVCall::CALLBRLYFLTUPD_REFEQ, jref, Clstn::VecVJobmask::ALL, 0, true, preRefSel);
};
...
void QryBrlyFltList::rerun_orderSQL(
	...
	if (preIxOrd == VecVOrd::STO) sqlstr += " ORDER BY TblBrlyMFlight.stop ASC";
	...
	else if (preIxOrd == VecVOrd::SRF) sqlstr += " ORDER BY TblBrlyMFlight.sref ASC";
...
void QryBrlyFltList::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYFLTMOD) {
...
bool QryBrlyFltList::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
