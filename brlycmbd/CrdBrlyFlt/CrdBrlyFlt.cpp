...
CrdBrlyFlt::CrdBrlyFlt(
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbsbrly, VecBrlyVCall::CALLBRLYREFPRESET, jref, VecBrlyVPreset::PREBRLYREFFLT, ref);
...
uint CrdBrlyFlt::enterSgeCollect(
	...
	nextIxVSgeSuccess = VecVSge::COLDONE;
...
uint CrdBrlyFlt::enterSgePlot(
	...
	nextIxVSgeSuccess = VecVSge::PLTDONE;
	...
	if (vid) {
		...
		addInv(new DpchInvBrlyDnsmapFrame(0, 0, refBrlyMFile, fullsize, grayscale, altzmax, tIx));
	};
