...
void PnlBrlyFltRef1NSegment::handleDpchAppDoButViewClick(
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecBrlyVPreset::PREBRLYIXCRDACCSEG, jref)) {
			sref = "CrdBrlySeg";
			xchg->triggerIxRefSrefIntvalToRefCall(dbsbrly, VecBrlyVCall::CALLBRLYCRDOPEN, jref, 0, 0, sref, recSeg.ref, jrefNew);
		};
...
void PnlBrlyFltRef1NSegment::handleDpchAppDoButNewClick(
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecBrlyVPreset::PREBRLYIXCRDACCSEG, jref) & VecBrlyWAccess::EDIT) != 0) {
			sref = "CrdBrlySeg";
