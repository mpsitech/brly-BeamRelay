...
void QryBrlyFltOrgMNFlight::rerun(
	...
	sqlstr += " FROM TblBrlyMFlight, TblBrlyRMFlightMFlight";
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYFLTRFLTMOD_OFLEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefFlt);
};
...
void QryBrlyFltOrgMNFlight::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYFLTRFLTMOD_OFLEQ) {
		call->abort = handleCallBrlyFltRfltMod_oflEq(dbsbrly, call->jref);
...
bool QryBrlyFltOrgMNFlight::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
