...
void QryBrlyFltMNLocation::rerun(
	...
	sqlstr += " FROM TblBrlyMLocation, TblBrlyRMFlightMLocation";
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYFLTRLOCMOD_FLTEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefFlt);
};
...
void QryBrlyFltMNLocation::fetch(
	...
	if (stmgr) {
		...
		for (unsigned int i = 0; i < rst.nodes.size(); i++) {
			...
			rec->ftmX1VisStart = Ftm::stamp(rec->x1VisStart);
			rec->ftmX1VisStop = Ftm::stamp(rec->x1VisStop);
			rec->ftmXVisStart = Ftm::stamp(rec->xVisStart);
			rec->ftmXVisStop = Ftm::stamp(rec->xVisStop);
		};
...
void QryBrlyFltMNLocation::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYFLTRLOCMOD_FLTEQ) {
		call->abort = handleCallBrlyFltRlocMod_fltEq(dbsbrly, call->jref);
...
bool QryBrlyFltMNLocation::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
