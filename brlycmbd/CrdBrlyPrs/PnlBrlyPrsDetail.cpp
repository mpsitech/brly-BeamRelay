...
void PnlBrlyPrsDetail::refreshRecPrs(
	...
	if (recPrs.ref == 0) xchg->removeClstns(VecBrlyVCall::CALLBRLYPRSJLNMMOD_PRSEQ, jref);
	else xchg->addRefClstn(VecBrlyVCall::CALLBRLYPRSJLNMMOD_PRSEQ, jref, Clstn::VecVJobmask::ALL, 0, true, recPrs.ref);
...
void PnlBrlyPrsDetail::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYPRSJLNMMOD_PRSEQ) {
		call->abort = handleCallBrlyPrsJlnmMod_prsEq(dbsbrly, call->jref);
