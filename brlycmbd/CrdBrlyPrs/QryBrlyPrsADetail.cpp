...
void QryBrlyPrsADetail::rerun(
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYPRSADETMOD_PRSEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefPrs);
};
...
void QryBrlyPrsADetail::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYPRSADETMOD_PRSEQ) {
		call->abort = handleCallBrlyPrsAdetMod_prsEq(dbsbrly, call->jref);
...
bool QryBrlyPrsADetail::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
