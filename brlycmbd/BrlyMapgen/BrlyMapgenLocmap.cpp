	...
	for (int i = 0; i < height;i++) {
		...
		for (int j = 0; j < width; j++) {
			...
			if ((theta < mintheta) || (theta > maxtheta) || (phi < minphi) || (phi > maxphi)) {
				...
				ix = ((int) shrdat.etopo[thetaix*21601+phiix]) + 32768;
			};

			red[i*width+j] = shrdat.RvsZ[ix];
			green[i*width+j] = shrdat.GvsZ[ix];
			blue[i*width+j] = shrdat.BvsZ[ix];
		};
	};
	...
	for (unsigned int i = 0; i < qlegs.nodes.size();i++) {
		...
		if (coos.size() >= 2) {
			...
			if (coos.size() == 2) {
				...
				// possibilities: XSEC/BGNENDVIS/(any), (any)/BGNENDVIS/XSEC, (any)/BGNENDVIS/(any)
