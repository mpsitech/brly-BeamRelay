...
CrdBrlyCon::CrdBrlyCon(
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbsbrly, VecBrlyVCall::CALLBRLYREFPRESET, jref, VecBrlyVPreset::PREBRLYREFCON, ref);
...
void CrdBrlyCon::changeRef(
	...
	xchg->triggerIxRefToIxCall(dbsbrly, VecBrlyVCall::CALLBRLYRECACCESS, jref, VecBrlyVCard::CRDBRLYCON, ref, ixBrlyVRecaccess);
...
void CrdBrlyCon::handleTimerWithSrefMonInSgeCalc3(
	...
	wrefLast = xchg->addWakeup(jref, "mon", 250000, true);
	// IP handleTimerWithSrefMonInSgeCalc3 --- INSERT
};

void CrdBrlyCon::handleTimerWithSrefMonInSgeCalc4(
			DbsBrly* dbsbrly
		) {
	wrefLast = xchg->addWakeup(jref, "mon", 250000, true);
...
string CrdBrlyCon::getSquawk(
	...
	if ( (ixVSge == VecVSge::CORRGEN) || (ixVSge == VecVSge::CORRCALC) || (ixVSge == VecVSge::CALC1) || (ixVSge == VecVSge::CALC2) || (ixVSge == VecVSge::CALC3) || (ixVSge == VecVSge::CALC4) ) {
		if (ixBrlyVLocale == VecBrlyVLocale::ENUS) {
			...
			else if (ixVSge == VecVSge::CORRCALC) retval = "calculating corridor visibility (" + to_string(opNSuccess + opNFailure) + "/" + to_string(opN) + " operations completed, last: " + getOpsqkLast() + ")";
			else if (ixVSge == VecVSge::CALC1) retval = "determining relevant flights (" + to_string(opNSuccess + opNFailure) + "/" + to_string(opN) + " operations completed, last: " + getOpsqkLast() + ")";
			else if (ixVSge == VecVSge::CALC2) retval = "calculating flight visibility (" + to_string(opNSuccess + opNFailure) + "/" + to_string(opN) + " operations completed, last: " + getOpsqkLast() + ")";
			else if (ixVSge == VecVSge::CALC3) retval = "calculating equipment line-up (" + to_string(opNSuccess + opNFailure) + "/" + to_string(opN) + " operations completed, last: " + getOpsqkLast() + ")";
			else if (ixVSge == VecVSge::CALC4) retval = "calculating relays (" + to_string(opNSuccess + opNFailure) + "/" + to_string(opN) + " operations completed, last: " + getOpsqkLast() + ")";
		} else if (ixBrlyVLocale == VecBrlyVLocale::DECH) {
			if (ixVSge == VecVSge::CORRGEN) retval = "erzeuge Korridore (" + to_string(opN) + " Operationen vorbereitet)";
			else if (ixVSge == VecVSge::CORRCALC) retval = "berechne Sichtbarkeit von Korridoren (" + to_string(opNSuccess + opNFailure) + "/" + to_string(opN) + " Operationen ausgef\\u00fchrt, letzte: " + getOpsqkLast() + ")";
			else if (ixVSge == VecVSge::CALC1) retval = "bestimme relevante Fl\\u00fcge (" + to_string(opNSuccess + opNFailure) + "/" + to_string(opN) + " Operationen ausgef\\u00fchrt, letzte: " + getOpsqkLast() + ")";
			else if (ixVSge == VecVSge::CALC2) retval = "berechne Sichtbarkeit von Fl\\u00fcgen (" + to_string(opNSuccess + opNFailure) + "/" + to_string(opN) + " Operationen ausgef\\u00fchrt, letzte: " + getOpsqkLast() + ")";
			else if (ixVSge == VecVSge::CALC3) retval = "berechne Aufreihung von Ger\\u00e4t (" + to_string(opNSuccess + opNFailure) + "/" + to_string(opN) + " Operationen ausgef\\u00fchrt, letzte: " + getOpsqkLast() + ")";
			else if (ixVSge == VecVSge::CALC4) retval = "berechne Verbindungsketten (" + to_string(opNSuccess + opNFailure) + "/" + to_string(opN) + " Operationen ausgef\\u00fchrt, letzte: " + getOpsqkLast() + ")";
		};
...
uint CrdBrlyCon::enterSgeCrgidle(
	...
	nextIxVSgeSuccess = VecVSge::CORRGEN;
...
uint CrdBrlyCon::enterSgeCorrgen(
	...
	uint retval;
	nextIxVSgeSuccess = VecVSge::CORRCALC;
...
uint CrdBrlyCon::enterSgeCorrcalc(
	...
	nextIxVSgeSuccess = VecVSge::CRGDONE;
	...
	for (unsigned int i = 0; i < corrs.nodes.size();i++) {
		...
		if ( (refs.size() == 256) || (i == (corrs.nodes.size()-1)) ) {
			// leg-loc
			addInv(new DpchInvBrlyBaseLegloc(0, 0, 0, 0, refs, {}, true, false));

			// leg-leg
			addInv(new DpchInvBrlyBaseLegleg(0, 0, 0, 0, refs, true, false));
			addInv(new DpchInvBrlyBaseLegleg(0, 0, 0, 0, refs, false, true));
...
uint CrdBrlyCon::enterSgeCongen(
	...
	uint retval;
	nextIxVSgeSuccess = VecVSge::CNGDONE;
	retval = nextIxVSgeSuccess;
...
uint CrdBrlyCon::enterSgeCalc1(
	...
	nextIxVSgeSuccess = VecVSge::CALC2;
...
uint CrdBrlyCon::enterSgeCalc2(
	...
	nextIxVSgeSuccess = VecVSge::CALC3;
...
uint CrdBrlyCon::enterSgeCalc3(
	...
	nextIxVSgeSuccess = VecVSge::CALC4;
