...
void QryBrlyConMNFlight::rerun(
	...
	sqlstr += " FROM TblBrlyMFlight, TblBrlyRMConnectionMFlight";
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYCONRFLTMOD_CONEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefCon);
};
...
void QryBrlyConMNFlight::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYCONRFLTMOD_CONEQ) {
		call->abort = handleCallBrlyConRfltMod_conEq(dbsbrly, call->jref);
...
bool QryBrlyConMNFlight::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
