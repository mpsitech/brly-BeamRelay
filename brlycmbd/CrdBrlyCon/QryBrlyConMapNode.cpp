...
void QryBrlyConMapNode::rerun(
	...
	sqlstr += " FROM TblBrlyMRelay, TblBrlyMNode";
	sqlstr += " WHERE TblBrlyMNode.refBrlyMRelay = TblBrlyMRelay.ref";
	sqlstr += " AND TblBrlyMRelay.refBrlyMConnection = " + to_string(preRefCon) + "";
...
bool QryBrlyConMapNode::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
