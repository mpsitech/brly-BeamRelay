...
void QryBrlyConConMNEquipment::rerun(
	...
	sqlstr += " AND x1Start <= " + to_string(preTime) + "";
	...
	sqlstr += " FROM TblBrlyMEquipment, TblBrlyLRMConnectionMEquipment, TblBrlyQSelect";
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYCONREQPMOD_CONEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefCon);
};
...
void QryBrlyConConMNEquipment::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYCONREQPMOD_CONEQ) {
		call->abort = handleCallBrlyConReqpMod_conEq(dbsbrly, call->jref);
...
bool QryBrlyConConMNEquipment::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
