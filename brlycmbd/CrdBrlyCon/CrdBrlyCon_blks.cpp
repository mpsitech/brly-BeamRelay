...
uint CrdBrlyCon::VecVSge::getIx(
	...
	if (s == "crgidle") return CRGIDLE;
	if (s == "corrgen") return CORRGEN;
	if (s == "corrcalc") return CORRCALC;
	if (s == "crgdone") return CRGDONE;
	if (s == "congen") return CONGEN;
	if (s == "cngdone") return CNGDONE;
...
string CrdBrlyCon::VecVSge::getSref(
	...
	if (ix == CRGIDLE) return("crgidle");
	if (ix == CORRGEN) return("corrgen");
	if (ix == CORRCALC) return("corrcalc");
	if (ix == CRGDONE) return("crgdone");
	if (ix == CONGEN) return("congen");
	if (ix == CNGDONE) return("cngdone");
...
void CrdBrlyCon::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
...
void CrdBrlyCon::StatApp::writeJSON(
			Json::Value& sup
			, string difftag
...
void CrdBrlyCon::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
...
CrdBrlyCon::DpchAppDo::DpchAppDo() :
			DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYCONDO)
		{
	ixVDo = 0;
};
...
void CrdBrlyCon::DpchAppDo::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppBrlyConDo"];}();
...
void CrdBrlyCon::DpchEngData::writeJSON(
			const uint ixBrlyVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngBrlyConData"] = Json::Value(Json::objectValue);
