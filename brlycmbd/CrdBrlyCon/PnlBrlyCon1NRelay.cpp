...
void PnlBrlyCon1NRelay::handleDpchAppDoButViewClick(
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecBrlyVPreset::PREBRLYIXCRDACCRLY, jref)) {
			sref = "CrdBrlyRly";
			xchg->triggerIxRefSrefIntvalToRefCall(dbsbrly, VecBrlyVCall::CALLBRLYCRDOPEN, jref, 0, 0, sref, recRly.ref, jrefNew);
		};
...
void PnlBrlyCon1NRelay::handleDpchAppDoButNewClick(
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecBrlyVPreset::PREBRLYIXCRDACCRLY, jref) & VecBrlyWAccess::EDIT) != 0) {
			sref = "CrdBrlyRly";
