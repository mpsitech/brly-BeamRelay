...
void QryBrlyConMNSegment::rerun(
	...
	sqlstr += " FROM TblBrlyMSegment, TblBrlyRMConnectionMSegment";
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYCONRSEGMOD_CONEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefCon);
};
...
void QryBrlyConMNSegment::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYCONRSEGMOD_CONEQ) {
		call->abort = handleCallBrlyConRsegMod_conEq(dbsbrly, call->jref);
...
bool QryBrlyConMNSegment::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
