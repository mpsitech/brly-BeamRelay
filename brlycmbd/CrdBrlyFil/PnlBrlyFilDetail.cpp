...
PnlBrlyFilDetail::PnlBrlyFilDetail(
	...
	xchg->addIxClstn(VecBrlyVCall::CALLBRLYKLSAKEYMOD_KLSEQ, jref, Clstn::VecVJobmask::ALL, 0, false, VecBrlyVKeylist::KLSTBRLYKMFILECONTENT);
	xchg->addIxClstn(VecBrlyVCall::CALLBRLYKLSAKEYMOD_KLSEQ, jref, Clstn::VecVJobmask::ALL, 0, false, VecBrlyVKeylist::KLSTBRLYKMFILEMIMETYPE);
...
void PnlBrlyFilDetail::refreshRecFil(
	...
	if (recFil.refBrlyCFile == 0) xchg->removeClstns(VecBrlyVCall::CALLBRLYFILMOD_CLUEQ, jref);
	else xchg->addRefClstn(VecBrlyVCall::CALLBRLYFILMOD_CLUEQ, jref, Clstn::VecVJobmask::ALL, 0, true, recFil.refBrlyCFile);
...
void PnlBrlyFilDetail::handleDpchAppDoButReuViewClick(
	...
	if (statshr.ButReuViewAvail && statshr.ButReuViewActive) {
		if (xchg->getIxPreset(VecBrlyVPreset::PREBRLYIXCRDACCTTB, jref)) if (recFil.refIxVTbl == VecBrlyVMFileRefTbl::TTB) {
			sref = "CrdBrlyTtb";
...
void PnlBrlyFilDetail::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYFILMOD_CLUEQ) {
		call->abort = handleCallBrlyFilMod_cluEq(dbsbrly, call->jref);
