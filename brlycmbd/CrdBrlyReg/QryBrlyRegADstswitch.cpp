...
void QryBrlyRegADstswitch::rerun(
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYREGADSSMOD_REGEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefReg);
};
...
void QryBrlyRegADstswitch::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYREGADSSMOD_REGEQ) {
		call->abort = handleCallBrlyRegAdssMod_regEq(dbsbrly, call->jref);
...
bool QryBrlyRegADstswitch::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
