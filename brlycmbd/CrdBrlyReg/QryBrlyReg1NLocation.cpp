...
void QryBrlyReg1NLocation::rerun(
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYLOCMOD_REGEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefReg);
};
...
void QryBrlyReg1NLocation::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYLOCMOD_REGEQ) {
		call->abort = handleCallBrlyLocMod_regEq(dbsbrly, call->jref);
...
bool QryBrlyReg1NLocation::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
