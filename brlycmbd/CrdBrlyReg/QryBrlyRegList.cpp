...
QryBrlyRegList::QryBrlyRegList(
	...
	xchg->addClstn(VecBrlyVCall::CALLBRLYREGMOD, jref, Clstn::VecVJobmask::ALL, 0, false, Arg(), 0, Clstn::VecVJactype::LOCK);
...
void QryBrlyRegList::refreshJnum() {
	...
	if (preRefSel == 0) xchg->removeClstns(VecBrlyVCall::CALLBRLYREGUPD_REFEQ, jref);
	else xchg->addRefClstn(VecBrlyVCall::CALLBRLYREGUPD_REFEQ, jref, Clstn::VecVJobmask::ALL, 0, true, preRefSel);
};
...
void QryBrlyRegList::rerun_orderSQL(
	...
	if (preIxOrd == VecVOrd::SUP) sqlstr += " ORDER BY TblBrlyMRegion.supRefBrlyMRegion ASC";
	else if (preIxOrd == VecVOrd::TIT) sqlstr += " ORDER BY TblBrlyMRegion.Title ASC";
	else if (preIxOrd == VecVOrd::SRF) sqlstr += " ORDER BY TblBrlyMRegion.sref ASC";
};
...
bool QryBrlyRegList::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
