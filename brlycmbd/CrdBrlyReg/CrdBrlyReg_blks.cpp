...
void CrdBrlyReg::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
...
void CrdBrlyReg::StatApp::writeJSON(
			Json::Value& sup
			, string difftag
...
void CrdBrlyReg::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
...
CrdBrlyReg::DpchAppDo::DpchAppDo() :
			DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYREGDO)
		{
	ixVDo = 0;
};
...
void CrdBrlyReg::DpchAppDo::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppBrlyRegDo"];}();
...
void CrdBrlyReg::DpchEngData::writeJSON(
			const uint ixBrlyVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngBrlyRegData"] = Json::Value(Json::objectValue);
