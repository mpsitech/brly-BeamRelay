...
if (!good) cout << "failure with slice in time" << endl;
	...
	if (good) {
		...
		if (fullsize) {
			...
			filename = writePng(xchg->tmppath, imgdat, fullwidth, fullheight);
		} else {
			preparePng(dns, spczmax, altzmax, width, height, width, height, 0, 0, grayscale, &imgdat);
			filename = writePng(xchg->tmppath, imgdat, width, height);
		};
	};
	...
	return(new DpchRetBrlyDnsmapFrame("", "", ixOpVOpres, 100, filename));
};
