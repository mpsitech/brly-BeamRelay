...
Brlycmbd::Brlycmbd(
	...
	cout << "\tstarting " << xchg->stgbrlycmbd.jobprcn << " job processor threads ... {" << flush;

	for (unsigned int i = 0; i < xchg->stgbrlycmbd.jobprcn; i++) {
		if (i != 0) cout << ", " << flush;

		xchg->cStable.lockMutex("Brlycmbd", "Brlycmbd[1]");

		jobprcs.push_back(0);
		pthread_create(&(jobprcs[i]), &attr, &BrlycmbdJobprc::run, (void*) xchg);

		xchg->cStable.wait();
		xchg->cStable.unlockMutex("Brlycmbd", "Brlycmbd[1]");
	};

	cout << "} success" << endl;

	// 8. start op processor threads
	cout << "\tstarting " << xchg->stgbrlycmbd.opprcn << " operation processor threads ... {" << flush;

	for (unsigned int i = 0; i < xchg->stgbrlycmbd.opprcn; i++) {
		if (i != 0) cout << ", " << flush;

		xchg->cStable.lockMutex("Brlycmbd", "Brlycmbd[2]");

		opprcs.push_back(0);
		pthread_create(&(opprcs[i]), &attr, &BrlycmbdOpprc::run, (void*) xchg);

		xchg->cStable.wait();
		xchg->cStable.unlockMutex("Brlycmbd", "Brlycmbd[2]");
	};

	cout << "} success" << endl;

	// 9. start web server
	if (xchg->stgbrlycmbd.appsrv) {
...
Brlycmbd::~Brlycmbd() {
	...
	// 3. stop web server
	if (xchg->stgbrlycmbd.appsrv) BrlycmbdAppsrv::stop(appsrv);
...
void Brlycmbd::loadPref() {
	...
	parseFile(xchg->exedir + "/PrefBrlycmbd.xml", &doc, &docctx);

	if (checkUclcXPaths(docctx, basexpath, "/", "PrefBrlycmbd")) {
		xchg->stgbrlyappearance.readXML(docctx, basexpath, true);
		xchg->stgbrlyappsrv.readXML(docctx, basexpath, true);
		xchg->stgbrlycmbd.readXML(docctx, basexpath, true);
