...
QryBrlyOprList::QryBrlyOprList(
	...
	xchg->addClstn(VecBrlyVCall::CALLBRLYOPRMOD, jref, Clstn::VecVJobmask::ALL, 0, false, Arg(), 0, Clstn::VecVJactype::LOCK);
...
void QryBrlyOprList::refreshJnum() {
	...
	if (preRefSel == 0) xchg->removeClstns(VecBrlyVCall::CALLBRLYOPRUPD_REFEQ, jref);
	else xchg->addRefClstn(VecBrlyVCall::CALLBRLYOPRUPD_REFEQ, jref, Clstn::VecVJobmask::ALL, 0, true, preRefSel);
};
...
void QryBrlyOprList::rerun_orderSQL(
	...
	if (preIxOrd == VecVOrd::TIT) sqlstr += " ORDER BY TblBrlyMOperator.Title ASC";
	else if (preIxOrd == VecVOrd::ICA) sqlstr += " ORDER BY TblBrlyMOperator.srefICAO ASC";
	else if (preIxOrd == VecVOrd::SRF) sqlstr += " ORDER BY TblBrlyMOperator.sref ASC";
};
...
void QryBrlyOprList::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYOPRUPD_REFEQ) {
		call->abort = handleCallBrlyOprUpd_refEq(dbsbrly, call->jref);
...
bool QryBrlyOprList::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
