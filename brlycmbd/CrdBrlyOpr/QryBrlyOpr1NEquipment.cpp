...
void QryBrlyOpr1NEquipment::rerun(
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYEQPMOD_OPREQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefOpr);
};
...
void QryBrlyOpr1NEquipment::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYEQPMOD_OPREQ) {
		call->abort = handleCallBrlyEqpMod_oprEq(dbsbrly, call->jref);
...
bool QryBrlyOpr1NEquipment::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
