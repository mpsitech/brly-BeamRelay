...
void PnlBrlyUsrDetail::refreshRecUsr(
	...
	if (recUsr.ref == 0) xchg->removeClstns(VecBrlyVCall::CALLBRLYUSRJSTEMOD_USREQ, jref);
	else xchg->addRefClstn(VecBrlyVCall::CALLBRLYUSRJSTEMOD_USREQ, jref, Clstn::VecVJobmask::ALL, 0, true, recUsr.ref);
	...
	statshr.TxfFkyActive = evalTxfFkyActive(dbsbrly);
...
void PnlBrlyUsrDetail::handleDpchAppDoButPrsViewClick(
	...
	if (statshr.ButPrsViewAvail && statshr.ButPrsViewActive) {
		if (xchg->getIxPreset(VecBrlyVPreset::PREBRLYIXCRDACCPRS, jref)) {
			sref = "CrdBrlyPrs";
...
void PnlBrlyUsrDetail::handleDpchAppDoButUsgViewClick(
	...
	if (statshr.ButUsgViewAvail && statshr.ButUsgViewActive) {
		if (xchg->getIxPreset(VecBrlyVPreset::PREBRLYIXCRDACCUSG, jref)) {
			sref = "CrdBrlyUsg";
