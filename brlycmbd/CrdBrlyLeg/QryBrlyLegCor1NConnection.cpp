...
void QryBrlyLegCor1NConnection::rerun(
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYCONMOD_COREQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefLeg);
};
...
void QryBrlyLegCor1NConnection::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYCONMOD_COREQ) {
		call->abort = handleCallBrlyConMod_corEq(dbsbrly, call->jref);
...
bool QryBrlyLegCor1NConnection::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
