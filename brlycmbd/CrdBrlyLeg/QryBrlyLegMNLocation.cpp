...
void QryBrlyLegMNLocation::rerun(
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYLEGRLOCMOD_LEGEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefLeg);
};
...
void QryBrlyLegMNLocation::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYLEGRLOCMOD_LEGEQ) {
		call->abort = handleCallBrlyLegRlocMod_legEq(dbsbrly, call->jref);
...
bool QryBrlyLegMNLocation::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
