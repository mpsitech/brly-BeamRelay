...
void DlgBrlyLegNew::handleRequest(
	...
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		if ((req->sref == "mon") && (ixVSge == VecVSge::INCO)) handleTimerWithSrefMonInSgeInco(dbsbrly);
	};
};
...
void DlgBrlyLegNew::handleTimerWithSrefMonInSgeInco(
			DbsBrly* dbsbrly
		) {
	wrefLast = xchg->addWakeup(jref, "mon", 250000, true);
	// IP handleTimerWithSrefMonInSgeInco --- INSERT
};
...
string DlgBrlyLegNew::getSquawk(
	...
	if ( (ixVSge == VecVSge::CREDONE) || (ixVSge == VecVSge::INCO) ) {
		if (ixBrlyVLocale == VecBrlyVLocale::ENUS) {
			...
			else if (ixVSge == VecVSge::INCO) retval = "incorporating leg (" + to_string(opNSuccess + opNFailure) + "/" + to_string(opN) + " operations completed)";
		} else if (ixBrlyVLocale == VecBrlyVLocale::DECH) {
			if (ixVSge == VecVSge::CREDONE) retval = "neue Strecke erstellt";
			else if (ixVSge == VecVSge::INCO) retval = "gliedere Strecke ein (" + to_string(opNSuccess + opNFailure) + "/" + to_string(opN) + " Operationen ausgef\\u00fchrt)";
		};
...
uint DlgBrlyLegNew::enterSgeInco(
	...
	// leg-loc
	addInv(new DpchInvBrlyBaseLegloc(0, 0, 0, 0, {refBrlyMLeg}, {}, true, false));

	// leg-leg
	addInv(new DpchInvBrlyBaseLegleg(0, 0, 0, 0, {refBrlyMLeg}, true, false));
	addInv(new DpchInvBrlyBaseLegleg(0, 0, 0, 0, {refBrlyMLeg}, false, true));
