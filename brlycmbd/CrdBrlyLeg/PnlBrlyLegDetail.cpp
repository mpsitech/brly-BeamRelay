...
void PnlBrlyLegDetail::refreshRecLeg(
	...
	if (recLeg.refBrlyCLeg == 0) xchg->removeClstns(VecBrlyVCall::CALLBRLYLEGMOD_CLUEQ, jref);
	else xchg->addRefClstn(VecBrlyVCall::CALLBRLYLEGMOD_CLUEQ, jref, Clstn::VecVJobmask::ALL, 0, true, recLeg.refBrlyCLeg);
...
void PnlBrlyLegDetail::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYLEGMOD_CLUEQ) {
		call->abort = handleCallBrlyLegMod_cluEq(dbsbrly, call->jref);
