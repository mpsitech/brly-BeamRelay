...
void QryBrlyLegOrgMNLeg::rerun(
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYLEGRLEGMOD_OLGEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefLeg);
};
...
void QryBrlyLegOrgMNLeg::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYLEGRLEGMOD_OLGEQ) {
		call->abort = handleCallBrlyLegRlegMod_olgEq(dbsbrly, call->jref);
...
bool QryBrlyLegOrgMNLeg::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
