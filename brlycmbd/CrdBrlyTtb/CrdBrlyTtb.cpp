...
CrdBrlyTtb::CrdBrlyTtb(
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbsbrly, VecBrlyVCall::CALLBRLYREFPRESET, jref, VecBrlyVPreset::PREBRLYREFTTB, ref);

	// initialize according to ref
	changeRef(dbsbrly, jref, ((ref + 1) == 0) ? 0 : ref, false);

	iex = new JobBrlyIexFlt(xchg, dbsbrly, jref, ixBrlyVLocale);
...
void CrdBrlyTtb::handleTimerWithSrefMonInSgeSegment(
	...
	wrefLast = xchg->addWakeup(jref, "mon", 250000, true);
	// IP handleTimerWithSrefMonInSgeSegment --- INSERT
};
...
string CrdBrlyTtb::getSquawk(
	...
	if ( (ixVSge == VecVSge::IMPORT) || (ixVSge == VecVSge::SEGMENT) ) {
		if (ixBrlyVLocale == VecBrlyVLocale::DECH) {
			...
			else if (ixVSge == VecVSge::SEGMENT) str << "segmentiere Fl\\u00fcge (" << opNSuccess << "/" << opN << " Operationen ausgef\\u00fchrt, letzte: " << getOpsqkLast() << ")";
		} else if (ixBrlyVLocale == VecBrlyVLocale::ENUS) {
			if (ixVSge == VecVSge::IMPORT) str << "importing FlightAware data (" << fltcnt << " flights added)";
			else if (ixVSge == VecVSge::SEGMENT) str << "segmenting flights (" << opNSuccess << "/" << opN << " operations completed, last: " << getOpsqkLast() << ")";
		};
		retval = str.str();
...
uint CrdBrlyTtb::enterSgeImport(
	...
	iex->reset(dbsbrly);
	iex->parseFromFile(dbsbrly, infolder + "/IexBrlyFlt.txt", false);
	iex->import(dbsbrly);
	...
	for (unsigned int i = 0; i < folders.size();i++) {
		...
		iex->parseFromFile(dbsbrly, infolder + "/" + folders[i] + "/IexBrlyFlt.txt", false);

		success = (iex->ixVSge == JobBrlyIexFlt::VecVSge::PRSDONE);
...
uint CrdBrlyTtb::enterSgeSegment(
	...
	nextIxVSgeSuccess = VecVSge::SEGDONE;
