...
void QryBrlyTtbRef1NFile::rerun(
	...
	xchg->addIxRefClstn(VecBrlyVCall::CALLBRLYFILMOD_RETREUEQ, jref, Clstn::VecVJobmask::ALL, 0, false, VecBrlyVMFileRefTbl::TTB, preRefTtb);
};
...
void QryBrlyTtbRef1NFile::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYFILMOD_RETREUEQ) {
		call->abort = handleCallBrlyFilMod_retReuEq(dbsbrly, call->jref);
...
bool QryBrlyTtbRef1NFile::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
