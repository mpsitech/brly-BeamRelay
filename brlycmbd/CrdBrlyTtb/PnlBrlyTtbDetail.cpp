...
PnlBrlyTtbDetail::PnlBrlyTtbDetail(
	...
	xchg->addClstn(VecBrlyVCall::CALLBRLYTTB_FILEQ, jref, Clstn::VecVJobmask::TREE, 0, false, Arg(), 0, Clstn::VecVJactype::LOCK);

	// IP constructor.cust3 --- INSERT

	xchg->addIxClstn(VecBrlyVCall::CALLBRLYKLSAKEYMOD_KLSEQ, jref, Clstn::VecVJobmask::ALL, 0, false, VecBrlyVKeylist::KLSTBRLYKALLIANCE);
...
void PnlBrlyTtbDetail::handleDpchAppDoButFilViewClick(
	...
	if (statshr.ButFilViewAvail && statshr.ButFilViewActive) {
		if (xchg->getIxPreset(VecBrlyVPreset::PREBRLYIXCRDACCFIL, jref)) {
			sref = "CrdBrlyFil";
...
void PnlBrlyTtbDetail::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYKLSAKEYMOD_KLSEQ) {
	...
	} else if (call->ixVCall == VecBrlyVCall::CALLBRLYTTB_FILEQ) {
...
bool PnlBrlyTtbDetail::handleCallBrlyTtb_filEq(
	...
	bool retval = false;
	boolvalRet = (recTtb.refBrlyMFile == refInv); // IP handleCallBrlyTtb_filEq --- LINE
	return retval;
};
