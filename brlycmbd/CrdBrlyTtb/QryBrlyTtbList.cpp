...
QryBrlyTtbList::QryBrlyTtbList(
	...
	xchg->addClstn(VecBrlyVCall::CALLBRLYTTBMOD, jref, Clstn::VecVJobmask::ALL, 0, false, Arg(), 0, Clstn::VecVJactype::LOCK);
...
void QryBrlyTtbList::refreshJnum() {
	...
	if (preRefSel == 0) xchg->removeClstns(VecBrlyVCall::CALLBRLYTTBUPD_REFEQ, jref);
	else xchg->addRefClstn(VecBrlyVCall::CALLBRLYTTBUPD_REFEQ, jref, Clstn::VecVJobmask::ALL, 0, true, preRefSel);
};
...
void QryBrlyTtbList::rerun_orderSQL(
	...
	if (preIxOrd == VecVOrd::FIL) sqlstr += " ORDER BY TblBrlyMTimetable.refBrlyMFile ASC";
	else if (preIxOrd == VecVOrd::TIT) sqlstr += " ORDER BY TblBrlyMTimetable.Title ASC";
};
...
bool QryBrlyTtbList::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
