...
void QryBrlyLocVisOrgMNLocation::rerun(
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYVLOCRLOCMOD_OLOEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefLoc);
};
...
void QryBrlyLocVisOrgMNLocation::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYVLOCRLOCMOD_OLOEQ) {
		call->abort = handleCallBrlyVlocRlocMod_oloEq(dbsbrly, call->jref);
...
bool QryBrlyLocVisOrgMNLocation::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
