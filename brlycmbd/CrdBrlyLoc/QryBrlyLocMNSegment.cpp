...
void QryBrlyLocMNSegment::rerun(
	...
	sqlstr += " FROM TblBrlyMSegment, TblBrlyRMLocationMSegment";
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYLOCRSEGMOD_LOCEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefLoc);
};
...
void QryBrlyLocMNSegment::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYLOCRSEGMOD_LOCEQ) {
		call->abort = handleCallBrlyLocRsegMod_locEq(dbsbrly, call->jref);
...
bool QryBrlyLocMNSegment::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
