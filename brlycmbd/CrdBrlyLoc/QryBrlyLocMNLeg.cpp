...
void QryBrlyLocMNLeg::rerun(
	...
	sqlstr += " FROM TblBrlyMLeg, TblBrlyRMLegMLocation";
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYLEGRLOCMOD_LOCEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefLoc);
};
...
void QryBrlyLocMNLeg::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYLEGRLOCMOD_LOCEQ) {
		call->abort = handleCallBrlyLegRlocMod_locEq(dbsbrly, call->jref);
...
bool QryBrlyLocMNLeg::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
