...
void QryBrlyLocBgn1NLeg::rerun(
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYLEGMOD_BLOEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefLoc);
};
...
void QryBrlyLocBgn1NLeg::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYLEGMOD_BLOEQ) {
		call->abort = handleCallBrlyLegMod_bloEq(dbsbrly, call->jref);
...
bool QryBrlyLocBgn1NLeg::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
