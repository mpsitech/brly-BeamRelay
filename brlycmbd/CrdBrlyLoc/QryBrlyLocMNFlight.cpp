...
void QryBrlyLocMNFlight::rerun(
	...
	sqlstr += " FROM TblBrlyMFlight, TblBrlyRMFlightMLocation";
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYFLTRLOCMOD_LOCEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefLoc);
};
...
void QryBrlyLocMNFlight::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYFLTRLOCMOD_LOCEQ) {
		call->abort = handleCallBrlyFltRlocMod_locEq(dbsbrly, call->jref);
...
bool QryBrlyLocMNFlight::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
