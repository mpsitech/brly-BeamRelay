...
void QryBrlyLocEqpMNConnection::rerun(
	...
	sqlstr += " FROM TblBrlyMConnection, TblBrlyLRMConnectionMEquipment";
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYCONREQPMOD_EQPEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefEqp);
};
...
void QryBrlyLocEqpMNConnection::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYCONREQPMOD_EQPEQ) {
		call->abort = handleCallBrlyConReqpMod_eqpEq(dbsbrly, call->jref);
...
bool QryBrlyLocEqpMNConnection::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
