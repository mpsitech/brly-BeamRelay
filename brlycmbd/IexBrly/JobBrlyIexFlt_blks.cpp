/**
	* \file JobBrlyIexFlt_blks.cpp
	* job handler for job JobBrlyIexFlt (implementation of blocks)
...
/******************************************************************************
 class JobBrlyIexFlt::VecVSge
 ******************************************************************************/

uint JobBrlyIexFlt::VecVSge::getIx(
			const string& sref
		) {
...
string JobBrlyIexFlt::VecVSge::getSref(
			const uint ix
		) {
...
void JobBrlyIexFlt::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
