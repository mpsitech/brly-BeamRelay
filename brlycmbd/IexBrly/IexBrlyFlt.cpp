/**
	* \file IexBrlyFlt.cpp
	* data blocks and readers/writers for import/export complex IexBrlyFlt (implementation)
...
uint IexBrlyFlt::VecVIme::getIx(
	...
	if (s == "imeiamfaflightwaypoint") return IMEIAMFAFLIGHTWAYPOINT;
...
string IexBrlyFlt::VecVIme::getSref(
	...
	if (ix == IMEIAMFAFLIGHTWAYPOINT) return("ImeIAMFaflightWaypoint");
...
void IexBrlyFlt::ImeitemIAMFaflightWaypoint::readTxt(
			Txtrd& txtrd
		) {
	lineno = txtrd.linecnt;

	if (txtrd.fields.size() > 0) {timestamp = atol(txtrd.fields[0].c_str()); ixWIelValid += ImeIAMFaflightWaypoint::VecWIel::TIMESTAMP;};
	if (txtrd.fields.size() > 1) {latitude = atof(txtrd.fields[1].c_str()); ixWIelValid += ImeIAMFaflightWaypoint::VecWIel::LATITUDE;};
	if (txtrd.fields.size() > 2) {longitude = atof(txtrd.fields[2].c_str()); ixWIelValid += ImeIAMFaflightWaypoint::VecWIel::LONGITUDE;};
	if (txtrd.fields.size() > 3) {groundspeed = atoi(txtrd.fields[3].c_str()); ixWIelValid += ImeIAMFaflightWaypoint::VecWIel::GROUNDSPEED;};
	if (txtrd.fields.size() > 4) {altitude = atoi(txtrd.fields[4].c_str()); ixWIelValid += ImeIAMFaflightWaypoint::VecWIel::ALTITUDE;};
	if (txtrd.fields.size() > 5) {altitudeStatus = txtrd.fields[5]; ixWIelValid += ImeIAMFaflightWaypoint::VecWIel::ALTITUDESTATUS;};
	if (txtrd.fields.size() > 6) {updateType = txtrd.fields[6]; ixWIelValid += ImeIAMFaflightWaypoint::VecWIel::UPDATETYPE;};
	if (txtrd.fields.size() > 7) {altitudeChange = txtrd.fields[7]; ixWIelValid += ImeIAMFaflightWaypoint::VecWIel::ALTITUDECHANGE;};
	...
	while (txtrd.readLine()) {
		switch (txtrd.ixVLinetype) {
			...
			default:
				throw SbeException(SbeException::TXTRD_CONTENT, {{"ime","ImeIAMFaflightWaypoint"}, {"lineno",to_string(lineno)}});
		};
	};
};
...
void IexBrlyFlt::ImeIAMFaflightWaypoint::readTxt(
			Txtrd& txtrd
		) {
		...
		switch (txtrd.ixVLinetype) {
			case Txtrd::VecVLinetype::DATA:
				if (txtrd.il == 1) {
					...
					throw SbeException(SbeException::TXTRD_ENDTKN, {{"ime","ImeIAMFaflightWaypoint"}, {"lineno",to_string(txtrd.linecnt)}});

				} else throw SbeException(SbeException::TXTRD_CONTENT, {{"ime","ImeIAMFaflightWaypoint"}, {"lineno",to_string(txtrd.linecnt)}});

			case Txtrd::VecVLinetype::FOOTER:
				if (txtrd.ixVToken == VecVIme::IMEIAMFAFLIGHTWAYPOINT) return;
	...
	if (txtrd.eof()) throw SbeException(SbeException::TXTRD_ENDTKN, {{"ime","ImeIAMFaflightWaypoint"}, {"lineno",to_string(txtrd.linecnt)}});
};
...
void IexBrlyFlt::ImeIAMFaflightWaypoint::writeTxt(
	...
	if (nodes.size() > 0) {
		...
		outfile << "\tImeIAMFaflightWaypoint.end" << endl;
	};
};
...
void IexBrlyFlt::ImeitemIMFaflight::readTxt(
			Txtrd& txtrd
		) {
	lineno = txtrd.linecnt;

	if (txtrd.fields.size() > 0) {srefIxBrlyVIop = txtrd.fields[0]; ixWIelValid += ImeIMFaflight::VecWIel::SREFIXBRLYVIOP;};
	if (txtrd.fields.size() > 1) {iLeg = txtrd.fields[1]; ixWIelValid += ImeIMFaflight::VecWIel::ILEG;};
	if (txtrd.fields.size() > 2) {hintRefBrlyMFlight = txtrd.fields[2]; ixWIelValid += ImeIMFaflight::VecWIel::HINTREFBRLYMFLIGHT;};
	if (txtrd.fields.size() > 3) {faFlightID = txtrd.fields[3]; ixWIelValid += ImeIMFaflight::VecWIel::FAFLIGHTID;};
	if (txtrd.fields.size() > 4) {ident = txtrd.fields[4]; ixWIelValid += ImeIMFaflight::VecWIel::IDENT;};
	if (txtrd.fields.size() > 5) {aircrafttype = txtrd.fields[5]; ixWIelValid += ImeIMFaflight::VecWIel::AIRCRAFTTYPE;};
	if (txtrd.fields.size() > 6) {filed_ete = txtrd.fields[6]; ixWIelValid += ImeIMFaflight::VecWIel::FILED_ETE;};
	if (txtrd.fields.size() > 7) {filed_time = atol(txtrd.fields[7].c_str()); ixWIelValid += ImeIMFaflight::VecWIel::FILED_TIME;};
	if (txtrd.fields.size() > 8) {filed_departuretime = atol(txtrd.fields[8].c_str()); ixWIelValid += ImeIMFaflight::VecWIel::FILED_DEPARTURETIME;};
	if (txtrd.fields.size() > 9) {filed_airspeed_kts = atoi(txtrd.fields[9].c_str()); ixWIelValid += ImeIMFaflight::VecWIel::FILED_AIRSPEED_KTS;};
	if (txtrd.fields.size() > 10) {filed_airspeed_mach = atof(txtrd.fields[10].c_str()); ixWIelValid += ImeIMFaflight::VecWIel::FILED_AIRSPEED_MACH;};
	if (txtrd.fields.size() > 11) {filed_altitude = atoi(txtrd.fields[11].c_str()); ixWIelValid += ImeIMFaflight::VecWIel::FILED_ALTITUDE;};
	if (txtrd.fields.size() > 12) {route = txtrd.fields[12]; ixWIelValid += ImeIMFaflight::VecWIel::ROUTE;};
	if (txtrd.fields.size() > 13) {actualdeparturetime = atol(txtrd.fields[13].c_str()); ixWIelValid += ImeIMFaflight::VecWIel::ACTUALDEPARTURETIME;};
	if (txtrd.fields.size() > 14) {estimatedarrivaltime = atol(txtrd.fields[14].c_str()); ixWIelValid += ImeIMFaflight::VecWIel::ESTIMATEDARRIVALTIME;};
	if (txtrd.fields.size() > 15) {actualarrivaltime = atol(txtrd.fields[15].c_str()); ixWIelValid += ImeIMFaflight::VecWIel::ACTUALARRIVALTIME;};
	if (txtrd.fields.size() > 16) {diverted = txtrd.fields[16]; ixWIelValid += ImeIMFaflight::VecWIel::DIVERTED;};
	if (txtrd.fields.size() > 17) {origin = txtrd.fields[17]; ixWIelValid += ImeIMFaflight::VecWIel::ORIGIN;};
	if (txtrd.fields.size() > 18) {destination = txtrd.fields[18]; ixWIelValid += ImeIMFaflight::VecWIel::DESTINATION;};
	if (txtrd.fields.size() > 19) {originName = txtrd.fields[19]; ixWIelValid += ImeIMFaflight::VecWIel::ORIGINNAME;};
	if (txtrd.fields.size() > 20) {originCity = txtrd.fields[20]; ixWIelValid += ImeIMFaflight::VecWIel::ORIGINCITY;};
	if (txtrd.fields.size() > 21) {destinationName = txtrd.fields[21]; ixWIelValid += ImeIMFaflight::VecWIel::DESTINATIONNAME;};
	if (txtrd.fields.size() > 22) {destinationCity = txtrd.fields[22]; ixWIelValid += ImeIMFaflight::VecWIel::DESTINATIONCITY;};

	while (txtrd.readLine()) {
		switch (txtrd.ixVLinetype) {
			case Txtrd::VecVLinetype::HEADER:
				if ((txtrd.il == 1) && (txtrd.ixVToken == VecVIme::IMEIAMFAFLIGHTWAYPOINT)) {
					imeiamfaflightwaypoint.readTxt(txtrd);
					continue;
			...
			default:
				throw SbeException(SbeException::TXTRD_CONTENT, {{"ime","ImeIMFaflight"}, {"lineno",to_string(lineno)}});
		};
	};
};
...
void IexBrlyFlt::ImeitemIMFaflight::readXML(
	...
	if (checkXPath(docctx, basexpath, lineno)) {
		if (extractStringUclc(docctx, basexpath, "srefIxBrlyVIop", "iop", srefIxBrlyVIop)) ixWIelValid += ImeIMFaflight::VecWIel::SREFIXBRLYVIOP;
...
void IexBrlyFlt::ImeIMFaflight::readTxt(
			Txtrd& txtrd
		) {
	...
	while (txtrd.readLine()) {
		switch (txtrd.ixVLinetype) {
			case Txtrd::VecVLinetype::DATA:
				...
				} else throw SbeException(SbeException::TXTRD_CONTENT, {{"ime","ImeIMFaflight"}, {"lineno",to_string(txtrd.linecnt)}});

			case Txtrd::VecVLinetype::FOOTER:
				if (txtrd.ixVToken == VecVIme::IMEIMFAFLIGHT) return;
				else throw SbeException(SbeException::TXTRD_TKNMISPL, {{"tkn",VecVIme::getSref(txtrd.ixVToken)}, {"lineno",to_string(txtrd.linecnt)}});
			...
			default:
				throw SbeException(SbeException::TXTRD_ENDTKN, {{"ime","ImeIMFaflight"}, {"lineno",to_string(txtrd.linecnt)}});
		};
	};

	if (txtrd.eof()) throw SbeException(SbeException::TXTRD_ENDTKN, {{"ime","ImeIMFaflight"}, {"lineno",to_string(txtrd.linecnt)}});
};
...
void IexBrlyFlt::ImeIMFaflight::writeTxt(
	...
	if (nodes.size() > 0) {
		...
		for (unsigned int i = 0; i < nodes.size(); i++) nodes[i]->writeTxt(outfile);
		outfile << "ImeIMFaflight.end" << endl;
	};
};
...
void IexBrlyFlt::parseFromFile(
	...
	if (xmlNotTxt) {
		...
		try {
			...
			Txtrd rd(fullpath, rectpath, "IexBrlyFlt", Version(""), VecVIme::getIx);
			readTxt(rd, imeimfaflight);
	};
};

void IexBrlyFlt::exportToFile(
			const string& fullpath
			, const bool xmlNotTxt
			, const bool shorttags
			, ImeIMFaflight& imeimfaflight
	...
	if (xmlNotTxt) {
		...
		startwriteFile(fullpath, &wr);
			writeXML(wr, shorttags, imeimfaflight);
		closewriteFile(wr);
		...
		writeTxt(txtfile, imeimfaflight);
		txtfile.close();
	};
};

void IexBrlyFlt::readTxt(
			Txtrd& txtrd
			, ImeIMFaflight& imeimfaflight
		) {
	while (txtrd.readLine()) {
		switch (txtrd.ixVLinetype) {
			case Txtrd::VecVLinetype::HEADER:
				if ((txtrd.il == 0) && (txtrd.ixVToken == VecVIme::IMEIMFAFLIGHT)) imeimfaflight.readTxt(txtrd);
				else throw SbeException(SbeException::TXTRD_TKNMISPL, {{"tkn",VecVIme::getSref(txtrd.ixVToken)}, {"lineno",to_string(txtrd.linecnt)}});
...
void IexBrlyFlt::readXML(
	...
	if (checkUclcXPaths(docctx, basexpath, basexpath, "IexBrlyFlt")) {
		...
		throw SbeException(SbeException::IEX_FILETYPE, {{"iexsref","IexBrlyFlt"}});
	};
};
...
void IexBrlyFlt::writeTxt(
	...
	outfile << "IexBrlyFlt v" BRLY_VERSION << endl;
...
map<uint,uint> IexBrlyFlt::icsBrlyVIopInsAll() {
	return {{(uint)VecVIme::IMEIAMFAFLIGHTWAYPOINT,VecBrlyVIop::INS},{(uint)VecVIme::IMEIMFAFLIGHT,VecBrlyVIop::INS}};
};
