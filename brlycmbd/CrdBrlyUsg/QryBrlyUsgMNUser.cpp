...
void QryBrlyUsgMNUser::rerun(
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYUSRRUSGMOD_USGEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefUsg);
};
...
void QryBrlyUsgMNUser::handleCall(
	...
	if (call->ixVCall == VecBrlyVCall::CALLBRLYUSRRUSGMOD_USGEQ) {
		call->abort = handleCallBrlyUsrRusgMod_usgEq(dbsbrly, call->jref);
...
bool QryBrlyUsgMNUser::handleCallBrlyStubChgFromSelf(
			DbsBrly* dbsbrly
		) {
	bool retval = false;
